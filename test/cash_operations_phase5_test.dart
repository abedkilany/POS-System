...
void main() {
  test('store-wide negative cash policy is the only outflow authority',
      () async {
    ...
    await expectLater(
      AccountingService.ensureCashOutflowAllowed(
    ...
    final restored = StoreProfile.fromJson(
      StoreProfile.defaults.copyWith(allowNegativeCashBalance: true).toJson(),
    );
    expect(restored.allowNegativeCashBalance, isTrue);
  });
  ...
  test('foundation creates Phase 5 cash operations schema', () async {
    ...
    final transferColumns =
        await db.customSelect("PRAGMA table_info('cash_transfers')").get();
    final names =
        transferColumns.map((row) => row.data['name']?.toString()).toSet();
    expect(
        names,
        containsAll(<String>[
          'transfer_kind',
  ...
  test('deposit writes operation, journal, ledger and balance exactly once',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
  ...
  test('withdrawal and expense reduce balance and are visible in ledger',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    ...
    expect(rows.map((row) => row.read<String>('type')).toSet(),
        containsAll(<String>['cash_withdrawal', 'expense']));
  });

  test('cash out rejects insufficient balance when negative cash is disabled',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db, balance: 10);
  ...
  test(
      'legacy posted expense uses journal + cash operation + ledger + balance exactly once',
      () async {
    final db = await _openAccountingDb();
    ...
    final occurredAt =
        DateTime.parse(ledger.read<String>('occurred_at')).toUtc();
    ...
    final reportedShift =
        openShiftReport.firstWhere((item) => item.id == 'shift-p5');
  ...
  test(
      'vault transfer posts one journal, two ledger legs, both balances and is idempotent',
      () async {
    final db = await _openAccountingDb();
    addTearDown(SqliteMigrationManager.resetForTesting);
    ...
    expect(ledger.map((r) => r.read<String>('direction')).toSet(),
        <String>{'in', 'out'});
  ...
  test(
      'vault transfer also supports vault to drawer with mirrored ledger directions',
      () async {
    final db = await _openAccountingDb();
    addTearDown(SqliteMigrationManager.resetForTesting);
    await _seedTransferLocations(db);
    ...
    expect(
      legs
          .map((r) =>
              "${r.read<String>('direction')}:${r.read<String>('cash_location_id')}:${r.read<String>('cash_drawer_session_id')}")
          .toSet(),
  ...
  test(
      'shift transfer requires matching open sessions and posts both session ledger legs',
      () async {
    final db = await _openAccountingDb();
    addTearDown(SqliteMigrationManager.resetForTesting);
    await _seedTransferLocations(db);
  ...
  test(
      'shift transfer rejects missing, closed or mismatched sessions inside service layer',
      () async {
    final db = await _openAccountingDb();
    addTearDown(SqliteMigrationManager.resetForTesting);
    await _seedTransferLocations(db);
...
void _phase5ExpensePostingIntegrityTests() {
  test(
      'bulk legacy expenses roll back all cash/accounting when one item cannot be funded',
      () async {
    final db = await _openAccountingDb();
    addTearDown(SqliteMigrationManager.resetForTesting);
    await _seedDrawer(db, balance: 10);
