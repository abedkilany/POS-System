...
void main() {
  ...
  test('phase 8 snapshot catalog includes authoritative cash/accounting data',
    ...
    for (final name in <String>[
      ...
      expect(
          LocalDatabaseService.phase8AccountingSnapshotTables, contains(name));
    }
  });
  ...
  test('cash operation emits Phase 8 mutation only after successful commit',
      () async {
    ...
    final result = await CashOperationService(db, authorization: _authorization).deposit(
  ...
  test('failed cash transaction does not emit Phase 8 mutation', () async {
    ...
    AccountingService.setMutationListener(() => mutations++);
    await expectLater(
      CashOperationService(db, authorization: _authorization).withdrawal(
        cashLocationId: 'drawer-p8',
        cashDrawerSessionId: 'shift-p8',
        counterpartAccountId: 'acc_owner_capital',
        amount: 1000,
  ...
  test('closing a cash drawer is strict and cannot silently succeed twice',
      () async {
    final db = await _phase8Db();
    addTearDown(db.close);
    addTearDown(() {
  ...
  test('older or reopened sync session cannot overwrite a closed local shift',
      () async {
    final db = VentioDriftDatabase(NativeDatabase.memory());
    await db.initializeFoundation();
    await LocalDatabaseService.useSqliteDatabaseForTesting(db);
  ...
  test('assigned cash drawer remains discoverable after its shift is closed',
      () async {
    final db = await _phase8Db();
    addTearDown(db.close);
    addTearDown(() {
  ...
  test('unallocated account receipt still posts voucher and cash ledger',
      () async {
    final db = await _phase8Db();
    addTearDown(db.close);
    addTearDown(() {
