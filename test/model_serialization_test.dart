...
void main() {
  ...
  group('JSON round trips', () {
    test('Product round trip excludes derived inventory stock from product payloads', () {
      ...
      final decoded = Product.fromJson(payload);
    ...
    test('Purchase round trip preserves nested items and totals', () {
      final purchase = Purchase(
          ...
          items: const [
            PurchaseItem(
                productId: 'p1', productName: 'Beans', quantity: 3, unitCost: 7)
          ],
          createdAt: created,
          updatedAt: updated);
    ...
    test('StoreProfile round trip preserves receipt settings', () {
      ...
      expect(decoded.allowNegativeStock, isTrue);
      expect(StoreProfile.fromJson({}).allowNegativeStock, isFalse);
    });
    ...
    test('AppUser round trip preserves permission overrides', () {
      final user = AppUser(
          ...
          passwordHash: 'hash',
          roleId: 'cashier',
          extraPermissions: {'reports.view'},
          deniedPermissions: {'products.delete'},
          createdAt: created);
  ...
  group('Defensive JSON defaults', () {
    test('Product prefers nameEn when name is missing', () {
      final product = Product.fromJson(
          {'id': 'p1', 'nameEn': 'Coffee', 'price': 1, 'cost': 0, 'stock': 0});
