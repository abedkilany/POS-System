...
Future<VentioDriftDatabase> _openDb() async {
  ...
  // This suite exercises migration from the retired pre-schema-25 layout.
...
void main() {
  ...
  test('backfill skips an already-applied migration adjustment on rerun',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
  ...
  test('read model stops falling back to legacy stock after backfill completes',
    ...
    expect(
      integrity.issues.any((issue) =>
          issue.productId == 'p2' && issue.classification == 'legacy_unassigned'),
      isFalse,
    );
  });
}
