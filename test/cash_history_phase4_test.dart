...
void main() {
  test(
      'phase 4 history supports SQL filters, search, paging summary and invoice search',
      () async {
    ...
    final invoiceSearch =
        await service.list(cashLocationId: 'drawer-1', search: 'INV-900');
    expect(invoiceSearch, hasLength(1));
    expect(invoiceSearch.single.id, 'cash-in-1');

    final incoming =
        await service.list(cashLocationId: 'drawer-1', direction: 'in');
  ...
  test(
      'phase 4 details resolve drawer, shift, allocations and journal reference',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seed(db);
  ...
  test(
      'current shift range starts at session opened_at instead of start of today',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await db.customInsert(
  ...
  test(
      'cash page uses phase 4 ledger history instead of accountTransactions history',
      () {
    final cashPage =
        File('lib/features/cash/cash_page.dart').readAsStringSync();
    final historyPanel =
        File('lib/features/cash/cash_history_panel.dart').readAsStringSync();
