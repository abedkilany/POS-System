...
void main() {
  ...
  test('allows an unallocated negative batch remainder when enabled', () async {
    final db = VentioDriftDatabase(NativeDatabase.memory());
    await db.initializeFoundation();
    addTearDown(db.close);
    final now = DateTime.utc(2026, 8, 14);
    final product = Product(
      id: 'negative-batch-product',
      name: 'Negative batch product',
      code: 'NBP',
      price: 2,
      cost: 1,
      stock: 0,
    ...
    final allocations =
        await db.transaction(() => service.allocateFefoInTransaction(
              product: product,
              warehouseId: 'main',
              quantity: 2,
              saleDate: now,
    ...
    expect(allocations.single.batchId, isEmpty);
