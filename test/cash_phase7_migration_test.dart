...
void main() {
  ...
  test(
      'legacy receipt becomes voucher + journal + ledger without moving cash twice',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
  ...
  test(
      'legacy supplier cash payment becomes payment voucher without moving cash twice',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    ...
    final voucher = await db
        .customSelect(
            "SELECT amount, unallocated_amount FROM payment_vouchers WHERE id = 'legacy_payment_legacy-pay-1'")
        .getSingle();
    expect(voucher.read<double>('amount'), 75);
    expect(voucher.read<double>('unallocated_amount'), 75);
    final ledgerCount = await db
        .customSelect(
            "SELECT COUNT(*) AS c FROM cash_ledger_transactions WHERE reference_type = 'payment_voucher' AND reference_id = 'legacy_payment_legacy-pay-1' AND deleted_at = ''")
        .getSingle();
    expect(ledgerCount.read<int>('c'), 1);
    final balance = await db
        .customSelect(
            "SELECT current_balance FROM cash_locations WHERE id = 'drawer-supplier'")
        .getSingle();
    expect(balance.read<double>('current_balance'), 500);
  });

  test('cash voucher without resolvable Cash Location blocks Phase 7',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    ...
    expect(
        report.issues.any((issue) =>
            issue.type == 'missing_cash_location' ||
            issue.type == 'missing_receipt_cash_ledger'),
        isTrue);
    final run = await db.customSelect(
        'SELECT status FROM cash_phase7_runs WHERE id = ?',
        variables: <Variable<Object>>[
          Variable<String>(report.runId)
        ]).getSingle();
  ...
  test('allocation/voucher mismatch is reported as blocking', () async {
    ...
    expect(
        report.issues
            .any((issue) => issue.type == 'voucher_allocation_mismatch'),
        isTrue);
    expect(
        report.issues
            .any((issue) => issue.type == 'invalid_payment_allocation'),
        isTrue);
  });
}
