...
void main() {
  ...
  test('rejects negative balance when policy is disabled', () async {
    final db = await _openDb();
    addTearDown(db.close);
    final service = StockTransactionService(db, defaultStoreId: 'store-1');
  ...
  test('allows negative balance when policy is enabled', () async {
    final db = await _openDb();
    addTearDown(db.close);
    final service = StockTransactionService(
      db,
      defaultStoreId: 'store-1',
      allowNegativeStockResolver: (_, __) => true,
    );
  ...
  test('duplicate movement idempotency key does not re-apply balance',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
  ...
  test('completed operation is replay-safe after reopening the database',
      () async {
    ...
    final replayService =
        StockTransactionService(reopened, defaultStoreId: 'store-1');
  ...
  test('initialization is idempotent and keeps existing balances intact',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
  ...
  test('integrity checker reports mismatch without mutating data', () async {
    ...
    // Corrupt only the projection to prove the checker reports the mismatch
    // without silently repairing it.
    await db.customUpdate(
      "UPDATE warehouse_inventory SET quantity = 5 WHERE store_id = 'store-1' AND warehouse_id = 'wh-1' AND product_id = 'p1'",
    );
  ...
  test('migrates an old schema and keeps legacy stock movements readable',
      () async {
    final db = VentioDriftDatabase(NativeDatabase.memory());
    addTearDown(db.close);
