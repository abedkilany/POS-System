...
void main() {
  ...
  test('cash receipt supports multi-invoice allocation and unallocated credit',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db);
    await _seedSale(db,
        id: 'sale-1', invoiceNo: 'INV-1', customerId: 'cust-1', total: 50);
    await _seedSale(db,
        id: 'sale-2', invoiceNo: 'INV-2', customerId: 'cust-1', total: 100);
    ...
    final sale1 = await db
        .customSelect("SELECT paid_amount FROM sales WHERE id = 'sale-1'")
        .getSingle();
    final sale2 = await db
        .customSelect("SELECT paid_amount FROM sales WHERE id = 'sale-2'")
        .getSingle();
    expect((sale1.data['paid_amount'] as num).toDouble(), 30);
    expect((sale2.data['paid_amount'] as num).toDouble(), 40);

    final ledger = await CashLedgerService(db)
        .list(referenceType: 'receipt_voucher', referenceId: 'receipt-1');
    expect(ledger, hasLength(1));
    expect(ledger.single.amount, 80,
        reason: 'ledger tracks cash received, not only allocated amount');
    ...
    final lines = await db.customSelect(
      ...
      variables: <Variable<Object>>[
        Variable<String>(journal.single.data['id'].toString())
      ],
    ).get();
  ...
  test(
    ...
    final purchase = await db
        .customSelect(
            "SELECT paid_amount FROM purchases WHERE id = 'purchase-1'")
        .getSingle();
    expect((purchase.data['paid_amount'] as num).toDouble(), 50);

    final ledger = await CashLedgerService(db)
        .list(referenceType: 'payment_voucher', referenceId: 'payment-1');
    expect(ledger, hasLength(1));
  ...
  test('voucher idempotency does not duplicate allocations, cache, or ledger',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db);
    await _seedSale(db,
        id: 'sale-1', invoiceNo: 'INV-1', customerId: 'cust-1', total: 100);
    final service = PaymentVoucherService(db);
    ...
    final first = await service.createReceipt(
      ...
      allocations: const <PaymentAllocationDraft>[
        PaymentAllocationDraft(referenceId: 'sale-1', amount: 25)
      ],
    );
    ...
    expect(journal, hasLength(1),
        reason: 'retry must not duplicate accounting posting');
  });

  test(
      'retry backfills a missing voucher journal without duplicating financial effects',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db);
    ...
    await db.customUpdate(
      ...
      variables: <Variable<Object>>[
        Variable<String>(firstJournal.data['id'].toString())
      ],
    );
  ...
  test('rejects over-allocation and allocation to another customer', () async {
    ...
    await _seedDrawer(db);
    await _seedSale(db,
        id: 'sale-1', invoiceNo: 'INV-1', customerId: 'cust-2', total: 30);
    final service = PaymentVoucherService(db);
    ...
    await expectLater(
      service.createReceipt(
        ...
        cashDrawerSessionId: 'shift-1',
        allocations: const <PaymentAllocationDraft>[
          PaymentAllocationDraft(referenceId: 'sale-1', amount: 40)
        ],
      ),
      throwsStateError,
    );
  ...
  test(
      'non-cash receipt does not require drawer and does not touch cash ledger',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedSale(db,
    ...
    await service.createReceipt(
      ...
      allocations: const <PaymentAllocationDraft>[
        PaymentAllocationDraft(referenceId: 'sale-1', amount: 15)
      ],
    );

    final ledger =
        await CashLedgerService(db).list(referenceId: 'bank-receipt');
    ...
    final journal = await db
        ...
        .get();
    expect(journal, hasLength(1),
        reason: 'non-cash vouchers still require accounting posting');
    final lines = await db.customSelect(
  ...
  test(
      'legacy cash vouchers are backfilled into Cash Ledger without changing balance',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db);
  ...
  test('pre-voucher legacy account payments are backfilled into Cash Ledger',
    ...
    expect(rows, hasLength(2),
        reason: 'pre-voucher backfill must be idempotent');
    ...
    expect((balance.data['current_balance'] as num).toDouble(), 100,
        reason:
            'legacy account-transaction backfill must not change live balance');
  });
  ...
  test('Phase 9 receipt/payment persist account ledger in voucher transaction',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db);
    await _seedSale(db,
        id: 'sale-p9', invoiceNo: 'INV-P9', customerId: 'cust-p9', total: 50);
    await _seedPurchase(db,
        id: 'purchase-p9',
        purchaseNo: 'PUR-P9',
        supplierId: 'sup-p9',
    ...
    await service.createReceipt(
      ...
      allocations: const <PaymentAllocationDraft>[
        PaymentAllocationDraft(
            referenceId: 'sale-p9', referenceNumber: 'INV-P9', amount: 20),
      ],
    );
    final customerMovement = await db
        .customSelect(
    ...
    await service.createPayment(
      ...
      allocations: const <PaymentAllocationDraft>[
        PaymentAllocationDraft(
            referenceId: 'purchase-p9', referenceNumber: 'PUR-P9', amount: 15),
      ],
    );
    final supplierMovement = await db
        .customSelect(
  ...
  test('Phase 9 voucher reversal persists matching account-ledger reversal',
    ...
    await _seedSale(db,
        id: 'sale-rev-p9',
        invoiceNo: 'INV-REV-P9',
        customerId: 'cust-rev-p9',
        total: 40);
    final service = PaymentVoucherService(db);
    await service.createReceipt(
      id: 'receipt-rev-p9',
      ...
      allocations: const <PaymentAllocationDraft>[
        PaymentAllocationDraft(
            referenceId: 'sale-rev-p9',
            referenceNumber: 'INV-REV-P9',
            amount: 10),
      ],
    );
    expect(
        await service.reverseReceiptVoucher(
            voucherId: 'receipt-rev-p9', deviceId: 'dev-1'),
        isTrue);
  ...
  test(
      'Phase 9 account-ledger failure rolls back the whole receipt transaction',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedDrawer(db);
    await _seedSale(db,
        id: 'sale-fail-p9',
        invoiceNo: 'INV-FAIL-P9',
        customerId: 'cust-fail-p9',
        total: 50);
    ...
    await expectLater(
      service.createReceipt(
        ...
        allocations: const <PaymentAllocationDraft>[
          PaymentAllocationDraft(
              referenceId: 'sale-fail-p9',
              referenceNumber: 'INV-FAIL-P9',
              amount: 10),
        ],
      ),
      throwsA(anything),
    ...
    expect(
        await db
            .customSelect(
                "SELECT id FROM receipt_vouchers WHERE id = 'receipt-fail-p9'")
            .getSingleOrNull(),
        isNull);
    expect(
        await db
            .customSelect(
                "SELECT id FROM journal_entries WHERE reference_type = 'receipt_voucher' AND reference_id = 'receipt-fail-p9'")
            .getSingleOrNull(),
        isNull);
    expect(
        await db
            .customSelect(
                "SELECT id FROM cash_ledger_transactions WHERE reference_type = 'receipt_voucher' AND reference_id = 'receipt-fail-p9'")
            .getSingleOrNull(),
        isNull);
    final balance = await db
        .customSelect(
    ...
    expect((balance.data['current_balance'] as num).toDouble(), 100);
    final sale = await db
        .customSelect("SELECT paid_amount FROM sales WHERE id = 'sale-fail-p9'")
        .getSingle();
