  ...
  group('Backup validation contract', () {
    Map<String, dynamic> validBackup({
      ...
      return <String, dynamic>{
        ...
        'storeProfile':
            StoreProfile.defaults.copyWith(name: storeName).toJson(),
        'products': products,
        'customers': customers,
        'sales': sales,
        'suppliers': suppliers,
    ...
    test('rejects non-object, malformed, and incomplete backups safely', () {
      ...
      final incomplete = store.validateBackupJson(
          jsonEncode(<String, dynamic>{'products': const []}));
      expect(incomplete.isValid, isFalse);
      expect(incomplete.errorMessage,
          contains('Missing required backup sections'));
    });

    test('uses a safe default store name when profile name is absent', () {
      final backup = validBackup(storeName: '')
        ..['storeProfile'] = <String, Object?>{};
    ...
    test('preserves the negative-stock policy in backup profile data', () {
      final backup = validBackup()
        ..['storeProfile'] =
            StoreProfile.defaults.copyWith(allowNegativeStock: true).toJson();
      final result = AppStore().validateBackupJson(jsonEncode(backup));

      expect(result.isValid, isTrue);
      final profile = StoreProfile.fromJson(
        Map<String, dynamic>.from(backup['storeProfile'] as Map),
      );
      expect(profile.allowNegativeStock, isTrue);
    });
    ...
    test('decodes a local backup archive into backup JSON', () {
      ...
      final archive = Archive()
        ..addFile(ArchiveFile(
            'backup.json', 0, utf8.encode(jsonEncode(validBackup()))))
        ..addFile(ArchiveFile(
            'manifest.json', 0, utf8.encode('{"type":"local-auto-backup"}')));
