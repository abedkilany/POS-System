...
void main() {
  ...
  test(
      'expense post commits document, accounting, cash and compatibility ledger together',
      () async {
  ...
  test(
      'expense cancel reverses cash and ledger and marks expense cancelled atomically',
      () async {
    final db = await _openDb();
    addTearDown(db.close);
    await _seedExpenseDrawer(db);
    ...
    expect(await _balance(db), 100);
    final shiftRow = await db
        .customSelect(
          "SELECT expected_cash FROM cash_drawer_sessions WHERE id = 'shift-exp-p9'",
        )
        .getSingle();
    expect((shiftRow.data['expected_cash'] as num).toDouble(), 100);
