...
void main() {
  ...
  test('legacy purchases payload_json column does not break typed reads',
      () async {
    final db = VentioDriftDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    await db.initializeFoundation();
    ...
    await db.customStatement(
      "ALTER TABLE purchases ADD COLUMN payload_json TEXT NOT NULL DEFAULT ''",
    );
    ...
    await BusinessSqliteStore.upsertEntityPayload(
      ...
      BusinessSqliteStore.purchasesKey,
      <String, dynamic>{
        'id': 'purchase-legacy-payload-1',
        'purchaseNo': 'PO-LEGACY-1',
        'supplierId': 'supplier-legacy-1',
    ...
    expect(purchases, hasLength(1));
    expect(purchases.single.purchaseNo, 'PO-LEGACY-1');
    expect(purchases.single.items.single.productName, 'Legacy Product');
  });

  test(
      'business tables stay payload-json free and roles users use typed columns',
      () async {
    final db = VentioDriftDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    await db.initializeFoundation();
    ...
    final rolesJson = await BusinessSqliteStore.readEntityListJsonByKey(
        db, BusinessSqliteStore.rolesKey);
    final usersJson = await BusinessSqliteStore.readEntityListJsonByKey(
        db, BusinessSqliteStore.usersKey);
