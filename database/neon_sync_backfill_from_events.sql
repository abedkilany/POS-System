...
select se.store_id, 'product', item->>'id', item, 'upsert', se.created_at
...
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'customer', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'customers', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'sale', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'sales', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'supplier', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'suppliers', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'expense', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'expenses', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'category', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'categories', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'brand', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'brands', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'unit', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'units', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'role', item->>'id', item, 'upsert', se.created_at
from sync_events se, jsonb_array_elements(coalesce(se.payload->'roles', '[]'::jsonb)) item
where se.operation = 'restore_snapshot' and item ? 'id'
on conflict (store_id, entity_type, entity_id) do update set payload = excluded.payload, operation = excluded.operation, updated_at = excluded.updated_at;

insert into entity_snapshots (store_id, entity_type, entity_id, payload, operation, updated_at)
select se.store_id, 'user', item->>'id', item, 'upsert', se.created_at
