...
extension ProductQuantityTypeJson on ProductQuantityType {
  String get code =>
      this == ProductQuantityType.measurable ? 'measurable' : 'countable';

  static ProductQuantityType fromCode(String? value) {
    return value == 'measurable'
        ? ProductQuantityType.measurable
        : ProductQuantityType.countable;
  }
}
...
class ProductSaleUnit {
  ...
  factory ProductSaleUnit.fromJson(Map<String, dynamic> json) =>
      ProductSaleUnit(
        ...
        originalPrice:
            (json['originalPrice'] as num? ?? json['price'] as num? ?? 0)
                .toDouble(),
        originalCurrency:
...
class Product {
  ...
  /// Derived total quantity from authoritative `warehouse_inventory`.
  ///
  /// This value is an in-memory/query projection only. It is never persisted in
  /// `products`, never synchronized in product payloads, and must not be used as
  ...
  final bool expiryTrackingEnabled, expiryEntryRequired;
  final int expiryAlertDays, defaultShelfLifeDays, minimumReceiptShelfLifeDays;
  ...
  bool get requiresExpiryDate => trackStock && expiryTrackingEnabled;
  bool get forbidsExpiryDate => trackStock && !expiryTrackingEnabled;
  double get stockCostValue => usdCost * stock;
  double get stockRetailValue => usdPrice * stock;
  bool get allowsDecimalQuantity =>
      quantityType == ProductQuantityType.measurable;
  List<ProductSaleUnit> get effectiveSaleUnits {
    final base = ProductSaleUnit(
        id: 'base',
        name: unit,
        conversionToBase: 1,
        price: price,
        ...
        barcode: barcode,
        isDefault: true);
    final merged = <ProductSaleUnit>[
      base,
      ...saleUnits.where((item) => item.conversionToBase > 0)
    ];
    final seen = <String>{};
    return merged.where((item) {
      final key = item.id.trim().isNotEmpty
          ? item.id
          : '${item.name}-${item.conversionToBase}-${item.barcode}';
      return seen.add(key);
    }).toList();
  }
  ...
  List<ProductSaleUnit> get effectivePurchaseUnits {
    ...
    final merged = <ProductSaleUnit>[
      base,
      ...sourceUnits.where((item) => item.conversionToBase > 0)
    ];
    final seen = <String>{};
    return merged.where((item) {
      final key = item.id.trim().isNotEmpty
  ...
  Product copyWith(
      ...
      bool? expiryEntryRequired,
      int? expiryAlertDays,
      int? defaultShelfLifeDays,
      int? minimumReceiptShelfLifeDays,
      bool? isActive,
      DateTime? createdAt,
      DateTime? updatedAt,
      DateTime? deletedAt,
    ...
    final resolvedUsdPrice = usdPrice ??
        (price != null && resolvedOriginalCurrency == 'USD'
            ? resolvedPrice
            : this.usdPrice);
    final resolvedUsdCost = usdCost ??
        (cost != null && resolvedCostCurrency == 'USD'
            ? resolvedCost
            : this.usdCost);
    ...
    return Product(
        ...
        usdCost: resolvedUsdCost,
        costExchangeRateAtEntry:
            costExchangeRateAtEntry ?? this.costExchangeRateAtEntry,
        ...
        expiryEntryRequired: expiryEntryRequired ?? this.expiryEntryRequired,
        ...
        minimumReceiptShelfLifeDays:
            minimumReceiptShelfLifeDays ?? this.minimumReceiptShelfLifeDays,
  ...
  Map<String, dynamic> toJson() => {
        ...
        'usdCost': usdCost,
        'costExchangeRateAtEntry': costExchangeRateAtEntry,
        'originalPrice': originalPrice,
        'originalCurrency': originalCurrency,
        'usdPrice': usdPrice,
        ...
        'unit': unit,
        'quantityType': quantityType.code,
        'saleUnits': saleUnits.map((unit) => unit.toJson()).toList(),
        'purchaseUnits': purchaseUnits.map((unit) => unit.toJson()).toList(),
        ...
        'minimumReceiptShelfLifeDays': minimumReceiptShelfLifeDays,
        'isActive': isActive,
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    ...
    final originalCurrency =
        (json['originalCurrency'] as String? ?? 'USD').toUpperCase();
    final rawCost =
        (json['cost'] as num? ?? json['usdCost'] as num? ?? 0).toDouble();
    final costCurrencyRaw =
        (json['costCurrency'] as String? ?? 'USD').toUpperCase();
    ...
    return Product(
        ...
        price: price,
        cost: rawCost,
        originalCost: (json['originalCost'] as num? ?? rawCost).toDouble(),
        costCurrency: costCurrency,
        usdCost: (json['usdCost'] as num? ?? rawCost).toDouble(),
        costExchangeRateAtEntry:
            (json['costExchangeRateAtEntry'] as num? ?? 0).toDouble(),
        originalPrice: (json['originalPrice'] as num? ?? price).toDouble(),
        originalCurrency: originalCurrency == 'LBP' ? 'LBP' : 'USD',
        usdPrice: (json['usdPrice'] as num? ?? price).toDouble(),
        exchangeRateAtEntry:
            (json['exchangeRateAtEntry'] as num? ?? 0).toDouble(),
        stock: (json['inventoryStock'] as num? ?? 0).toDouble(),
        ...
        quantityType:
            ProductQuantityTypeJson.fromCode(json['quantityType'] as String?),
        saleUnits: (json['saleUnits'] as List<dynamic>? ?? const [])
            .map((item) => ProductSaleUnit.fromJson(
                Map<String, dynamic>.from(item as Map)))
            .toList(),
        purchaseUnits: (json['purchaseUnits'] as List<dynamic>? ?? const [])
            .map((item) => ProductSaleUnit.fromJson(
                Map<String, dynamic>.from(item as Map)))
            .toList(),
        ...
        minimumReceiptShelfLifeDays:
            (json['minimumReceiptShelfLifeDays'] as num? ?? 0).toInt(),
