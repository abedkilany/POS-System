...
class AppStore extends ChangeNotifier {
  ...
  String _currentRole = 'admin'; // retained for startup compatibility
  String _deviceId = '';
  ...
  Future<bool> _postPurchaseAccounting(Purchase purchase) async {
    try {
      return await _traceAsync<bool>(
        ...
        'accounting_post',
        () => AccountingService.recordPurchase(purchase, paymentPostedSeparately: true),
      );
    } catch (error, stackTrace) {
      unawaited(
        AppLogger.error(
  ...
  void _recordPurchaseInvoiceCorrectionLedger(
    ...
    _upsertAccountTransactionInternal(
      AccountTransaction(
        ...
        debit: total,
        credit: 0,
        note: reason.trim().isEmpty ? 'Purchase corrected' : reason.trim(),
        createdAt: now,
        updatedAt: now,
        deviceId: _deviceId,
        storeId: appIdentity.storeId,
  ...
  void _recordSaleInvoiceCorrectionLedger(
    ...
    final accountId = sale.customerId.trim().isNotEmpty
        ...
        : sale.customerName.trim();
    if (accountId.isEmpty) return;
    final invoiceId = '${sale.id}-sale-invoice';
    final invoiceIndex = _accountTransactions.indexWhere(
      (transaction) => transaction.id == invoiceId,
    );
    final total = invoiceIndex == -1
        ? sale.invoiceTotal
  ...
  Future<void> addOrUpdateProduct(Product product) async {
    ...
    if (enablesExpiryTracking && normalizedProduct.stock > 0.000001) {
      throw StateError(
  ...
  Future<void> createAndPostExpensesBulk(List<Expense> expenses) async {
    ...
    final now = DateTime.now();
    final prepared = <({Expense source, Expense candidate, int index, bool isCreate})>[];
  ...
  Future<Map<String, String>> _openCashVoucherContext() async {
    final sqliteDb = SqliteMigrationManager.database;
    if (!LocalDatabaseService.isSqliteAuthoritative || sqliteDb == null) {
      throw StateError('Payment vouchers require the SQLite authoritative store.');
    }
    final branchFilter = appIdentity.branchId.trim().isEmpty
        ? ''
        : 'AND cds.branch_id = ?';
  ...
  Future<Sale> _settleSalePaymentInternal({
    ...
    final sqliteDb = SqliteMigrationManager.database;
    if (!LocalDatabaseService.isSqliteAuthoritative || sqliteDb == null) {
      throw StateError('Sale settlement requires the SQLite authoritative store.');
    }
  ...
  Future<Purchase> settlePurchasePayment({
    ...
    final sqliteDb = SqliteMigrationManager.database;
    if (!LocalDatabaseService.isSqliteAuthoritative || sqliteDb == null) {
      throw StateError('Purchase settlement requires the SQLite authoritative store.');
    }
  ...
  Future<Purchase> createPurchase({
    ...
    var purchase = Purchase(
      ...
      items: items,
      note: note,
      paymentStatus: normalizedPaidAmount > 0 ? 'credit' : normalizedPaymentStatus,
      paymentMethod: normalizedPaymentMethod,
      paidAmount: 0,
    ...
    if (receiveNow && normalizedPaidAmount > 0) {
      throw StateError('Purchase payments require the SQLite authoritative store.');
    }
    _putPurchaseAtIndex(purchase, _purchases.length);
    _recordSyncChange(
      entityType: 'purchase',
  ...
  Future<Purchase> editReceivedPurchase({
    ...
    final normalizedStatus = paymentStatus.trim().toLowerCase() == 'credit'
        ...
        : paymentStatus.trim().toLowerCase() == 'partial'
            ? 'partial'
            : 'paid';
    final requestedPaid = normalizedStatus == 'paid'
        ? total
        : normalizedStatus == 'credit'
            ? 0.0
            : (paidAmount ?? current.paidAmount).clamp(0, total).toDouble();
    if (total + 0.000001 < current.paidAmount) {
      throw StateError('Purchase total cannot be reduced below allocated payments. Reverse payments first.');
    }
    if (requestedPaid + 0.000001 < current.paidAmount) {
      throw StateError('Payments cannot be reduced by editing the purchase. Use payment reversal.');
    }
    if (current.paidAmount > 0 && supplierId.trim() != current.supplierId.trim()) {
      throw StateError('Supplier cannot be changed while allocated payments exist.');
    }
    final paymentDelta = (requestedPaid - current.paidAmount).clamp(0, double.infinity).toDouble();
    final normalizedPaid = current.paidAmount.clamp(0, total).toDouble();
    if (paymentDelta > 0) {
      await AccountingService.validatePurchasePayment(
        paymentMethod: paymentMethod,
        paidAmount: paymentDelta,
    ...
    if (paymentDelta > 0) {
      updated = await settlePurchasePayment(
        purchaseId: updated.id,
        amount: paymentDelta,
        paymentMethod: paymentMethod,
        notes: 'Payment added during purchase correction ${updated.purchaseNo}',
        idempotencyKey: '${updated.id}:edit:$editVersion:payment',
        date: now,
      );
    }
  ...
  Future<void> receivePurchase(
    ...
    final plannedPayment = purchase.paidAmount > 0
    ...
    if (LocalDatabaseService.isSqliteAuthoritative && sqliteDb != null) {
      ...
      await sqliteDb.transaction(() async {
        ...
        for (var lineIndex = 0;
          ...
          if (product.expiryTrackingEnabled) {
            final batchAllocations = await batchService.receiveInTransaction(
              product: product,
              warehouseId: warehouseId,
            ...
            for (var batchIndex = 0;
              ...
              receiptMovements.add(StockMovement(
                id: '${received.id}-$lineIndex-${allocation.batchId}-purchase-receive',
                productId: item.productId,
                productName: item.productName,
                type: 'purchase_receive',
                quantity: allocation.quantity,
                ...
                documentLineId: '${received.id}-line-$lineIndex',
                idempotencyKey:
                    '${received.id}:purchase_receive:$lineIndex:$batchIndex',
                createdAt: now,
                updatedAt: now,
                deviceId: received.deviceId,
            ...
            receiptMovements.add(StockMovement(
              ...
              sourceMovementId: '',
              reversalOfMovementId: '',
              idempotencyKey: '${received.id}:purchase_receive:$lineIndex',
        ...
        if (receiptMovements.isNotEmpty) {
          await stockService.recordMovementsInTransaction(
            ...
            movementGroupId: received.id,
            idempotencyKey: '${received.id}:purchase_receive',
            movements: receiptMovements,
            storeId: received.storeId,
            branchId: received.branchId,
            deviceId: received.deviceId,
      ...
      await sqliteDb.transaction(() async {
        ...
        for (final line in countedLines) {
          final warehouseId = session.warehouseId.isEmpty
              ? Warehouse.defaultId
              : session.warehouseId;
          final theoreticalAtCount = await warehouseStockFromSqlite(
            line.productId,
            warehouseId: warehouseId,
          );
          final delta =
          ...
          movements.add(
            StockMovement(
              ...
              documentLineId: '${session.id}-line-${line.productId}',
              idempotencyKey: '${session.id}:count:${line.productId}',
              unitCost: _findProductById(line.productId)?.usdCost ?? 0,
              createdAt: now,
              updatedAt: now,
              deviceId: _deviceId,
              storeId: appIdentity.storeId,
  ...
  Future<void> adjustExpiryBatchStock({
    ...
    await db.transaction(() async {
      await BatchInventoryService(db).adjustSpecificBatchInTransaction(
        product: product,
        warehouseId: warehouse.id,
        batchId: batchId,
  ...
  Future<void> adjustStock({
    ...
    if (LocalDatabaseService.isSqliteAuthoritative &&
      ...
      final operationId = '${now.microsecondsSinceEpoch}-$productId-adjustment';
      ...
      await _traceAsync<void>('inventory.adjustStock', 'sqlite_transaction',
        ...
        await sqliteDb.transaction(() async {
          if (product.expiryTrackingEnabled) {
            final batchService = BatchInventoryService(sqliteDb);
            final allocations = quantityDelta < 0
                ? await batchService.allocateFefoInTransaction(
                    product: product,
                    warehouseId: resolvedWarehouse.id,
                ...
                : await batchService.addStockInTransaction(
                    product: product,
                    warehouseId: resolvedWarehouse.id,
                    sourceType: 'manual_adjustment',
                    sourceId: operationId,
                    expectedQuantity: quantityDelta,
              ...
              for (var batchIndex = 0;
                ...
                baseMovement.copyWith(
                  ...
                  quantity: quantityDelta < 0
                      ? -allocations[batchIndex].quantity
                      : allocations[batchIndex].quantity,
                  batchId: allocations[batchIndex].batchId,
  ...
  Future<ManufacturingOrder> completeManufacturingOrder({
    ...
    if (LocalDatabaseService.isSqliteAuthoritative && sqliteDb != null) {
      ...
      await sqliteDb.transaction(() async {
        for (var lineIndex = 0;
          ...
          consumedCost += usedQty * _safeUsdCost(product);
          final baseMovement = StockMovement(
          ...
          if (product.expiryTrackingEnabled) {
            final allocations =
                await BatchInventoryService(sqliteDb).allocateFefoInTransaction(
              product: product,
              warehouseId: rawWarehouse.id,
              quantity: usedQty,
              saleDate: now,
            ...
            for (var batchIndex = 0;
              ...
              movements.add(baseMovement.copyWith(
                id: '${baseMovement.id}-batch-$batchIndex',
                quantity: -allocation.quantity,
                batchId: allocation.batchId,
                documentLineId:
            ...
            movements.add(baseMovement);
          }
        }
        final producedUnitCost = quantity <= 0 ? 0.0 : consumedCost / quantity;
        ...
        if (output.expiryTrackingEnabled) {
          final allocations =
              await BatchInventoryService(sqliteDb).addStockInTransaction(
            product: output,
            warehouseId: finishedWarehouse.id,
            sourceType: 'manufacturing_output',
            sourceId: order.id,
            expectedQuantity: quantity,
            allocations: outputBatchAllocations,
            receivedAt: now,
            storeId: appIdentity.storeId,
            branchId: appIdentity.branchId,
            deviceId: _deviceId,
          );
          producedBatchAllocations = allocations;
          for (var batchIndex = 0;
              batchIndex < allocations.length;
              batchIndex += 1) {
            final allocation = allocations[batchIndex];
      ...
      if (outputIndex != null && output.trackStock) {
        final producedUnitCost = movements
            .lastWhere((movement) => movement.productId == output.id)
            .unitCost;
        final refreshedStock = await totalWarehouseStockFromSqlite(output.id);
        final updatedOutput = _withSyncMeta<Product>(
          _products[outputIndex].copyWith(
            stock: refreshedStock,
            cost: producedUnitCost,
            usdCost: producedUnitCost,
            originalCost: producedUnitCost,
            costCurrency: 'USD',
            costExchangeRateAtEntry: storeProfile.usdToLbpRate,
          ),
          now,
    ...
    for (var lineIndex = 0; lineIndex < bom.components.length; lineIndex += 1) {
      ...
      final usedQty = component.quantity * factor;
      if (usedQty <= 0) continue;
      if (stockForWarehouse(component.productId, rawWarehouse.id) < usedQty) {
        throw ArgumentError(
  ...
  Future<Sale> createSale({
      ...
      if (LocalDatabaseService.isSqliteAuthoritative && sqliteDb != null) {
        ...
        await sqliteDb.transaction(() async {
          ...
          for (var lineIndex = 0;
            ...
            final allocations = await batchService.allocateFefoInTransaction(
              product: product,
              warehouseId: resolvedWarehouse.id,
    ...
    if (requestedInitialPaidAmount > 0) {
      if (!LocalDatabaseService.isSqliteAuthoritative ||
          SqliteMigrationManager.database == null) {
        throw StateError('Sale payments require the SQLite authoritative store.');
      }
      ...
      if (requestedInitialPaymentMethod.toLowerCase() == 'cash' && requestedInitialCashAmount > 0) {
        final sqliteDb = SqliteMigrationManager.database!;
  ...
  Future<Sale> editSale({
    ...
    if (invoiceTotal + 0.000001 < current.paidAmount) {
      throw StateError('Sale total cannot be reduced below allocated payments. Reverse payments first.');
    }
    if (requestedPaid + 0.000001 < current.paidAmount) {
      throw StateError('Payments cannot be reduced by editing the invoice. Use payment reversal.');
    }
    final paymentDelta = (requestedPaid - current.paidAmount).clamp(0, double.infinity).toDouble();
    final resolvedPaid = current.paidAmount.clamp(0, invoiceTotal).toDouble();
    final resolvedCash = current.cashReceivedAmount.clamp(0, resolvedPaid).toDouble();
    var updated = _withSyncMeta<Sale>(
      current.copyWith(
        customerId:
            customerId.trim().isEmpty ? walkInCustomerId : customerId.trim(),
        customerName: customerName.trim().isEmpty
    ...
    await AccountingService.recordSale(updated, paymentPostedSeparately: true);
    if (paymentDelta > 0) {
      updated = await settleSalePayment(
        saleId: updated.id,
        amount: paymentDelta,
        paymentMethod: normalizedMethod == 'Credit' ? 'Cash' : normalizedMethod,
        notes: 'Payment added during sale correction ${updated.invoiceNo}',
        idempotencyKey: '${updated.id}:edit:$editVersion:payment',
        date: now,
      );
    }
  ...
  Future<void> _refreshProductStockCompatibilityCache(
    ...
    for (final productId in uniqueIds) {
      ...
      if (index == null) continue;
      final updated = _withSyncMeta<Product>(
        _products[index].copyWith(stock: total),
        now,
      );
      _products[index] = updated;
      _rememberSqliteDirtyBusinessRow(_productsKey, updated.toJson());
