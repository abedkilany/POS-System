...
class _ManufacturingPageState extends State<ManufacturingPage> {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final boms = widget.store.billsOfMaterials.where((bom) {
          ...
          ..sort((a, b) {
            final nameComparison = a.name.trim().toLowerCase().compareTo(
                ...
                secondary: Row(
                  ...
                  children: [
                    ...
                    if (<String>{'completed', 'complete'}
                      ...
                      OutlinedButton.icon(
                        onPressed: () => _reverseManufacturingOrder(order),
                    ...
                    IconButton(
                      tooltip: _localizedText(
                        ar: 'تعديل أمر التصنيع',
                        en: 'Edit manufacturing order',
                        fr: 'Modifier l’ordre de fabrication',
                      ),
  ...
  Future<void> _confirmDeleteManufacturingOrder(
    ...
    if (<String>{'completed', 'complete'}
        .contains(order.status.trim().toLowerCase())) return;
  ...
  Future<void> _showOrderDetails(ManufacturingOrder order) async {
    BillOfMaterials? bom;
    for (final candidate in widget.store.billsOfMaterials) {
      if (candidate.id == order.bomId) {
        bom = candidate;
        break;
      }
    }
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text(_localizedText(
          ar: 'تفاصيل أمر التصنيع',
          en: 'Manufacturing order details',
          fr: 'Détails de l’ordre de fabrication',
        )),
        content: SizedBox(
          width: 520,
          child: SingleChildScrollView(
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                _orderDetailRow('رقم الأمر', 'Order number', order.orderNo),
                _orderDetailRow('الوصفة', 'Recipe', order.bomName),
                _orderDetailRow(
                    'المنتج الناتج', 'Output product', order.outputProductName),
                _orderDetailRow(
                    'الكمية', 'Quantity', order.quantity.toString()),
                _orderDetailRow('الحالة', 'Status',
                    _manufacturingStatusLabel(order.status)),
                _orderDetailRow('مستودع المواد الأولية',
                    'Raw materials warehouse', order.rawMaterialsWarehouseName),
                _orderDetailRow(
                    'مستودع المنتجات التامة',
                    'Finished goods warehouse',
                    order.finishedGoodsWarehouseName),
                _orderDetailRow(
                    'التاريخ', 'Date', _formatOrderDate(order.date)),
                if (order.status.toLowerCase() != 'in_progress') ...[
                  const Divider(height: 24),
                  _orderDetailRow(
                      'التكلفة الفعلية للمواد',
                      'Actual material cost',
                      order.totalMaterialCost.toStringAsFixed(2)),
                  _orderDetailRow('قيمة الهدر', 'Waste value',
                      order.totalWasteCost.toStringAsFixed(2)),
                  _orderDetailRow(
                      'تكلفة الإنتاج المقبول',
                      'Accepted output cost',
                      order.totalEligibleCost.toStringAsFixed(2)),
                  _orderDetailRow('تكلفة الوحدة الفعلية', 'Actual unit cost',
                      order.actualUnitCost.toStringAsFixed(4)),
                  _orderDetailRow(
                      'مرجع القيد', 'Journal reference', order.journalEntryId),
                  if (order.reversalJournalEntryId.isNotEmpty)
                    _orderDetailRow('قيد العكس', 'Reversal journal',
                        order.reversalJournalEntryId),
                  if (order.reversalReason.isNotEmpty)
                    _orderDetailRow(
                        'سبب العكس', 'Reversal reason', order.reversalReason),
                ],
                if (order.notes.trim().isNotEmpty)
                  _orderDetailRow('ملاحظات', 'Notes', order.notes),
                if (bom != null) ...[
                  const Divider(height: 24),
                  Text(
                    _localizedText(
                      ar: 'مكوّنات الوصفة',
                      en: 'Recipe components',
                      fr: 'Composants de la recette',
                    ),
                    style: const TextStyle(fontWeight: FontWeight.w700),
                  ),
                  const SizedBox(height: 4),
                  ...bom.components.map(
                    (component) => _orderDetailRow(
                      component.productName,
                      component.productName,
                      '${(component.quantity * order.quantity / bom!.outputQuantity).toStringAsFixed(2)} (${_localizedText(ar: 'للوصفة', en: 'per recipe', fr: 'par recette')}: ${component.quantity})',
                    ),
                  ),
                ],
              ],
  ...
  Future<void> _reverseManufacturingOrder(ManufacturingOrder order) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text(_localizedText(
          ar: 'عكس أمر التصنيع',
          en: 'Reverse manufacturing order',
          fr: 'Contrepasser l’ordre de fabrication',
        )),
        content: TextField(
          controller: controller,
          autofocus: true,
    ...
    try {
      await widget.store.reverseManufacturingOrder(
        orderId: order.id,
        reason: reason,
      );
    } catch (error) {
  ...
  Widget _orderDetailRow(String ar, String en, String value) {
    return Padding(
  ...
  Future<void> _showEditOrderDialog(ManufacturingOrder order) async {
    if (<String>{'completed', 'complete', 'reversed', 'posted'}
        .contains(order.status.toLowerCase())) {
      return;
    }
    if (!widget.store.hasPermission(
    ...
    final boms = widget.store.billsOfMaterials
        ...
        .toList(growable: false);
    final warehouses = widget.store.warehouses;
    if (boms.isEmpty || warehouses.isEmpty) return;
    var bomId = boms.any((item) => item.id == order.bomId)
        ? order.bomId
        : boms.first.id;
    var rawWarehouseId =
        warehouses.any((item) => item.id == order.rawMaterialsWarehouseId)
            ? order.rawMaterialsWarehouseId
            : warehouses.first.id;
    var finishedWarehouseId =
        warehouses.any((item) => item.id == order.finishedGoodsWarehouseId)
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                      labelText: _localizedText(
                          ar: 'الوصفة', en: 'Recipe', fr: 'Recette')),
                  items: boms
                      .map((item) => DropdownMenuItem(
                          value: item.id, child: Text(item.name)))
                      .toList(),
                  onChanged: (value) {
                    if (value != null) setDialogState(() => bomId = value);
                  },
                ),
                const SizedBox(height: 12),
                TextField(
                    ...
                    decoration:
                        InputDecoration(labelText: _t('quantity_to_produce'))),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  initialValue: rawWarehouseId,
                  decoration:
                      InputDecoration(labelText: _t('raw_materials_warehouse')),
                  items: warehouses
                      .map((item) => DropdownMenuItem(
                          value: item.id, child: Text(item.name)))
                      .toList(),
                ...
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  initialValue: finishedWarehouseId,
                  decoration: InputDecoration(
                      labelText: _t('finished_goods_warehouse')),
                  items: warehouses
                      .map((item) => DropdownMenuItem(
    ...
    try {
      final rawWarehouse =
          warehouses.firstWhere((item) => item.id == rawWarehouseId);
      final finishedWarehouse =
          warehouses.firstWhere((item) => item.id == finishedWarehouseId);
      await widget.store.updateManufacturingOrder(
        id: order.id,
        bomId: bomId,
  ...
  Future<void> _showFinishOrderDialog(ManufacturingOrder order) async {
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text(
                  _localizedText(
                    ar: 'الاستهلاك والهدر الفعلي',
                    en: 'Actual consumption and waste',
                    fr: 'Consommation et perte réelles',
                  ),
                  style: Theme.of(context).textTheme.titleSmall,
                ),
                const SizedBox(height: 8),
                      ...
                      children: [
                        ...
                        Row(children: [
                          Expanded(
                            child: TextField(
                              controller:
                                  consumedControllers[component.productId],
                              keyboardType:
                              ...
                              decoration: InputDecoration(
                                labelText: _localizedText(
                                  ar: 'المستهلك فعلياً',
                                  en: 'Actually consumed',
                                  fr: 'Consommé réellement',
                                ),
                              ),
                            ),
                          ),
                          ...
                          Expanded(
                            child: TextField(
                              controller: wasteControllers[component.productId],
                              keyboardType:
                                  const TextInputType.numberWithOptions(
                                      decimal: true),
                              decoration: InputDecoration(
                        ...
                        TextField(
                          controller:
                              wasteReasonControllers[component.productId],
                          decoration: InputDecoration(
                            labelText: _localizedText(
                              ar: 'سبب الهدر (عند وجوده)',
                              en: 'Waste reason (when applicable)',
                              fr: 'Motif de perte (le cas échéant)',
                            ),
                          ),
                        ),
                      ],
    ...
    if (confirmed != true || !mounted) {
      qtyController.dispose();
      for (final controller in consumedControllers.values) {
        controller.dispose();
      }
      for (final controller in wasteControllers.values) {
        controller.dispose();
      }
      for (final controller in wasteReasonControllers.values) {
        controller.dispose();
      }
      return;
    }
