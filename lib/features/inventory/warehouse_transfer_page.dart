...
class _WarehouseTransferPageState extends State<WarehouseTransferPage> {
  ...
  Future<void> _submitTransfer() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(AppLocalizations.of(context).format(
                'transfer_order_completed', {'number': order.orderNo}))),
      );
      setState(() {});
    } catch (error) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.store.hasPermission(AppPermission.inventoryWarehousesManage)) {
      return Scaffold(
        appBar: AppBar(title: Text(tr.text('transfer_stock'))),
        body: Center(child: Text(tr.text('no_access_current_role'))),
      );
    }
    final warehouses = _warehouses;
    if (warehouses.length < 2) {
      return Scaffold(
        appBar: AppBar(title: Text(tr.text('transfer_stock'))),
        body: Center(child: Text(tr.text('two_warehouses_required'))),
      );
    }
    return Scaffold(
      appBar: AppBar(
      ...
      child: Column(
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              labelText:
                  AppLocalizations.of(context).text('search_products_barcode'),
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              Text(
                  '${AppLocalizations.of(context).text('total')}: ${_fmt(_cart.fold<double>(0, (s, i) => s + i.baseQuantity))}'),
            ],
          ),
        ),
        Expanded(
          child: _cart.isEmpty
              ? Center(
                  child: Text(AppLocalizations.of(context)
                      .text('select_products_for_transfer')),
                )
              : ListView.separated(
        ...
        children: [
          Expanded(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${tr.text('available')}: ${_fmt(item.availableInSelectedUnit)} ${item.selectedUnit.name}',
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                isDense: true,
                labelText: tr.text('qty'),
                errorText: overStock
                    ? 'Max ${_fmt(item.availableInSelectedUnit)}'
                    : null,
                border: const OutlineInputBorder(),
              ),
            ),
      ...
      builder: (context, snapshot) {
        ...
        if (orders.isEmpty) {
          return Padding(
            ...
            child:
                Text(AppLocalizations.of(context).text('no_transfer_orders')),
          );
        }
