...
class MaintenanceService {
  ...
  Future<MaintenanceSummary> runHealthCheck({bool deep = false}) async {
    ...
    final phase10 = await Phase10HealthService(store).run(deep: deep);
    final counts = <String, int>{
      ...
      'phase10Critical': phase10.criticalCount,
      'phase10Warnings': phase10.warningCount,
      'phase10HealthyChecks': phase10.checks
          .where((item) => item.level == Phase10HealthLevel.healthy)
          .length,
    };
    ...
    return MaintenanceSummary(
      ...
      databaseDetails: <String, dynamic>{
        ...
        'phase10ProductionHealth': phase10.toJson(),
      },
      counts: counts,
      issues: issues,
    );
  ...
  List<MaintenanceIssue> _phase10Issues(Phase10HealthReport report) {
    return report.checks.map((check) {
      final severity = switch (check.level) {
        Phase10HealthLevel.healthy => MaintenanceSeverity.ok,
        Phase10HealthLevel.warning => MaintenanceSeverity.warning,
        Phase10HealthLevel.critical => MaintenanceSeverity.critical,
      };
