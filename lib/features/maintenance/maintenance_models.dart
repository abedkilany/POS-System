...
class MaintenanceSummary {
  ...
  int get criticalCount => issues
      .where((item) => item.severity == MaintenanceSeverity.critical)
      .length;
  int get warningCount => issues
      .where((item) => item.severity == MaintenanceSeverity.warning)
      .length;
  int get infoCount =>
      issues.where((item) => item.severity == MaintenanceSeverity.info).length;
  ...
  List<String> get recommendations {
    ...
    if ((counts['pendingSyncChanges'] ?? 0) > 0 ||
        (counts['pendingSyncQueue'] ?? 0) > 0) {
      items.add(
    ...
    if (items.isEmpty) {
      items
          .add('Create a fresh backup before major updates or device changes.');
    } else {
