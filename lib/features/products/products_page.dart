...
class _ProductsPageState extends State<ProductsPage> {
              ...
              return Row(children: [
                ...
                SizedBox(
                    width: VentioResponsive.adaptiveWidth(context,
                        mobile: 160, tablet: 200, desktop: 220),
                    child: categoryField)
              ]);
            },
          ),
                    ...
                    children: [
                      ...
                      ..._priceListFieldOptions().map((option) {
                        ...
                        return CheckboxListTile(
                          value: selected,
                          title:
                              Text(isArabic ? option.labelAr : option.labelEn),
          ...
          FilledButton.icon(
            ...
            onPressed: () async {
              ...
              await PriceListPdfService.printPriceList(
                ...
                valueResolver: (product, field) =>
                    _priceFieldValue(product, field),
                arabic: isArabic,
              );
            },
          ),
  ...
  String _priceListTitle(bool isArabic, Set<String> selection) {
    final labels = <String>[
      ...selection.map((field) {
        final option =
            _priceListFieldOptions().firstWhere((item) => item.id == field);
  ...
  String _priceAmountValue(Product product, String selection) {
    switch (selection) {
      ...
      case 'wholesale_bulk':
        final price =
            widget.store.productPriceFor(product.id, 'wholesale_bulk');
        final value = price?.baseAmount ?? product.originalPrice;
        return '${price?.baseCurrencyCode.toUpperCase() ?? product.originalCurrency} ${value.toStringAsFixed(2)}';
      case 'isActive':
  ...
  List<_PriceListFieldOption> _priceListFieldOptions() {
    return const <_PriceListFieldOption>[
      ...
      _PriceListFieldOption(
          id: 'nameEn', labelAr: 'الاسم الإنجليزي', labelEn: 'English name'),
      _PriceListFieldOption(
          id: 'nameAr', labelAr: 'الاسم العربي', labelEn: 'Arabic name'),
      _PriceListFieldOption(id: 'code', labelAr: 'الكود', labelEn: 'Code'),
      _PriceListFieldOption(
          id: 'barcode', labelAr: 'الباركود', labelEn: 'Barcode'),
      _PriceListFieldOption(
          id: 'category', labelAr: 'الفئة', labelEn: 'Category'),
      _PriceListFieldOption(id: 'brand', labelAr: 'الماركة', labelEn: 'Brand'),
      _PriceListFieldOption(
          id: 'supplier', labelAr: 'المورد', labelEn: 'Supplier'),
      _PriceListFieldOption(
          id: 'description', labelAr: 'الوصف', labelEn: 'Description'),
      _PriceListFieldOption(id: 'unit', labelAr: 'الوحدة', labelEn: 'Unit'),
      _PriceListFieldOption(
          id: 'quantityType', labelAr: 'نوع الكمية', labelEn: 'Quantity type'),
      ...
      _PriceListFieldOption(
          id: 'wholesale', labelAr: 'الجملة', labelEn: 'Wholesale'),
      _PriceListFieldOption(
          id: 'wholesale_bulk',
          labelAr: 'جملة الجملة',
          labelEn: 'Wholesale bulk'),
      _PriceListFieldOption(
          id: 'isActive', labelAr: 'الحالة', labelEn: 'Status'),
      _PriceListFieldOption(
          id: 'imagePath', labelAr: 'الصورة', labelEn: 'Image'),
    ];
  }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(product == null
                ? tr.text('product_saved')
                : tr.text('product_updated'))));
      }
    } catch (error) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  Future<void> _addProductToQuickProducts(
      Product product, AppLocalizations tr) async {
    const storageKey = 'sale_quick_product_pages_v1';
    List<dynamic> pages;
    try {
      final raw = LocalDatabaseService.getString(storageKey);
      pages = raw == null || raw.trim().isEmpty
          ? <dynamic>[]
    ...
    for (final page in pages) {
      ...
      for (var i = 0; i < slots.length; i++) {
        final slot = slots[i];
        final isEmpty = slot is! Map ||
            (slot['productId'] as String?)?.trim().isEmpty != false;
        if (isEmpty) {
          slots[i] = {
...
class _ProductFormResult {
  const _ProductFormResult(
      {required this.product,
      required this.addToQuickProducts,
      this.supplierPriceSave,
      this.priceSave});

  final Product product;
  final bool addToQuickProducts;
  final Future<void> Function()? supplierPriceSave;
  final Future<void> Function()? priceSave;
}
...
class _ProductDialogState extends State<_ProductDialog> {
  ...
  void initState() {
    ...
    costCurrency = product?.costCurrency ??
        widget.store.storeProfile.defaultProductCurrency;
    final activeTaxProfiles = widget.store.storeProfile.activeTaxProfiles;
    final requestedTaxProfileId = product?.taxProfileId.trim() ?? '';
    if (activeTaxProfiles.any((item) => item.id == requestedTaxProfileId)) {
      taxProfileId = requestedTaxProfileId;
    } else if (activeTaxProfiles.any(
      (item) => item.id == widget.store.storeProfile.defaultTaxProfileId,
    )) {
              ...
              children: [
                _ProductFormSection(
                  ...
                  children: [
                    _ProductImagePicker(
                      ...
                      onClear: imagePath.trim().isEmpty
                          ? null
                          : () => setState(() => imagePath = ''),
                    ),
                    const SizedBox(height: 12),
                    _ResponsiveFields(children: [
                      ...
                      _CatalogDropdown(
                        ...
                        onManage: () =>
                            _manageCatalogItems(context, 'category'),
                      ),
                      _CatalogDropdown(
                        label: tr.text('brand'),
                        value: brand,
                      ...
                      DropdownButtonFormField<ProductQuantityType>(
                        ...
                        decoration: InputDecoration(
                            labelText: tr.text('quantity_type')),
                        items: [
                          DropdownMenuItem(
                              value: ProductQuantityType.countable,
                              child: Text(tr.text('quantity_type_countable'))),
                          DropdownMenuItem(
                              value: ProductQuantityType.measurable,
                              child: Text(tr.text('quantity_type_measurable'))),
                        ],
                        onChanged: (value) => setState(() => quantityType =
                            value ?? ProductQuantityType.countable),
                      ),
                    ]),
                    ...
                    ExpansionTile(
                      ...
                      subtitle: Text(
                          '${tr.text('auto_generated')}: ${codeController.text}'),
                      children: [
                        TextFormField(
                            controller: codeController,
                            decoration:
                                InputDecoration(labelText: tr.text('sku_code')),
                            validator: _uniqueSku),
                      ],
                    ),
                    const SizedBox(height: 12),
                    TextFormField(
                ...
                _ProductFormSection(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    DropdownButtonFormField<String>(
                      initialValue: widget.store.storeProfile.activeTaxProfiles
                              .any((item) => item.id == taxProfileId)
                          ? taxProfileId
                      ...
                      items: [
                        for (final profile
                            in widget.store.storeProfile.activeTaxProfiles)
                          DropdownMenuItem<String>(
                            value: profile.id,
                            child: Text(
                              '${profile.name} (${profile.code}${profile.treatment.name == 'standard' ? ' ${profile.ratePercent.toStringAsFixed(profile.ratePercent == profile.ratePercent.roundToDouble() ? 0 : 2)}%' : ''})',
                            ),
                          ),
                      ],
                      onChanged: (value) {
                        if (value != null) {
                          setState(() => taxProfileId = value);
                        }
                      },
                    ),
                    const SizedBox(height: 12),
                    _CurrencyPriceOverridesEditor(
                      ...
                      availableCurrencyCodes: widget
                          ...
                          .toList(growable: false),
                      onChanged: (items) =>
                          setState(() => priceOverrideDrafts = items),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                ...
                _ProductFormSection(
                  ...
                  children: [
                    SwitchListTile(
                        contentPadding: EdgeInsets.zero,
                        title: Text(tr.text('track_quantity')),
                        subtitle: Text(tr.text('track_quantity_help')),
                        value: trackStock,
                        onChanged: (value) =>
                            setState(() => trackStock = value)),
                    _ResponsiveFields(children: [
                      TextFormField(
                          enabled: trackStock && widget.product == null,
                          controller: stockController,
                          decoration: InputDecoration(
                              labelText: tr.text('opening_stock'),
                              helperText: !trackStock
                                  ? tr.text('stock_ignored_non_stock')
                                  : widget.product == null
                                      ? null
                                      : tr.text('adjust_stock')),
                          keyboardType: const TextInputType.numberWithOptions(
                              decimal: true),
                          validator: trackStock
                              ? (quantityType == ProductQuantityType.measurable
                                  ? _nonNegativeNumber
                                  : _nonNegativeInteger)
                              : null),
                      TextFormField(
                          ...
                          decoration: InputDecoration(
                              labelText: tr.text('low_stock_alert'),
                              helperText: trackStock
                                  ? null
                                  : tr.text('no_low_stock_alerts_non_stock')),
                          keyboardType: TextInputType.number,
                          validator: trackStock ? _nonNegativeInteger : null),
                    ]),
                  ],
                ),
                const SizedBox(height: 12),
                ...
                SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: Text(tr.text('add_to_quick_products')),
                    subtitle: Text(tr.text('add_to_quick_products_help')),
                    value: addToQuickProducts,
                    onChanged: (value) =>
                        setState(() => addToQuickProducts = value)),
                SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: Text(tr.text('active_product')),
                    value: isActive,
  ...
  Future<void> _copyBarcode() async {
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text(tr.text('barcode_copied'))));
  }
  ...
  void _save() {
    ...
    final usdPrice = toUsdReferencePrice(
    ...
    Navigator.pop(
      ...
      _ProductFormResult(
        ...
        product: Product(
          ...
          taxProfileId: taxProfileId.trim(),
        ),
      ),
    );
  }
  ...
  Future<void> _saveProductPriceDrafts(
    ...
    if (basePrice != null) {
      ...
      for (final draft in priceOverrideDrafts) {
        ...
        final overrideAmount = double.tryParse(draft.amountText.trim());
        if (normalizedCurrency.isEmpty ||
            normalizedCurrency == currencyCode.trim().toUpperCase() ||
            overrideAmount == null ||
            overrideAmount < 0) {
          continue;
        }
    ...
    for (final draft in saleUnitDrafts) {
      ...
      await widget.store.setDefaultProductBasePrice(
        ...
        currencyCode: unit.originalCurrency,
      );
    }
  }
  ...
  String _resolvedSku() => codeController.text.trim().isEmpty
      ? _generateUniqueSku()
...
class _CurrencyPriceOverridesEditor extends StatelessWidget {
  const _CurrencyPriceOverridesEditor({
  ...
  final List<_CurrencyPriceOverrideDraft> drafts;
  final String baseCurrencyCode;
  final List<String> availableCurrencyCodes;
  final ValueChanged<List<_CurrencyPriceOverrideDraft>> onChanged;
        ...
        children: [
          LayoutBuilder(builder: (context, constraints) {
            ...
            if (constraints.maxWidth < 460) {
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [title, const SizedBox(height: 8), addButton],
              );
            }
            return Row(
              children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        onChanged: (value) =>
                            _update(index, draft.copyWith(amountText: value)),
                      ),
                    ),
                    IconButton(
...
class _SupplierPricesEditor extends StatelessWidget {
  ...
  String _supplierName(String id) {
    for (final supplier in suppliers) {
      if (supplier.id == id) {
        return supplier.name.trim().isNotEmpty
            ? supplier.name
            : supplier.nameEn;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final rows = prices.where((item) => !item.isDeleted).toList()
      ..sort((a, b) {
        ...
        return _supplierName(a.supplierId)
            .toLowerCase()
            .compareTo(_supplierName(b.supplierId).toLowerCase());
      });
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
  ...
  Widget _buildSupplierPriceCard(
    ...
    return Card(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(item.isPreferred
                          ? Icons.star
                          : Icons.local_shipping_outlined),
                      const SizedBox(width: 8),
                      Expanded(
                          child: Text(_supplierName(item.supplierId),
                              style: const TextStyle(
                                  fontWeight: FontWeight.w700))),
                    ],
                  ),
          ...
          return ListTile(
            leading: Icon(
                item.isPreferred ? Icons.star : Icons.local_shipping_outlined),
  ...
  Future<void> _importCsv(BuildContext context) async {
    ...
    for (final rawLine in lines.skip(1)) {
      ...
      for (final candidate in suppliers) {
        final name = (candidate.name.trim().isNotEmpty
                ? candidate.name
                : candidate.nameEn)
            .toLowerCase();
      ...
      final currency =
          cols.length > 2 && cols[2].toUpperCase() == 'LBP' ? 'LBP' : 'USD';
      final supplierSku = cols.length > 3 ? cols[3] : '';
      final minQty = cols.length > 4 && cols[4].isNotEmpty
          ? double.tryParse(cols[4])
          : null;
      final leadDays =
          cols.length > 5 && cols[5].isNotEmpty ? int.tryParse(cols[5]) : null;
      final notes = cols.length > 6 ? cols.sublist(6).join(',').trim() : '';
      final existingIndex = updated.indexWhere((item) =>
          !item.isDeleted &&
          item.supplierId == supplierRow.id &&
          item.productId == productId);
      final existing = existingIndex == -1 ? null : updated[existingIndex];
      final row = SupplierProductPrice(
        id: existing?.id ??
            'spp_${now.microsecondsSinceEpoch}_${updated.length}',
        productId: productId,
        supplierId: supplierRow.id,
        cost: cost,
        currency: currency,
        isPreferred: existing?.isPreferred ??
            updated.where((item) => !item.isDeleted).isEmpty,
  ...
  Future<void> _openEditor(BuildContext context,
      {SupplierProductPrice? item}) async {
    ...
    for (final row in prices) {
      if (row.id == result.id) {
        updated.add(result);
        replaced = true;
      } else if (result.isPreferred &&
          row.productId == result.productId &&
          row.id != result.id) {
        updated
            .add(row.copyWith(isPreferred: false, updatedAt: DateTime.now()));
      } else {
        updated.add(row);
      }
    }
    if (!replaced) {
      if (result.isPreferred) {
        for (var i = 0; i < updated.length; i++) {
          updated[i] = updated[i]
              .copyWith(isPreferred: false, updatedAt: DateTime.now());
        }
      }
      updated.add(result);
    }
...
class _SupplierPriceDialogState extends State<_SupplierPriceDialog> {
  ...
  void initState() {
    super.initState();
    supplierId = widget.price?.supplierId ??
        (widget.suppliers.isNotEmpty ? widget.suppliers.first.id : '');
    currency = widget.price?.currency ?? 'USD';
    isPreferred = widget.price?.isPreferred ?? false;
    costController =
        TextEditingController(text: widget.price?.cost.toString() ?? '');
    supplierSkuController =
        TextEditingController(text: widget.price?.supplierSku ?? '');
    minOrderQtyController = TextEditingController(
        text: widget.price?.minOrderQty?.toString() ?? '');
    leadTimeDaysController = TextEditingController(
        text: widget.price?.leadTimeDays?.toString() ?? '');
            ...
            child: Column(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  items: widget.suppliers
                      .map((supplier) => DropdownMenuItem(
                            value: supplier.id,
                            child: Text(supplier.name.trim().isNotEmpty
                                ? supplier.name
                                : supplier.nameEn),
                          ))
                      .toList(),
                ...
                if ((widget.price?.priceHistory.isNotEmpty ?? false)) ...[
                  ...
                  const SizedBox(height: 4),
                  ...widget.price!.priceHistory.reversed
                      .take(5)
                      .map((entry) => Align(
                            alignment: Alignment.centerLeft,
                            child: Text(
  ...
  void _save() {
    ...
    Navigator.pop(
      ...
      SupplierProductPrice(
        ...
        minOrderQty: minOrderQtyController.text.trim().isEmpty
            ? null
            : double.tryParse(minOrderQtyController.text.trim()),
        leadTimeDays: leadTimeDaysController.text.trim().isEmpty
            ? null
            : int.tryParse(leadTimeDaysController.text.trim()),
                    ...
                    children: [
                      ...
                      _ResponsiveFields(children: [
                        TextFormField(
                          ...
                          decoration:
                              InputDecoration(labelText: tr.text('unit_name')),
                          onChanged: (value) => unit.name = value,
                        ),
                        TextFormField(
                          initialValue: unit.conversionToBase,
                          decoration: InputDecoration(
                              labelText: tr.text('conversion_to_base')),
                          keyboardType: const TextInputType.numberWithOptions(
                              decimal: true),
                          validator: (value) {
                            final number =
                        ...
                        TextFormField(
                          ...
                          decoration: InputDecoration(
                              labelText: tr.text('unit_sale_price')),
                          keyboardType: const TextInputType.numberWithOptions(
                              decimal: true),
                          validator: (value) {
                            final number =
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          decoration: InputDecoration(
                              labelText: tr.text('price_currency')),
                          items: const [
                            DropdownMenuItem(value: 'USD', child: Text('USD')),
                            DropdownMenuItem(value: 'LBP', child: Text('LBP')),
                          ],
                          onChanged: (value) =>
                              unit.priceCurrency = value ?? 'USD',
                        ),
                        TextFormField(
                          key: ValueKey(
                              'unit-barcode-${unit.id}-${unit.barcode}'),
...
class _ProductImagePicker extends StatelessWidget {
  const _ProductImagePicker(
      {required this.imagePath, required this.onPick, this.onClear});
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(tr.text('product_image'),
                        style: const TextStyle(fontWeight: FontWeight.w700)),
                    const SizedBox(height: 4),
                    Text(
                        hasImage
                            ? imagePath.split('/').last
                            : tr.text('product_image_help'),
                        maxLines: 2,
                        overflow: TextOverflow.ellipsis),
                  ],
                ),
...
class _CatalogDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final values = <String>{
      ...items
          .map((item) => item.code.isNotEmpty ? item.code : item.nameEn)
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          TextField(
              controller: codeController,
              decoration: InputDecoration(labelText: tr.text('code_optional'))),
        ],
      ),
      actions: [
        TextButton(
...
class _CatalogManagerDialogState extends State<_CatalogManagerDialog> {
  ...
  Future<void> _add(BuildContext context) async {
    final result = await showDialog<CatalogItem>(
        context: context,
        builder: (_) => _CatalogItemDialog(type: widget.type));
    if (result == null) return;
  ...
  Future<void> _edit(BuildContext context, CatalogItem item) async {
    final result = await showDialog<CatalogItem>(
        context: context,
        builder: (_) => _CatalogItemDialog(type: widget.type, item: item));
    if (result == null) return;
    await _saveItem(result);
  }
    ...
    return AlertDialog(
      ...
      actions: [
        TextButton.icon(
            ...
            icon: const Icon(Icons.add),
            label: Text(tr.text('add_lookup_item'))),
        TextButton(
            onPressed: () => Navigator.pop(context),
