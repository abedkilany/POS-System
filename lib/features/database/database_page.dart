...
class _DatabasePageState extends State<DatabasePage> {
  ...
  Future<bool> _verifyDatabasePassword(String password) async {
    final adminOk = await widget.store.verifyAdminPassword(password);
    if (!adminOk) return false;
    return widget.store.security.authorizeSensitiveAction(
      action: SensitiveAction.databaseDestructive,
      password: password,
    );
  }

  Future<bool> _ensureDatabaseMutationAuthorization() async {
    try {
      widget.store.security.requireSensitiveActionAuthorization(
        SensitiveAction.databaseDestructive,
      );
      return true;
    } on SensitiveActionAuthRequiredException {
      if (!mounted) return false;
      return requestSensitiveActionAuthorization(
        context,
        widget.store,
        action: SensitiveAction.databaseDestructive,
      );
  ...
  Future<void> _auditDatabaseMutation(
    String action, {
    String details = '',
  }) async {
    ...
    await AuditLogger.record(
      entityType: 'database',
      entityId: _selectedKey,
      action: action,
      summary: 'Database maintenance mutation',
      details: details,
      userId: user?.id ?? '',
      userName: user?.username ?? '',
      storeId: identity.storeId,
      branchId: identity.branchId,
      sessionId: widget.store.deviceId,
      traceId: widget.store.deviceId,
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(
                ...
                onSubmitted: (_) async {
                  final ok =
                      await _verifyDatabasePassword(controller.text);
  ...
  Future<void> _runSqlEditor() async {
    ...
    if (hasWrites) {
      ...
      if (!await _ensureDatabaseMutationAuthorization()) return;
  ...
  Future<void> _writeSelectedKey(String value) async {
    ...
    await _auditDatabaseMutation('raw_write', details: 'key=$changedKey');
  ...
  Future<void> _confirmDeleteKey() async {
    ...
    await _auditDatabaseMutation('delete_key', details: 'key=$changedKey');
    await widget.store.refreshAfterDatabaseChange(changedKey);
    await _reload();
  }
}
