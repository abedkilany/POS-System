...
class _DatabasePageState extends State<DatabasePage> {
                  ...
                  PopupMenuButton<String>(
                    ...
                    itemBuilder: (context) => [
                      ...
                      if (_selectedMode != 'sql')
                        const PopupMenuItem(
                            value: 'sql', child: Text('SQL Editor')),
                      PopupMenuItem(
                      ...
                      Wrap(
                        ...
                        children: [
                          FilledButton.icon(
                            ...
                            label: Text(
                                _sqlAllowWrites ? 'Execute SQL' : 'Run Query'),
                          ),
                          OutlinedButton.icon(
  ...
  Future<void> _exportSqlRows(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('SQL result exported as ${format.toUpperCase()}')));
    } catch (error) {
      if (!mounted) return;
  ...
  Future<void> _runSqlEditor() async {
    ...
    if (hasWrites) {
      if (!_sqlAllowWrites) {
        setState(() => _sqlError =
            'This script contains write statements. Enable Execute writes first.');
        return;
      }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('SQL executed locally. Sync was not triggered.')));
    } catch (error) {
      if (!mounted) return;
  ...
  Future<bool?> _confirmSqlExecution(int statementCount) {
    return showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'This will execute $statementCount SQL statement(s) locally only. It will refresh the app state but will not create sync changes.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: Text(_t('cancel'))),
