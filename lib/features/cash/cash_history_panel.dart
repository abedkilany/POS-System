...
class _CashHistoryPanelState extends State<CashHistoryPanel> {
  static const int _loadBatchSize = 500;
  ...
  Future<_CashHistorySnapshot> _load() async {
    ...
    if (sessionId.isEmpty && !_showAllReceipts) {
      return const _CashHistorySnapshot(
        rows: <CashLedgerTransaction>[],
        total: 0,
        summary: CashLedgerSummary(movementCount: 0, cashIn: 0, cashOut: 0),
    ...
    // By default history is locked to the open shift. The historical mode
    // removes only the session filter and keeps the cash-location filter
    // authoritative. It must not force type='receipt': legacy and older
    // cash-in records can use receipt-compatible types such as cash_in or
    // sale_payment and should remain discoverable in the historical view.
    DateTime? from;
    DateTime? to;
    final args = (
      cashLocationId: widget.cashLocationId,
      cashDrawerSessionId: _showAllReceipts ? '' : sessionId,
    ...
    final rows = <CashLedgerTransaction>[];
    var offset = 0;
    while (true) {
      final batch = await _ledger.list(
      ...
      rows.addAll(batch);
      if (batch.length < _loadBatchSize) break;
      offset += batch.length;
    }
            ...
            child: Row(
              children: [
                ...
                FilledButton.tonalIcon(
                  onPressed: () {
                    setState(() {
                      _showAllReceipts = !_showAllReceipts;
                      _direction = '';
                      _type = '';
                      _future = _load();
                    });
                  },
                  icon: Icon(_showAllReceipts
                      ? Icons.history_rounded
                      : Icons.receipt_long_outlined),
                  label: Text(_showAllReceipts
                      ? _l(tr, 'Current shift', 'الوردية الحالية')
                      : _l(tr, 'All old cash movements',
                          'كل الحركات النقدية القديمة')),
                ),
              ],
            ),
          ),
            ...
            child: Wrap(
              ...
              children: [
                SizedBox(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: _l(tr, 'Invoice, party, user, note…',
                          'فاتورة، طرف، مستخدم، ملاحظة…'),
                ...
                DropdownButton<String>(
                  value: _direction,
                  onChanged: (value) =>
                      _changeFilter(() => _direction = value ?? ''),
                  items: [
                    DropdownMenuItem(
                        value: '',
                        child: Text(_l(tr, 'All directions', 'كل الاتجاهات'))),
                    DropdownMenuItem(
                        value: 'in', child: Text(_l(tr, 'Cash in', 'داخل'))),
                    DropdownMenuItem(
                        value: 'out', child: Text(_l(tr, 'Cash out', 'خارج'))),
                  ],
                ),
                DropdownButton<String>(
                  value: _type,
                  onChanged: (value) =>
                      _changeFilter(() => _type = value ?? ''),
                  items: [
                    DropdownMenuItem(
                        value: '',
                        child: Text(_l(tr, 'All types', 'كل الأنواع'))),
                    DropdownMenuItem(
                        value: 'receipt',
                        child: Text(_l(tr, 'Receipt', 'قبض'))),
                    DropdownMenuItem(
                        value: 'supplier_payment',
                        child: Text(_l(tr, 'Supplier payment', 'دفع مورد'))),
                    DropdownMenuItem(
                        value: 'cash_deposit',
                    ...
                    DropdownMenuItem(
                        value: 'expense',
                        child: Text(_l(tr, 'Expense', 'مصروف'))),
                    DropdownMenuItem(
                    ...
                    DropdownMenuItem(
                        ...
                        child:
                            Text(_l(tr, 'Supplier refund', 'استرداد من مورد'))),
                    DropdownMenuItem(
                        value: 'expense_refund',
                        child: Text(
                            _l(tr, 'Expense refund', 'استرداد زائد مصروف'))),
                    DropdownMenuItem(
                        value: 'shortage',
                        child: Text(_l(tr, 'Cash shortage', 'عجز نقدي'))),
                    DropdownMenuItem(
                        value: 'overage',
                        child: Text(_l(tr, 'Cash overage', 'زيادة نقدية'))),
                    DropdownMenuItem(
                        value: 'reversal',
                        child: Text(_l(tr, 'Reversal', 'عكس حركة'))),
                  ],
                ),
              ],
            ),
                ...
                if (snapshot.hasError) {
                  return Center(
                    child: Padding(
                      ...
                      child: Text(
                          localizeRuntimeMessage(snapshot.error.toString(), tr),
                          textAlign: TextAlign.center),
                    ),
                  );
                }
                final data = snapshot.data ??
                    const _CashHistorySnapshot(
                      rows: <CashLedgerTransaction>[],
                      total: 0,
                      summary: CashLedgerSummary(
                          movementCount: 0, cashIn: 0, cashOut: 0),
                    );
                return Column(
                  children: [
                    ...
                    Expanded(
                      child: data.rows.isEmpty
                          ? Center(
                              child: Text(_l(tr, 'No cash movements found.',
                                  'لا توجد حركات صندوق مطابقة.')))
                          : ListView.separated(
                              itemCount: data.rows.length,
                              separatorBuilder: (_, __) =>
                                  const Divider(height: 1),
                              itemBuilder: (context, index) =>
                                  _movementTile(context, tr, data.rows[index]),
                            ),
                    ),
                    const Divider(height: 1),
                  ],
  ...
  Widget _summaryBar(
      BuildContext context, AppLocalizations tr, CashLedgerSummary value) {
    String money(double amount) =>
        formatUsdReferenceAmount(amount, widget.store.storeProfile);
    return Padding(
      ...
      child: Wrap(
        ...
        runSpacing: 8,
        children: [
          _summaryItem(context, _l(tr, 'Movements', 'الحركات'),
              '${value.movementCount}', Icons.receipt_long_outlined),
          _summaryItem(context, _l(tr, 'In', 'داخل'), money(value.cashIn),
              Icons.south_west_rounded),
          _summaryItem(context, _l(tr, 'Out', 'خارج'), money(value.cashOut),
              Icons.north_east_rounded),
          _summaryItem(context, _l(tr, 'Net', 'الصافي'), money(value.net),
  ...
  Widget _movementTile(
      BuildContext context, AppLocalizations tr, CashLedgerTransaction item) {
    final incoming = item.direction == 'in';
    final reference = item.referenceNumber.trim().isNotEmpty
    ...
    return ListTile(
      ...
      title: Text(_typeLabel(tr, item.type),
            ...
            builder: (context, snapshot) {
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    _detailRow(
                        tr, _l(tr, 'Transaction ID', 'رقم الحركة'), item.id),
                    _detailRow(
                        tr, _l(tr, 'Type', 'النوع'), _typeLabel(tr, item.type)),
                    _detailRow(
                        tr,
                        _l(tr, 'Direction', 'الاتجاه'),
                        item.direction == 'in'
                            ? _l(tr, 'In', 'داخل')
                            : _l(tr, 'Out', 'خارج')),
                    _detailRow(tr, _l(tr, 'Amount', 'المبلغ'),
                        '${item.amount.toStringAsFixed(2)} ${item.currency}'),
                    _detailRow(
                        tr,
                        _l(tr, 'Occurred at', 'وقت الحركة'),
                        DateFormat('yyyy-MM-dd HH:mm:ss')
                            .format(item.occurredAt.toLocal())),
                    _detailRow(
                        tr,
                        _l(tr, 'Cash drawer', 'الصندوق'),
                        details == null
                            ? item.cashLocationId
                            : '${details.cashLocationName}${details.cashLocationCode.isEmpty ? '' : ' (${details.cashLocationCode})'}'),
                    _detailRow(
                        tr,
                        _l(tr, 'Shift', 'الوردية'),
                        details?.sessionNumber.isNotEmpty == true
                            ? '${details!.sessionNumber} • ${details.sessionStatus}'
                            : item.cashDrawerSessionId),
                    _detailRow(tr, _l(tr, 'Party', 'الطرف'), item.partyName),
                    _detailRow(
                        tr,
                        _l(tr, 'Reference', 'المرجع'),
                        [
                          item.referenceType,
                          item.referenceNumber,
                          item.referenceId
                        ].where((e) => e.trim().isNotEmpty).join(' • ')),
                    _detailRow(tr, _l(tr, 'Payment method', 'وسيلة الدفع'),
                        item.paymentMethod),
                    ...
                    if (details?.allocationReferences.isNotEmpty == true)
                      _detailRow(
                        ...
                        details!.allocationReferences
                            .map((allocation) =>
                                '${allocation.referenceNumber.isNotEmpty ? allocation.referenceNumber : allocation.referenceId} • ${allocation.amount.toStringAsFixed(2)} ${allocation.currency}')
                            .join('\n'),
                      ),
                    _detailRow(
                        tr,
                        _l(tr, 'Journal entry', 'القيد المحاسبي'),
                        details?.journalEntryNo.isNotEmpty == true
                            ? '${details!.journalEntryNo} • ${details.journalStatus}'
                            : _l(tr, 'Not linked', 'غير مرتبط')),
                    if (details?.journalDescription.isNotEmpty == true)
                      _detailRow(tr, _l(tr, 'Journal description', 'وصف القيد'),
                          details!.journalDescription),
                  ],
                ),
              );
            },
            ...
            onPressed: () async {
              try {
                ...
                ScaffoldMessenger.of(dialogContext).showSnackBar(
                  SnackBar(
                      content:
                          Text(localizeRuntimeMessage(error.toString(), tr))),
                );
              }
            },
              ...
              onPressed: () async {
                final confirmed = await showDialog<bool>(
                  ...
                  builder: (confirmContext) => AlertDialog(
                    title: Text(
                        _l(tr, 'Reverse cash movement', 'عكس حركة الصندوق')),
                    content: Text(_l(
                      tr,
                ...
                try {
                  final reversed =
                    ...
                    createdBy:
                        widget.store.currentUser?.fullName.trim().isNotEmpty ==
                                true
                            ? widget.store.currentUser!.fullName.trim()
                            : (widget.store.currentUser?.username ??
  ...
  String _typeLabel(AppLocalizations tr, String value) {
    switch (value) {
      ...
      case 'expense_refund':
        return _l(tr, 'Expense refund', 'استرداد زائد مصروف');
      case 'shortage':
      ...
      default:
        return value.trim().isEmpty
            ? _l(tr, 'Cash movement', 'حركة نقدية')
            : value;
    }
  }
}
...
class _CashHistorySnapshot {
  const _CashHistorySnapshot(
      {required this.rows, required this.total, required this.summary});
