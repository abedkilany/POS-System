...
class _CashPageState extends State<CashPage> {
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: openSales
                      .map((sale) => DropdownMenuItem<String>(
                            value: sale.id,
                            child: Text(
                                '${sale.invoiceNo} • ${sale.customerName} • ${formatUsdReferenceAmount(sale.balanceDue, widget.store.storeProfile)}'),
                          ))
                      .toList(),
                  onChanged: (value) {
                    final found =
                        openSales.where((sale) => sale.id == value).toList();
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: openPurchases
                      .map((purchase) => DropdownMenuItem<String>(
                            value: purchase.id,
                            child: Text(
                                '${purchase.purchaseNo} • ${purchase.supplierName} • ${formatUsdReferenceAmount(purchase.balanceDue, widget.store.storeProfile)}'),
                          ))
                      .toList(),
                  onChanged: (value) {
                    final found = openPurchases
                        .where((purchase) => purchase.id == value)
                        .toList();
                    if (found.isEmpty) return;
                    setDialogState(() {
  ...
  Future<void> _refundDialog() async {
    ...
    if (widget.store.canDeleteOrCancel) {
      for (final sale in widget.store.sales.where((item) => !item.isDeleted)) {
        try {
          final available =
              await widget.store.refundableSaleCashAmount(sale.id);
    ...
    if (widget.store.canManageExpenses) {
      for (final expense in widget.store.expenses.where(
        (item) => !item.isDeleted && item.isPosted,
      )) {
        try {
          final available =
              await widget.store.refundableExpenseCashAmount(expense.id);
          if (available > 0.000001) {
            options.add(<String, Object>{
              'kind': 'expense',
              'id': expense.id,
          ...
          // A single stale expense must not prevent other valid refunds.
        }
      }
    }
    ...
    if (options.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_l(
            tr,
            'No invoice or expense has a cash overpayment available for refund.',
            'لا توجد فاتورة أو مصروف لديه مبلغ نقدي زائد متاح للاسترداد.',
          )),
        ),
      );
      return;
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    labelText: _l(
                        tr, 'Refundable document', 'المستند القابل للاسترداد'),
                  ),
                  items: options.map((item) {
                    ...
                    final direction = kind == 'purchase'
                        ...
                        : kind == 'expense'
                            ...
                            : _l(
                                tr, 'Cash → Customer', 'من الصندوق إلى الزبون');
                ...
                const SizedBox(height: 12),
                TextField(
                  key: const ValueKey('CashRefundAmountField'),
                  controller: amountController,
                  keyboardType:
                      const TextInputType.numberWithOptions(decimal: true),
                  decoration: InputDecoration(
          ...
          actions: [
            ...
            FilledButton(
              key: const ValueKey('CashRefundConfirmButton'),
              onPressed: () => Navigator.pop(dialogContext, true),
    ...
    if (amount <= 0 || amount > available + displayRoundingTolerance) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              _l(tr, 'Invalid refund amount.', 'قيمة الاسترداد غير صالحة.')),
        ),
      );
      return;
    }
        ...
        SnackBar(
          content: Text(
            refunded > 0
                ...
                : _l(tr, 'Nothing remains to refund.',
                    'لا يوجد مبلغ متبقٍ للاسترداد.'),
          ),
        ),
      );
    } catch (error) {
  ...
  Future<void> _settleExpenseDialog(
    ...
    widget.store.requireAllPermissions(<String>{
      AppPermission.cashBoxManage,
    ...
    try {
      if (currentDrawer == null || currentSession == null) {
        throw StateError(
          _l(tr, 'An open cash shift is required.',
              'يجب وجود وردية صندوق مفتوحة.'),
        );
      }
  ...
  Future<void> _cashOperationDialog({
    ...
    for (final account in activeAccounts) {
      ...
      childrenByParent
          .putIfAbsent(parentId, () => <AccountingAccount>[])
          .add(account);
    }
    ...
    List<AccountingAccount> postableDescendants(String parentId) {
      ...
      while (pending.isNotEmpty) {
        ...
        pending.addAll(
            childrenByParent[account.id] ?? const <AccountingAccount>[]);
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Align(
                alignment: AlignmentDirectional.centerStart,
                child: Text(
                    '${_l(tr, 'Cash location', 'موقع النقدية')}: ${currentDrawer.name}'),
              ),
              const SizedBox(height: 12),
              ...
              DropdownButtonFormField<AccountingAccount>(
                ...
                onChanged: selectedParent == null
                    ? null
                    : (value) =>
                        setDialogState(() => selectedCounterpart = value),
              ),
              const SizedBox(height: 12),
              TextField(
                controller: amountController,
    ...
    if (amount <= 0) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text(_l(tr, 'Enter a valid amount.', 'أدخل مبلغاً صالحاً.'))),
        );
      }
      return;
    }
    try {
      final user = widget.store.activeUser;
      final service = CashOperationService.current(authorization: widget.store);
      final args = (
        cashLocationId: currentDrawer.id,
        cashDrawerSessionId: currentSession.id,
        ...
        storeId: widget.store.appIdentity.storeId,
        idempotencyKey:
            'cash-ui-$type-${DateTime.now().toUtc().microsecondsSinceEpoch}',
      );
  ...
  Future<void> _cashTransferDialog({
    ...
    if (candidates.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_l(tr, 'No eligible cash location was found.',
              'لا يوجد موقع نقدية مناسب للتحويل.'))));
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) => AlertDialog(
          title: Text(kind == 'shift_transfer'
              ? _l(tr, 'Shift transfer', 'تحويل شيفت')
              : _l(tr, 'Vault transfer', 'تحويل خزنة')),
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            DropdownButtonFormField<String>(
              initialValue: flow,
              decoration:
                  InputDecoration(labelText: _l(tr, 'Direction', 'الاتجاه')),
              items: <DropdownMenuItem<String>>[
                DropdownMenuItem(
                    value: 'out',
                    child: Text(
                        '${currentDrawer.name} → ${_l(tr, 'destination', 'الوجهة')}')),
                DropdownMenuItem(
                    value: 'in',
                    child: Text(
                        '${_l(tr, 'source', 'المصدر')} → ${currentDrawer.name}')),
              ],
            ...
            DropdownButtonFormField<String>(
              ...
              items: candidates
                  ...
                  .toList(),
              onChanged: (value) => setDialogState(
                  () => otherLocationId = value ?? otherLocationId),
            ),
            const SizedBox(height: 12),
            TextField(
                controller: amountController,
          ...
          actions: [
            ...
            FilledButton(
                onPressed: () => Navigator.pop(dialogContext, true),
                child: Text(_l(tr, 'Transfer', 'تحويل'))),
          ],
        ),
      ),
    );
    ...
    try {
      ...
      if (from.type == 'cash_drawer' && fromSessionId.isEmpty) {
        ...
        if (fromSessionId.isEmpty) {
          throw StateError(_l(tr, 'The source cash drawer has no open shift.',
              'درج النقدية المصدر لا يملك شيفتاً مفتوحاً.'));
        }
      }
      if (to.type == 'cash_drawer' && toSessionId.isEmpty) {
        ...
        if (toSessionId.isEmpty) {
          throw StateError(_l(
              tr,
              'The destination cash drawer has no open shift.',
              'درج النقدية الوجهة لا يملك شيفتاً مفتوحاً.'));
        }
      }
      final user = widget.store.activeUser;
      await AccountingService.createCashTransfer(
        ...
        transferKind: kind,
        idempotencyKey:
            'cash-ui-$kind-${DateTime.now().toUtc().microsecondsSinceEpoch}',
      );
      _refresh();
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(localizeRuntimeMessage(error.toString(), tr))));
    }
  }
  ...
  Future<void> _openDrawerDialog(
    ...
    try {
      ...
      await AccountingService.openCashDrawer(
        ...
        openedBy: activeUser?.fullName.trim().isNotEmpty == true
            ? activeUser!.fullName
            : widget.store.currentRole,
  ...
  Future<void> _closeDrawerDialog(
    ...
    try {
      ...
      await AccountingService.closeCashDrawer(
        ...
        closedBy: activeUser?.fullName.trim().isNotEmpty == true
            ? activeUser!.fullName
            : widget.store.currentRole,
  ...
  Future<List<CashLedgerTransaction>> _loadAllShiftMovements(
  ...
  Future<void> _showShiftReportsDialog() async {
    ...
    final sessionsFuture =
        AccountingService.listClosedCashDrawerSessions(limit: 500);
        ...
        builder: (dialogContext, setDialogState) => AlertDialog(
          title: Row(
            children: [
              ...
              Expanded(
                  child: Text(_l(
                      tr, 'Closed shift reports', 'تقارير الورديات المغلقة'))),
            ],
          ),
          content: SizedBox(
            width: 900,
                  ...
                  child: FutureBuilder<List<CashShiftReportSession>>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (snapshot.hasError) {
                        return Center(
                            child: Text(localizeRuntimeMessage(
                                snapshot.error.toString(), tr)));
                      }
                      final sessions =
                          snapshot.data ?? const <CashShiftReportSession>[];
                      if (sessions.isEmpty) {
                        return Center(
                            child: Text(_l(tr, 'No closed shifts found.',
      ...
      builder: (context, snapshot) {
        ...
        return FutureBuilder<List<AdvancedAccountingItem>>(
          ...
          builder: (context, sessionsSnapshot) {
            final sessions =
                sessionsSnapshot.data ?? const <AdvancedAccountingItem>[];
            final currentDrawerSessions = currentDrawer == null
                ? const <AdvancedAccountingItem>[]
                : sessions
                    .where((item) => item.referenceId == currentDrawer.id)
                    .toList(growable: false);
            final currentSession = currentDrawerSessions.isEmpty
                ? null
                : currentDrawerSessions.first;
            ...
            final expectedBalance =
                (currentSession?.credit ?? currentDrawer?.balance ?? 0)
                    .toDouble();
            final shiftNet =
                currentSession == null ? 0.0 : expectedBalance - openingBalance;
                      ...
                      return Wrap(
                        ...
                        children: [
                          _dashboardMetric(
                            ...
                            value: currentDrawer == null
                                ? '—'
                                : money(currentDrawer.balance),
                          ...
                          _dashboardMetric(
                            ...
                            value:
                                currentSession == null ? '—' : money(shiftNet),
            ...
            children: [
              ...
              Text(
                currentSession == null
                    ...
                    : _l(tr, 'Open shift • live cash overview',
                        'وردية مفتوحة • ملخص النقد المباشر'),
          ...
          child: Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color:
                      scheme.primary.withValues(alpha: emphasize ? 0.10 : 0.06),
  ...
  Widget _quickActionsCard(
    ...
    final enabled =
        canManage && currentDrawer != null && currentSession != null;
    return Card(
      elevation: 0,
      margin: EdgeInsets.zero,
      child: Padding(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: enabled && widget.store.canManageCustomerPayments
                ...
                OutlinedButton.icon(
                  onPressed: enabled &&
                          (widget.store.canDeleteOrCancel ||
                              widget.store.canManageSupplierPayments ||
                ...
                PopupMenuButton<String>(
                  enabled: enabled,
                  tooltip:
                      _l(tr, 'More cash operations', 'عمليات نقدية إضافية'),
                  onSelected: (value) {
                    if (currentDrawer == null || currentSession == null) return;
                    if (value == 'vault_transfer' ||
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            _shiftLine(context, _l(tr, 'Opening balance', 'رصيد الافتتاح'),
                money(opening)),
            _shiftLine(
                context, _l(tr, 'Shift net', 'صافي الوردية'), money(shiftNet)),
            _shiftLine(context, _l(tr, 'Expected balance', 'الرصيد المتوقع'),
                money(expected)),
            _shiftLine(context, _l(tr, 'Current balance', 'الرصيد الحالي'),
                money(actual),
                strong: true),
