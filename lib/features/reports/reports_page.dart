...
class ReportsPage extends StatelessWidget {
  ...
  Future<void> _print(
    ...
    await SimpleReportPdfService.printReport(
      ...
      arabic: arabic,
      lines: [
        '${tr.text('daily_sales_report')}: ${data.todaySales}',
        '${tr.text('monthly_sales')}: ${data.monthSales}',
        '${tr.text('monthly_purchases')}: ${data.monthPurchases}',
        '${tr.text('net_profit')}: ${data.netProfit}',
        '${tr.text('expenses')}: ${data.totalExpenses}',
        '${tr.text('inventory_value')}: ${data.inventoryRetailValue}',
        '${tr.text('products_below_limit')}: ${data.lowStockCount}',
        '${tr.text('cash_in')}: ${data.todayCashIn}',
        '${tr.text('cash_out')}: ${data.todayCashOut}',
      ],
    );
  }
        ...
        children: [
          Align(
              ...
              child: FilledButton.icon(
                  onPressed: () => _print(context, tr, data),
                  icon: const Icon(Icons.print_outlined),
                  label: Text(tr.text('print')))),
          ...
          GridView.count(
            ...
            children: [
              ...
              ReportCard(
                  ...
                  subtitle:
                      '${tr.text('net_profit')}: ${formatUsdReferenceAmount(netProfit, store.storeProfile)}'),
              ReportCard(
...
class _ReportsPageData {
  ...
  factory _ReportsPageData.fromSummary(Map<String, Object?> summary) {
    return _ReportsPageData(
      ...
      netProfit: _doubleValue(summary['netProfit']),
  ...
  factory _ReportsPageData.fromStore(AppStore store, DateTime today) {
    ...
    // Accounting cash/profit metrics never fall back to customer/supplier
    // transactions or operational profit estimates. If the DB-first snapshot
    // is unavailable, keep these accounting values neutral rather than
    // presenting misleading legacy figures.
    const todayCashIn = 0.0;
    const todayCashOut = 0.0;
    const todayCashInByMethod = <String, double>{};
    const todayCashOutByMethod = <String, double>{};
