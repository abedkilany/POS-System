...
class SettingsBackupActions {
  static Future<void> downloadBackupFile(
    ...
    try {
      ...
      final content = await store.exportEncryptedBackupJson(password);
  ...
  static Future<void> importBackupFile(
    ...
    final tr = AppLocalizations.of(context);
    final authorized = await requestSensitiveActionAuthorization(
      context,
      store,
    ...
    try {
      ...
      if (store.isEncryptedBackupJson(rawJson)) {
        // Automatic .vtb backups are encrypted with the Store Recovery Key.
        // On the originating store, restore transparently with the current key;
        // after a disaster/fresh install, fall back to asking for the original
        // recovery key (or the password used for a manual encrypted backup).
        var decrypted = false;
        if (isLocalBackupArchive) {
          final recoveryKey = store.appIdentity.recoveryKey.trim();
          if (recoveryKey.length >= 8) {
            try {
              rawJson = store.decryptBackupJson(rawJson, recoveryKey);
              decrypted = true;
            } catch (_) {
              // The archive may originate from another/freshly restored device.
            }
          }
        }
        if (!decrypted) {
      ...
      await store.recovery.importBackupJson(rawJson,
          selectedSectionIds: selectedSections);
      await _publishImportedSnapshotToDirectIfNeeded(store);
      final postRestoreHealth = await Phase10HealthService(store).run(deep: true);
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(postRestoreHealth.criticalCount == 0
              ? tr.text('backup_imported_health_verified')
              : tr.format('backup_imported_health_warning', {
                  'count': postRestoreHealth.criticalCount,
                })),
        ),
      );
    } catch (error) {
  ...
  static Future<void> downloadRecoveryFile(
    ...
    try {
      await downloadTextFile(
          filename: filename,
          content: store.recovery.exportRecoveryFileJson(
  ...
  static Future<void> clearLocalData(
    ...
    if (confirmed != true) return;
    if (!context.mounted ||
        !await requestSensitiveActionAuthorization(
          context,
          store,
  ...
  static Future<void> resetBusinessData(
    ...
    try {
      final backupPassword = await _requestBackupPassword(context);
