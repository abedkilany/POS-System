...
import '../maintenance/phase10_health_service.dart';
...
class SettingsPage extends StatelessWidget {
  ...
  List<Widget> _generalCards(BuildContext context) {
    ...
    return [
      ...
      _SectionCard(
        ...
        title: tr.text('negative_stock_settings'),
        subtitle: tr.text('negative_stock_settings_desc'),
        child: SwitchListTile.adaptive(
          contentPadding: EdgeInsets.zero,
          title: Text(tr.text('allow_negative_stock')),
          subtitle: Text(tr.text('allow_negative_stock_desc')),
          value: profile.allowNegativeStock,
          onChanged: store.hasPermission(AppPermission.settingsManage)
              ? (value) async {
                  await store.updateStoreProfile(
                    profile.copyWith(allowNegativeStock: value),
                  );
                }
              : null,
        ),
      ...
      _SectionCard(
        icon: Icons.account_balance_wallet_outlined,
        title: tr.text('negative_cash_settings'),
        subtitle: tr.text('negative_cash_settings_desc'),
        child: SwitchListTile.adaptive(
          contentPadding: EdgeInsets.zero,
          title: Text(tr.text('allow_negative_cash_balance')),
          subtitle: Text(tr.text('allow_negative_cash_balance_desc')),
          value: profile.allowNegativeCashBalance,
          onChanged: store.hasPermission(AppPermission.settingsManage)
              ? (value) async {
                  await store.updateStoreProfile(
                    profile.copyWith(allowNegativeCashBalance: value),
                  );
                }
              : null,
        ),
  ...
  String _costingMethodTitle(
    ...
    switch (method) {
      ...
      case InventoryCostingMethod.batch:
        return tr.text('unified_batch');
      ...
      child: Column(
        ...
        children: [
          ListTile(
            ...
            title: Text(tr.text('unified_batch')),
            subtitle: Text(tr.text('unified_batch_fixed_desc')),
  ...
  Future<void> _editOrganizationBranding(
    ...
    String storedLabel() {
      if (tr.isArabic) {
        return 'يُحفظ الشعار داخل قاعدة البيانات ويُزامن مع الأجهزة.';
      }
                            ...
                            onPressed: () async {
                              ...
                              if (bytes.length > maxLogoBytes) {
                                ...
                                ScaffoldMessenger.of(dialogContext)
                                    .showSnackBar(
                                  SnackBar(
                                    content: Text(tr.text('logo_too_large')),
                                  ),
                                );
                                return;
                              }
  ...
  Future<void> _resetBusinessData(BuildContext context) async {
    ...
    try {
      final backupPassword =
          await SettingsBackupActions._requestBackupPassword(context);
      if (backupPassword == null) return;
      final backup =
          'RESET_PROTECTION_TOKEN:$token\n${await store.exportEncryptedBackupJson(backupPassword)}';
    ...
    final passwordOk =
        await store.verifyAdminPassword(passwordController.text) &&
            await store.security.authorizeSensitiveAction(
              action: SensitiveAction.databaseDestructive,
...
class _UnifiedSyncSettingsCardState extends State<_UnifiedSyncSettingsCard> {
  ...
  Future<void> _refreshDirectSyncPlanAccess() async {
    ...
    try {
      ...
      if (token.isNotEmpty) {
        // Access tokens are intentionally short-lived. The Direct entitlement
        // check must use the refresh token as well, otherwise an otherwise
        // valid subscription is displayed as "Locked by plan" after the
        // 15-minute access-token lifetime expires.
        final result = await AccountAuthService().refreshSession(
          accountToken: token,
          refreshToken: cache?.refreshToken.trim() ?? '',
        );
        SyncDiagnosticsLog.add(
  ...
  Future<void> _saveSyncSettings() => _run(() async {
        widget.store.requirePermission(AppPermission.syncManage);
        final identity = widget.store.appIdentity;
        final tr = AppLocalizations.of(context);
        if (_deviceRole == DeviceRole.host) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final actionButtons = Wrap(
          ...
          children: [
            FilledButton.icon(
              onPressed: _busy || !_canManageSync ? null : _syncNow,
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(spacing: 10, runSpacing: 10, children: [
          OutlinedButton.icon(
            onPressed: _busy || !_canManageSync ? null : _scanPairingQr,
            icon: const Icon(Icons.qr_code_scanner_outlined),
            label: Text(tr.text('scan_qr_code')),
          ),
        ]),
        ...
        children: [
          ...
          if (isHost) ...[
            ...
            SizedBox(
              width: double.infinity,
              child: FilledButton.icon(
                onPressed: _busy || !_canManageSync || !approvedForThisDevice
                    ? null
