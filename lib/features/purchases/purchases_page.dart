...
class _PurchasesPageState extends State<PurchasesPage> {
  ...
  void didUpdateWidget(covariant PurchasesPage oldWidget) {
    ...
    if (oldWidget.store != widget.store) {
      ...
      _dataFuture = widget.store.commerce.ensurePurchasesLoaded();
    }
  }
      ...
      if (purchase != null) {
        for (var lineIndex = 0;
          ...
          final existing = item.batchAllocations.length == 1 &&
              ...
              : null;
          if (existing != null) {
            allocationsByLine[lineIndex] = <BatchAllocation>[existing];
            continue;
          }
          // Legacy drafts may predate line-level expiry capture. Prompt only
          // for those rows; normal Phase-2 drafts already carry the date.
  ...
  Future<List<BatchAllocation>?> _promptBatchAllocations(
    ...
    if (!product.expiryTrackingEnabled) {
      return <BatchAllocation>[
        BatchAllocation(batchId: '', quantity: quantity),
      ];
    }
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(labelText: tr.text('batch_number')),
              ),
              const SizedBox(height: 12),
              OutlinedButton.icon(
          ...
          actions: [
            ...
            FilledButton(
              key: const ValueKey('PurchaseBatchSaveButton'),
              onPressed: () {
    ...
    return <BatchAllocation>[allocation];
  }
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
              key: const ValueKey('PurchaseDeleteDraftConfirmButton'),
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            TextField(
              key: const ValueKey('PurchaseReturnReasonField'),
        ...
        actions: [
          TextButton(
              ...
              child: Text(AppLocalizations.of(context).text('cancel'))),
          FilledButton(
              key: const ValueKey('PurchaseReturnConfirmButton'),
              onPressed: () => Navigator.pop(context, true),
                    ...
                    if ((purchase.isDraft || purchase.isReceived) &&
                      ...
                      FilledButton.icon(
                        onPressed: () {
                          Navigator.pop(sheetContext);
                          if (!parentContext.mounted) return;
                          _openPurchaseDialog(parentContext,
                              editPurchase: purchase);
                        },
  ...
  Future<void> _openPurchaseDialog(BuildContext context,
      {Purchase? template, Purchase? editPurchase}) async {
    if (!widget.store.canManagePurchases) return;
    final tr = AppLocalizations.of(context);
    // The list/details view may hold an older version after another user or
    // device updated the invoice. Always build the edit form from SQLite's
    // latest snapshot; the save path still performs the final version check.
    if (editPurchase != null) {
      final latest =
          await widget.store.reloadPurchaseFromSqlite(editPurchase.id);
      if (latest == null) return;
      editPurchase = latest;
    }
    final formKey = GlobalKey<FormState>();
    final source = editPurchase ?? template;
    final items =
    ...
    void applySuggestedSupplierPrice() {
      ...
      final displayCost = fromUsdReferencePrice(
          suggestedBaseCost(product) * unit.conversionToBase,
          costCurrency,
          widget.store.storeProfile);
      costController.text =
    ...
    Future<void> createQuickProduct(StateSetter setDialogState) async {
      ...
      try {
        ...
        selectedExpiryDate = created.expiryTrackingEnabled &&
                created.defaultShelfLifeDays > 0
            ? DateTime.now().add(Duration(days: created.defaultShelfLifeDays))
            : null;
    ...
    Future<void> editPurchaseLine(
      ...
      DateTime? editExpiry = item.batchAllocations.isNotEmpty
          ? item.batchAllocations.first.expirationDate
          : null;
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          OutlinedButton.icon(
                            icon: const Icon(Icons.event_outlined),
                            onPressed: product?.expiryTrackingEnabled != true
                                ? null
                                : () async {
                                    final picked = await showDatePicker(
                                      context: editContext,
                                      initialDate: editExpiry ?? DateTime.now(),
                                      firstDate: DateTime.now(),
                                      lastDate: DateTime.now()
                                          .add(const Duration(days: 36500)),
                                    );
                                    if (picked != null) {
                                      setEditState(() => editExpiry = picked);
                                    }
                                  },
                            label: Text(product?.expiryTrackingEnabled != true
                                ? '${tr.text('expiration_date')} — ${tr.text('disabled')}'
                                : editExpiry == null
                                    ? '${tr.text('expiration_date')} *'
                                    : MaterialLocalizations.of(editContext)
                                        .formatMediumDate(editExpiry!)),
                          ),
    ...
    bool addPurchaseLine(Product product, ProductSaleUnit unit, double qty,
      ...
      if (product.expiryTrackingEnabled && selectedExpiryDate == null) {
        showPurchaseError(tr.text('expiration_date'));
        return false;
      }
      final cost =
      ...
      items.add(PurchaseItem(
        ...
        batchAllocations: product.expiryTrackingEnabled
            ? <BatchAllocation>[
                BatchAllocation(
                  batchId: '',
                  quantity: qty * unit.conversionToBase,
    ...
    Future<List<PurchaseItem>?> collectRequiredBatchAllocations(
      ...
      for (final item in sourceItems) {
        ...
        final existing = item.batchAllocations.length == 1 &&
            ...
            ? item.batchAllocations.first
            : null;
        final allocations = existing != null
            ? <BatchAllocation>[existing]
            : await _promptBatchAllocations(
                dialogContext,
                product: product,
                quantity: item.baseQuantity,
              );
        if (allocations == null || allocations.length != 1) return null;
        resolvedItems.add(PurchaseItem(
          lineId: item.lineId,
          productId: item.productId,
          productName: item.productName,
    ...
    Future<void> savePurchaseFromDialog(BuildContext dialogContext) async {
      ...
      try {
        ...
        ScaffoldMessenger.of(this.context).showSnackBar(
          SnackBar(
              content: Text(editPurchase != null
