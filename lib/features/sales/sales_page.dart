...
class _SalesPageState extends State<SalesPage> {
  ...
  bool _refreshingProductData = false;
  ...
  final RevisionValueCache<Map<String, _BarcodeProductMatch>>
      _barcodeLookupCache =
      RevisionValueCache<Map<String, _BarcodeProductMatch>>();
  final RevisionKeyCache<Map<String, String>> _invoiceSearchIndexCache =
      RevisionKeyCache<Map<String, String>>();
  Timer? _productSearchRevealTimer;
  int _storeUiRevision = 0;
  late Future<void> _dataFuture;

  void _handleStoreChanged() {
    if (!mounted) return;
    final nextRevision = _currentStoreUiRevision();
    if (nextRevision == _storeUiRevision) return;
    setState(() {
      _storeUiRevision = nextRevision;
    });
  }
  ...
  Future<void> _refreshProductData(AppLocalizations tr) async {
    if (_refreshingProductData) return;
    setState(() => _refreshingProductData = true);
    try {
      await widget.store.refreshSalesProductData();
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(tr.text('refresh_completed'))),
      );
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (mounted) setState(() => _refreshingProductData = false);
    }
  }
  ...
  Map<String, _BarcodeProductMatch> _barcodeLookup() {
    return _barcodeLookupCache.getOrCompute(widget.store.productsRevision, () {
      ...
      for (final product in widget.store.products) {
        ...
        if (code.isNotEmpty) {
          lookup[code] =
              _BarcodeProductMatch(product: product, unit: defaultUnit);
        }
        ...
        for (final unit in saleUnits) {
          ...
          if (unitBarcode.isEmpty) continue;
          lookup[unitBarcode] =
              _BarcodeProductMatch(product: product, unit: unit);
        }
      }
      return lookup;
    });
  ...
  double _salePriceFor(Product product, ProductSaleUnit unit) {
    final configured = widget.store.productPriceFor(
      product.id,
      _salePriceType,
      unitId: unit.id,
    );
    if (configured != null) {
      final override = widget.store.productPriceOverrideFor(
        configured,
        widget.store.storeProfile.defaultSaleInvoiceCurrency,
      );
      if (override != null) {
        return toUsdReferencePrice(
          override.amount,
      ...
      return toUsdReferencePrice(
        configured.baseAmount,
        configured.baseCurrencyCode,
        widget.store.storeProfile,
      );
    }
    return widget.store.defaultProductUsdPrice(product, unitId: unit.id);
  }

  void _setSalePriceType(String value) {
    if (value == _salePriceType) return;
    setState(() {
      _salePriceType = value;
      for (var index = 0; index < _cart.length; index++) {
        final item = _cart[index];
        _cart[index] = item.copyWith(
          saleUnit: item.saleUnit.copyWith(
            price: _salePriceFor(item.product, item.saleUnit),
          ),
        );
      }
    });
  ...
  Widget _buildSalePriceTypeSelector(
    ...
    return Wrap(
      ...
      children: [
        Text('${tr.text('invoice_price_type')}:',
            style: Theme.of(context).textTheme.labelLarge),
        ChoiceChip(
          label: Text(tr.text('retail_price')),
          selected: _salePriceType == 'retail',
          onSelected: (_) => _setSalePriceType('retail'),
        ),
        ChoiceChip(
          label: Text(tr.text('wholesale_price')),
          selected: _salePriceType == 'wholesale',
          onSelected: (_) => _setSalePriceType('wholesale'),
        ),
        ChoiceChip(
          label: Text(tr.text('bulk_wholesale_price')),
          selected: _salePriceType == 'wholesale_bulk',
          onSelected: (_) => _setSalePriceType('wholesale_bulk'),
        ),
      ],
    );
  }
        ...
        Expanded(
          child: RawAutocomplete<Customer>(
            ...
            displayStringForOption: (customer) =>
                _customerSearchText(customer, tr),
          ...
          AppSectionHeader(
            ...
            action: Wrap(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: _refreshingProductData
                      ? null
                      : () => _refreshProductData(tr),
                  icon: _refreshingProductData
                      ? const SizedBox(
                          width: 16,
                          height: 16,
                          child: CircularProgressIndicator(strokeWidth: 2),
    ...
    final content = compact
        ...
        : Row(
            children: [
              ...
              FilledButton.icon(
                key: const ValueKey('SalesContinuePaymentDesktop'),
    ...
    for (final item in heldCart.items) {
      ...
      restored.add(_DraftSaleItem(
          ...
          saleUnit: restoredUnit.copyWith(
            price: _salePriceFor(product, restoredUnit),
          )));
    }
    if (restored.isEmpty) {
      if (!mounted) return;
  ...
  Widget _buildInvoicesPanelFromSales(
    ...
    final query = _invoiceSearchController.text.trim().toLowerCase();
    final searchIndex = _invoiceSearchIndex(sales);
    final filteredSales = query.isEmpty
        ? sales
        : sales
            .where((sale) => searchIndex[sale.id]?.contains(query) ?? false)
            .toList(growable: false);
    ...
    return _invoiceSearchIndexCache.getOrCompute(
      ...
      () {
        ...
        for (final sale in sales) {
          ...
          index[sale.id] = buffer.toString().toLowerCase();
        }
        return index;
      },
    );
  ...
  _BarcodeAddResult _addProduct(Product product,
    ...
    final selectedUnit = rawSelectedUnit.copyWith(
      price: _salePriceFor(product, rawSelectedUnit),
    );
  ...
  Future<_BarcodeAddResult> _addByCode(String code) async {
    ...
    _barcodeController.clear();
    return _addProduct(match.product,
        saleUnit: match.unit, showBarcodeFeedback: true);
  }
  ...
  Future<void> _returnSale(BuildContext sheetContext, Sale sale) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(
            tr.format('invoice_return_confirm', {'invoice': sale.invoiceNo})),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogContext, false),
            child: Text(tr.text('cancel')),
          ...
          FilledButton(
            key: const ValueKey('SalesReturnConfirmButton'),
            onPressed: () => Navigator.pop(dialogContext, true),
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(tr.text('invoice_return_created')),
        ),
      );
    } catch (error) {
      if (!mounted) return;
...
class _BarcodeProductMatch {
  const _BarcodeProductMatch({
