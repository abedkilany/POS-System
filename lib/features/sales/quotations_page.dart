...
class _QuotationsPageState extends State<QuotationsPage> {
  ...
  Future<void> _printQuotation(SaleQuotation quotation) async {
    ...
    await SimpleReportPdfService.printReport(
      ...
      lines: [
        '${tr.text('customer')}: ${quotation.customerName}',
        '${tr.text('status')}: ${_localizedStatus(tr, quotation.status)}',
        ...quotation.items
            .map((item) => '${item.productName} • ${item.quantity}'),
        '${tr.text('total')}: ${quotation.total.toStringAsFixed(2)} ${quotation.invoiceCurrency}',
      ],
    );
  }
