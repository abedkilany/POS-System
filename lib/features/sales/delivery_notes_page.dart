...
class _DeliveryNotesPageState extends State<DeliveryNotesPage> {
  ...
  Future<void> _printNote(DeliveryNote note) async {
    ...
    await SimpleReportPdfService.printReport(
      ...
      lines: [
        '${tr.text('customer')}: ${note.customerName}',
        '${tr.text('invoice')}: ${note.invoiceNo}',
        '${tr.text('status')}: ${_statusLabel(tr, note.status)}',
        ...note.items.map((item) =>
            '${item.productName} • ${item.quantity} ${item.unitName}'.trim()),
      ],
    );
  }
