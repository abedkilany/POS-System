...
class _ExpensesPageState extends State<ExpensesPage> {
  ...
  Future<void> _openExpenseForm(BuildContext context, {Expense? expense}) async {
    ...
    if (result != null) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context).text(expense == null ? 'expense_saved' : 'expense_updated'))));
      }
    }
  }
}
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: VentioResponsive.clampToScreen(
        context,
        VentioResponsive.adaptiveWidth(context, mobile: 180, tablet: 200, desktop: 220),
        min: 150,
      ),
      child: Card(
        child: ListTile(
