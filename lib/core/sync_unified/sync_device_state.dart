...
class SyncDeviceState {
  ...
  SyncDeviceState copyWith({
    ...
    return SyncDeviceState(
      ...
      lastAppliedHostCursor: clearLastAppliedHostCursor
          ? null
          : (lastAppliedHostCursor ?? this.lastAppliedHostCursor),
      lastAckCursor:
      ...
      lanLastAckCursor:
          clearLanProgress ? null : (lanLastAckCursor ?? this.lanLastAckCursor),
      lanLastAppliedSequence: clearLanProgress
          ? 0
          : (lanLastAppliedSequence ?? this.lanLastAppliedSequence),
      lanLastAckSequence: clearLanProgress
          ? 0
          : (lanLastAckSequence ?? this.lanLastAckSequence),
  ...
  Map<String, dynamic> toJson() => {
        ...
        'lanLastAckSequence': lanLastAckSequence,
        'lastSyncTransport': lastSyncTransport,
        'lastSeenAt': lastSeenAt?.toIso8601String(),
        'updatedAt': updatedAt?.toIso8601String(),
      };

  factory SyncDeviceState.fromJson(
      Map<String, dynamic> json, AppIdentity identity) {
    ...
    return SyncDeviceState(
      ...
      activeTransport: _normalizeTransport(
          activeRaw.isNotEmpty ? activeRaw : identity.activeSyncTransport),
      lastAppliedHostCursor:
          DateTime.tryParse(json['lastAppliedHostCursor']?.toString() ?? ''),
      ...
      lanLastAckCursor:
          DateTime.tryParse(json['lanLastAckCursor']?.toString() ?? ''),
      lanLastAppliedSequence:
          int.tryParse(json['lanLastAppliedSequence']?.toString() ?? '') ?? 0,
      lanLastAckSequence:
          int.tryParse(json['lanLastAckSequence']?.toString() ?? '') ?? 0,
...
class HostPeerSyncState {
  ...
  factory HostPeerSyncState.fromJson(Map<String, dynamic> json) =>
      HostPeerSyncState(
        deviceId: json['deviceId']?.toString() ?? '',
        lastAppliedHostCursor:
            DateTime.tryParse(json['lastAppliedHostCursor']?.toString() ?? ''),
        lastAckCursor:
            DateTime.tryParse(json['lastAckCursor']?.toString() ?? ''),
        lastAppliedSequence:
            int.tryParse(json['lastAppliedSequence']?.toString() ?? '') ?? 0,
        lastAckSequence:
            int.tryParse(json['lastAckSequence']?.toString() ?? '') ?? 0,
...
class SyncDeviceStateStore {
  ...
  static SyncDeviceState load(AppIdentity identity) {
    ...
    if (raw != null && raw.trim().isNotEmpty) {
      try {
        state = SyncDeviceState.fromJson(
            jsonDecode(raw) as Map<String, dynamic>, identity);
      } catch (_) {
    ...
    if (state.lastAppliedHostCursor == null && legacyCursor != null) {
      state = state.copyWith(
          lastAppliedHostCursor: legacyCursor, updatedAt: DateTime.now());
    }
    return state;
  }

  static Future<SyncDeviceState> save(
      AppIdentity identity, SyncDeviceState state) async {
    ...
    await LocalDatabaseService.setString(
        _stateKey, jsonEncode(normalized.toJson()));
    return normalized;
  }
  ...
  static Future<void> resetClientProgress(AppIdentity identity,
    ...
    await save(
      ...
      current.copyWith(
        ...
        lanLastAckCursor:
            normalizedTransport == 'lan' ? null : current.lanLastAckCursor,
        lanLastAppliedSequence:
            normalizedTransport == 'lan' ? 0 : current.lanLastAppliedSequence,
        lanLastAckSequence:
            normalizedTransport == 'lan' ? 0 : current.lanLastAckSequence,
        lastSyncTransport: normalizedTransport,
  ...
  static DateTime? lastSuccessfulSyncAt(AppIdentity identity) {
    ...
    if (identity.isHost) {
      for (final peer in loadPeerStates()) {
        var peerLatest =
            _latest(peer.lastAckCursor, peer.lastAppliedHostCursor);
  ...
  static Future<void> setActiveTransport(
      AppIdentity identity, String transport) async {
    final current = load(identity);
    await save(
        identity,
        current.copyWith(
            activeTransport: transport, updatedAt: DateTime.now()));
  }
    ...
    await save(
      ...
      current.copyWith(
        ...
        lanLastAppliedHostCursor: normalizedTransport == 'lan'
            ...
            : current.lanLastAppliedHostCursor,
        lanLastAckCursor: normalizedTransport == 'lan'
            ? (ackCursor ?? appliedCursor ?? current.lanLastAckCursor)
            : current.lanLastAckCursor,
        lanLastAppliedSequence: normalizedTransport == 'lan'
            ? _latestInt(current.lanLastAppliedSequence, appliedSequence)
            : current.lanLastAppliedSequence,
        lanLastAckSequence: normalizedTransport == 'lan'
            ? _latestInt(
                current.lanLastAckSequence, ackSequence ?? appliedSequence)
            : current.lanLastAckSequence,
  ...
  static DateTime? cursorForTransport(AppIdentity identity, String transport,
    ...
    final state = load(identity);
    final normalizedTransport = _normalizeTransport(transport);
    final transportCursor = _transportAppliedCursor(state, normalizedTransport);
    return _latest(transportCursor, currentTransportCursor);
  }

  static DateTime? lastAppliedCursorForTransport(
      AppIdentity identity, String transport) {
    return _transportAppliedCursor(
        load(identity), _normalizeTransport(transport));
  }

  static int lastAppliedSequenceForTransport(
      AppIdentity identity, String transport) {
    return _transportAppliedSequence(
        load(identity), _normalizeTransport(transport));
  }

  static DateTime? lastAckCursorForTransport(
      AppIdentity identity, String transport) {
    return _transportAckCursor(load(identity), _normalizeTransport(transport));
  }

  static int lastAckSequenceForTransport(
      AppIdentity identity, String transport) {
    return _transportAckSequence(
        load(identity), _normalizeTransport(transport));
  }

  static DateTime? _transportAppliedCursor(
      SyncDeviceState state, String transport) {
    final normalized = _normalizeTransport(transport);
    ...
    switch (normalized) {
      ...
      case 'lan':
        return state.lanLastAppliedHostCursor ??
            (fallbackToLegacy ? state.lastAppliedHostCursor : null);
      default:
        return state.lastAppliedHostCursor;
    }
  }

  static DateTime? _transportAckCursor(
      SyncDeviceState state, String transport) {
    final normalized = _normalizeTransport(transport);
    final fallbackToLegacy =
        _normalizeTransport(state.lastSyncTransport) == normalized;
  ...
  static int _transportAppliedSequence(
      SyncDeviceState state, String transport) {
    final normalized = _normalizeTransport(transport);
    final fallbackToLegacy =
        _normalizeTransport(state.lastSyncTransport) == normalized;
  ...
  static int _transportAckSequence(SyncDeviceState state, String transport) {
    final normalized = _normalizeTransport(transport);
    final fallbackToLegacy =
        _normalizeTransport(state.lastSyncTransport) == normalized;
    switch (normalized) {
      ...
      case 'lan':
        return state.lanLastAckSequence != 0
            ? state.lanLastAckSequence
            : (fallbackToLegacy ? state.lastAckSequence : 0);
      default:
        return state.lastAckSequence;
    }
  }
  ...
  static List<HostPeerSyncState> loadPeerStates() {
    ...
    try {
      ...
      return decoded
          .map((item) => HostPeerSyncState.fromJson(
  ...
  static Future<void> recordPeerSyncResult({
    ...
    final byId = <String, HostPeerSyncState>{
      for (final state in states) state.deviceId: state
    };
    final current = byId[id];
    byId[id] = HostPeerSyncState(
      deviceId: id,
      lastAppliedHostCursor:
          _latest(current?.lastAppliedHostCursor, appliedCursor),
      lastAckCursor:
          _latest(current?.lastAckCursor, ackCursor ?? appliedCursor),
      lastAppliedSequence:
    ...
    _setLivePeerStatus(
      id,
      online: true,
      ackSequence: _latestInt(
          current?.lastAckSequence ?? 0, ackSequence ?? appliedSequence),
      ackAt: (ackCursor != null ||
              appliedCursor != null ||
              (ackSequence ?? 0) > 0 ||
              (appliedSequence ?? 0) > 0)
          ? now
  ...
  static Future<void> removePeerState(String deviceId) async {
    ...
    if (id.isEmpty) return;
    final states =
        loadPeerStates().where((state) => state.deviceId != id).toList();
    await LocalDatabaseService.setString(
      _hostPeerStatesKey,
...
class ClientSuspensionStateStore {
  ...
  static bool get isSuspended =>
      (LocalDatabaseService.getString(_suspendedKey) ?? '').trim() == 'true';
  static String get reason => LocalDatabaseService.getString(_reasonKey) ?? '';
  static DateTime? get updatedAt =>
      DateTime.tryParse(LocalDatabaseService.getString(_updatedAtKey) ?? '');
  ...
  static Future<void> markSuspended({String reason = ''}) async {
    ...
    await LocalDatabaseService.setString(
        _reasonKey,
        reason.trim().isEmpty
            ? 'This device has been suspended by the Host.'
            : reason.trim());
    await LocalDatabaseService.setString(
...
class SyncDeviceAccessStore {
  ...
  static const String _suspendedDevicesKey =
      'sync_monitoring_suspended_devices_v1';
  static const String _deletedDevicesKey = 'sync_monitoring_deleted_devices_v1';
  static const String _deletedDeviceTokensKey =
      'sync_monitoring_deleted_device_tokens_v1';
  static const String _wipePendingDevicesKey =
      'sync_monitoring_wipe_pending_devices_v1';
  static const String _wipePendingDeviceTokensKey =
      'sync_monitoring_wipe_pending_device_tokens_v1';
  ...
  static bool isSuspended(String deviceId) =>
      suspendedDeviceIds().contains(deviceId.trim());
  static bool isDeleted(String deviceId) =>
      deletedDeviceIds().contains(deviceId.trim());
  static bool isWipePending(String deviceId) =>
      wipePendingDeviceIds().contains(deviceId.trim());

  static Map<String, String> deletedDeviceTokens() =>
      _loadMap(_deletedDeviceTokensKey);
  static Map<String, String> wipePendingDeviceTokens() =>
      _loadMap(_wipePendingDeviceTokensKey);
  ...
  static Future<void> markWipePending(String deviceId,
  ...
  static Future<void> markDeleted(String deviceId,
      {String deviceToken = ''}) async {
    final id = deviceId.trim();
    if (id.isEmpty) return;
