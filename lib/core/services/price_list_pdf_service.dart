  ...
  static Future<Uint8List> build({
    ...
    final data = products
        .map((p) => [
              p.name,
              ...fields.map((field) =>
                  valueResolver?.call(p, field) ?? _fieldValue(p, field)),
            ])
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) => context.pageNumber == 1
            ? ProfessionalPdfTheme.header(
                ...
                meta: [
                  ...
                  MapEntry(arabic ? 'عدد الأصناف' : 'Products',
                      '${products.length}'),
                ],
              )
        ...
        footer: (context) => ProfessionalPdfTheme.footer(
            context: context, profile: profile, isArabic: arabic),
        build: (_) => [
          if (products.isEmpty)
            pw.Padding(
              padding: const pw.EdgeInsets.symmetric(vertical: 30),
              child: pw.Center(
                  child: pw.Text(arabic ? 'لا توجد أصناف.' : 'No products.')),
            )
          else
