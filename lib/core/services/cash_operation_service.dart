...
class CashOperationService {
  ...
  factory CashOperationService.current({
    ...
    return CashOperationService(db, authorization: authorization);
  }

  final VentioDriftDatabase _db;
  final BusinessSessionContext _authorization;
  ...
  Future<CashOperationResult> _post({
    ...
    _authorization.requirePermission(AppPermission.cashBoxManage);
    final cleanLocationId = cashLocationId.trim();
    if (cleanLocationId.isEmpty) {
      throw ArgumentError('Cash location is required.');
    }
    ...
    final result = await _db.transaction(() async {
      ...
      final location = await _db.customSelect(
        "SELECT id, name, account_id, current_balance FROM cash_locations WHERE id = ? AND deleted_at = '' AND is_active = 1 LIMIT 1",
        variables: <Variable<Object>>[Variable<String>(cleanLocationId)],
      ).getSingleOrNull();
      if (location == null) {
        throw StateError('Cash location does not exist or is inactive.');
      }
      final cashAccountId =
          location.data['account_id']?.toString().trim() ?? '';
      if (cashAccountId.isEmpty) {
        throw StateError('Cash location has no accounting account.');
      }
      ...
      if (sessionId.isNotEmpty) {
        ...
        if (session == null) {
          throw StateError(
              'Cash drawer session is not open for this location.');
        }
      }
      ...
      if (cleanCounterpartAccountId == cashAccountId) {
        throw StateError(
            'Counterpart account cannot be the same as the cash account.');
      }
      final counterpartRow = await _db.customSelect(
        ...
        variables: <Variable<Object>>[
          Variable<String>(cleanCounterpartAccountId)
        ],
      ).getSingleOrNull();
      if (counterpartRow == null) {
        throw StateError(
            'Selected counterpart account does not exist or is inactive.');
      }
      if ((counterpartRow.data['subtype']?.toString() ?? '') == 'group') {
        throw StateError(
            'A posting account must be selected as the counterpart.');
      }
      ...
      final description = switch (type) {
        'cash_deposit' =>
          'Cash deposit - ${location.data['name']?.toString() ?? ''}',
        'cash_withdrawal' =>
          'Cash withdrawal - ${location.data['name']?.toString() ?? ''}',
        'expense' =>
          'Cash expense - ${location.data['name']?.toString() ?? ''}',
        _ => 'Cash operation',
      };
      ...
      if (entryId.isEmpty) {
        throw StateError('Cash operation journal was not created.');
      }
