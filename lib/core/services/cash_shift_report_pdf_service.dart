      ...
      pw.MultiPage(
        ...
        header: (context) => context.pageNumber == 1
            ? ProfessionalPdfTheme.header(
                ...
                title: title,
                englishTitle:
                    detailed ? 'SHIFT DETAILS REPORT' : 'SHIFT SUMMARY',
                isArabic: ar,
                meta: [
                  MapEntry(ar ? 'رقم الوردية' : 'Shift no.', shiftNo),
                  MapEntry(
                      ar ? 'وقت الفتح' : 'Opened at', date(session.openedAt)),
                  MapEntry(
                      ar ? 'وقت الإغلاق' : 'Closed at', date(session.closedAt)),
                ],
              )
            : ProfessionalPdfTheme.compactHeader(
                profile: profile, title: title),
        footer: (context) => ProfessionalPdfTheme.footer(
            context: context, profile: profile, isArabic: ar),
        build: (_) => [
          ProfessionalPdfTheme.infoStrip(
            isArabic: ar,
            entries: [
              MapEntry(
                  ar ? 'الصندوق' : 'Cash drawer', session.cashLocationName),
          ...
          if (session.notes.trim().isNotEmpty) ...[
            ...
            ProfessionalPdfTheme.note(
                ar ? 'الملاحظات' : 'Notes', session.notes.trim(),
                isArabic: ar),
          ],
          if (detailed) ...[
            ...
            if (movements.isEmpty)
              pw.Text(ar
                  ? 'لا توجد حركات مرتبطة بهذه الوردية.'
                  : 'No movements are linked to this shift.')
            else
              ProfessionalPdfTheme.table(
                headers: ar
                    ? [
                ...
                data: movements.map((m) {
                  final reference = [m.partyName, m.referenceNumber]
