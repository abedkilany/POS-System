  ...
  static Future<File> createBackupNow(
    ...
    try {
      ...
      final recoveryDir = Directory(
          '${root.path}${Platform.pathSeparator}Recovery checkpoints');
      ...
      if (reason == 'manual' || reason == 'disaster_recovery_checkpoint') {
        ...
        if (isRecovery) {
          // Keep a bounded set of independently restorable checkpoints so the
          // recovery tool never silently reuses today's scheduled backup.
          await _trimBackups(recoveryDir, 5);
        }
        await LocalDatabaseService.setString(
        ...
        status.value = LocalAutoBackupStatus(
            lastSuccessAt: now,
            message: isRecovery
                ? 'Disaster-recovery checkpoint completed.'
                : 'Manual local backup completed.');
        return file;
      }
