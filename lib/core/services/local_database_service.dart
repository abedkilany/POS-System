...
class LocalDatabaseService {
  ...
  static const Set<String> _phase4SecureKeys = <String>{
    _secureRecoveryKeyKey,
  ...
  static bool _sqliteDatabaseForTesting = false;
  ...
  // Read by production code to keep Phase 4 cutover disabled for explicitly
  // attached SQLite test databases. Mutation remains test-only below.
  static bool get isSqliteDatabaseForTesting => _sqliteDatabaseForTesting;
  ...
  static Future<void> useSqliteDatabaseForTesting(
    ...
    _sqliteDatabaseForTesting = true;
    _sqliteMirror
      ..clear()
  ...
  static Future<void> resetForTesting() async {
    ...
    _sqliteReady = false;
    _sqliteDatabaseForTesting = false;
    _sqliteMirror.clear();
    _secureStringMirror.clear();
  ...
  static Future<void> _initializeInternal() {
    return StartupTimingService.measure(
      ...
      () async {
        ...
        // Schema 25: warehouse_inventory is the sole persisted inventory source.
        // Migrate any genuinely legacy stock once, then physically remove the
        // retired products.stock column before business data is hydrated.
        await StartupTimingService.measure(
          'retire_legacy_product_stock',
          () async {
            await InventoryReconciliationRepository.backfillFromLegacyData(activeDb);
            await activeDb.dropLegacyProductStockColumn();
          },
          category: 'database',
        );
  ...
  /// Loads the small set of device-bound secrets before the widget tree starts.
  /// Business/database initialization remains independently asynchronous.
  static Future<void> initializeSecureStorage() async {
    if (_memoryStoreForTesting != null) return;
    for (final key in _phase4SecureKeys) {
      try {
        final value = await _secureStorage.read(key: key);
        if (value == null || value.isEmpty) {
          _secureStringMirror.remove(key);
        } else {
          _secureStringMirror[key] = value;
        }
      } catch (_) {
        // Secure storage may be unavailable on unsupported web contexts.
        // Callers still fail closed because no secret is exposed from SQLite.
      }
    }
  }

  static String? getSecureString(String key) => _secureStringMirror[key];

  static Future<String?> readSecureString(String key) async {
    final memory = _memoryStore;
    if (memory != null) return _secureStringMirror[key];
    try {
      final value = await _secureStorage.read(key: key);
      if (value == null || value.isEmpty) {
        _secureStringMirror.remove(key);
      ...
      return _secureStringMirror[key];
    }
  }
  ...
  static Future<void> setSecureString(String key, String value) async {
    ...
    _secureStringMirror[key] = cleaned;
    if (_memoryStore != null) return;
    await _secureStorage.write(key: key, value: cleaned);
  }
    ...
    await db.transaction(() async {
      ...
      for (final row in batches) {
        ...
        ''', <Object?>[
          ...
          row['sourceLineId']?.toString() ?? '',
          (row['unitCost'] as num? ?? 0).toDouble(),
          (row['initialQuantity'] as num? ?? 0).toDouble(),
          row['costCurrency']?.toString() ?? 'USD',
          (row['exchangeRate'] as num? ?? 1).toDouble(),
          row['receivedAt']?.toString() ?? row['createdAt']?.toString() ?? '',
  ...
  static const Map<String, String> phase8AccountingSnapshotTables =
      <String, String>{
    ...
    'cashRefundAllocations': 'cash_refund_allocations',
  ...
  static Future<void> replacePhase8AccountingSnapshotRowsImmediate(
    ...
    // Older accounting snapshots do not contain Phase 1 chart rows or the
    // is_postable column. Re-applying the idempotent foundation preserves the
    // restored data while backfilling protected system accounts and flags.
  ...
  static Future<void> clearAll() async {
    ...
    if (_sqliteReady && db != null) {
      ...
      // Phase 6 audit history is append-only and intentionally survives
      // data resets/restores. The reset/restore action is appended separately.
      _sqliteMirror
        ..clear()
  ...
  static Future<void> replaceWarehouseInventoryRowsImmediate(
    ...
    if (!allowNegativeStock) {
      for (final row in rows) {
        final quantity = _doubleValue(row['quantity'], fallback: 0);
        if (quantity < -0.000001) {
          throw StateError(
            'Negative warehouse inventory is not allowed by the current stock policy.',
          );
        }
      }
    }
  ...
  static Future<void> replaceStockMovementRowsImmediate(
    ...
    final db = SqliteMigrationManager.database;
    if (!_sqliteReady || db == null) return;
    final replacedAt = DateTime.now().toUtc().toIso8601String();
    await db.customUpdate(
      "UPDATE stock_movements SET deleted_at = ?, updated_at = ? WHERE deleted_at = ''",
      variables: <Variable<Object>>[
        Variable<String>(replacedAt),
        Variable<String>(replacedAt),
      ],
    );
    for (final row in rows) {
