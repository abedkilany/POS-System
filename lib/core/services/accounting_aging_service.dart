...
class AccountingAgingService {
  ...
  static Future<AgingReportResult> customerAgingReport(
      {DateTime? asOfDate}) async {
    final db = _dbOrNull;
    if (db == null) {
      return AgingReportResult(
          asOfDate: _dateOnly(asOfDate ?? DateTime.now()),
    ...
    return _buildFromLedgerRows(rows, asOfDate ?? DateTime.now(),
        receivableMode: true);
  }

  static Future<AgingReportResult> supplierAgingReport(
      {DateTime? asOfDate}) async {
    final db = _dbOrNull;
    if (db == null) {
      return AgingReportResult(
  ...
  static AgingReportResult customerAgingFromStore({
    ...
    for (final sale in sales) {
      ...
      final paidInInvoiceCurrency =
          _clean(sale.paidAmount.clamp(0, invoiceTotal).toDouble());
      final paid = invoiceTotal <= 0
          ? 0.0
          : _clean(saleTotal * (paidInInvoiceCurrency / invoiceTotal));
      ...
      if (open > 0) {
        documents.add(_AgingDocument(
          ...
          partyId: partyId,
          partyName: sale.customerName.trim().isEmpty
              ? 'Walk-in customer'
    ...
    for (final txn in accountTransactions) {
      if (txn.isDeleted || !txn.isCustomer || txn.date.isAfter(asOf)) {
        continue;
      }
      ...
      if (amount > 0) {
        ...
        paymentsByParty[partyId] =
            _roundMoney((paymentsByParty[partyId] ?? 0) + amount);
      }
    }
  ...
  static AgingReportResult supplierAgingFromStore({
    ...
    for (final purchase in purchases) {
      if (purchase.isDeleted ||
          ...
          purchase.subtotal <= 0 ||
          purchase.date.isAfter(asOf)) {
        continue;
      }
      final partyId = _partyKey(purchase.supplierId, purchase.supplierName);
      final subtotal = _clean(purchase.subtotal);
      final open = _roundMoney(
          subtotal - _clean(purchase.paidAmount.clamp(0, subtotal).toDouble()));
      if (open > 0) {
        documents.add(_AgingDocument(
          id: purchase.id,
          number: purchase.purchaseNo,
          partyId: partyId,
          partyName: purchase.supplierName.trim().isEmpty
              ? 'Unknown supplier'
  ...
  static AgingReportResult _buildFromLedgerRows(
      List<QueryRow> rows, DateTime asOfDate,
      {required bool receivableMode}) {
    ...
    for (final row in rows) {
      ...
      final date = _dateOnly(
          DateTime.tryParse(data['entry_date']?.toString() ?? '') ?? asOf);
      ...
      if (amount > 0) {
        documents.add(_AgingDocument(
          ...
          partyId: partyId,
          partyName: partyName.isEmpty
              ? (receivableMode ? 'Walk-in customer' : 'Unknown supplier')
              : partyName,
          date: date,
        ...
        paymentsByParty[partyId] =
            _roundMoney((paymentsByParty[partyId] ?? 0) + amount.abs());
      }
    }
    return _buildReport(documents, paymentsByParty, asOf);
  }

  static AgingReportResult _buildReport(List<_AgingDocument> documents,
      Map<String, double> paymentsByParty, DateTime asOf) {
    documents.sort((a, b) {
      final byParty =
    ...
    for (final partyId in paymentsByParty.keys) {
      ...
      final partyDocs = documents
          .where((doc) => doc.partyId == partyId)
          .toList()
        ..sort((a, b) => a.date.compareTo(b.date));
      for (final doc in partyDocs) {
        if (remainingPayment <= 0) break;
        final applied = remainingPayment > doc.openAmount
            ? doc.openAmount
            : remainingPayment;
    ...
    for (final doc in documents) {
      ...
      final totalsRow = totals.putIfAbsent(doc.partyId,
          () => _AgingTotals(partyId: doc.partyId, partyName: doc.partyName));
    ...
    final rows = totals.values
        .map((row) => row.toRow())
        .where((row) => row.total > 0.009)
        .toList()
      ..sort((a, b) => b.total.compareTo(a.total));
    openDocuments.sort((a, b) => b.openAmount.compareTo(a.openAmount));
    return AgingReportResult(
        asOfDate: asOf, rows: rows, openDocuments: openDocuments);
  }
  ...
  static String _partyKey(String id, String name) =>
      id.trim().isNotEmpty ? id.trim() : 'name:${name.trim().toLowerCase()}';
...
class _AgingTotals {
  ...
  AgingReportRow toRow() => AgingReportRow(
        ...
        total: AccountingAgingService._roundMoney(
