...
class SqlResultExportService {
  ...
  static String _toCsv(List<Map<String, Object?>> rows, List<String> columns) {
    ...
    for (final row in rows) {
      buffer.writeln(
          columns.map((column) => _csvCell(_cellText(row[column]))).join(','));
    }
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  static String _worksheetXml(
    ...
    for (var i = 0; i < rows.length; i++) {
      buffer.write(_xlsxRow(
          i + 2, [for (final column in columns) _cellText(rows[i][column])]));
    }
  ...
  static String _xlsxRow(int index, List<Object?> values) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      cells.add(
          '<c r="$ref" t="inlineStr"><is><t>${_xmlEscape(_cellText(values[i]))}</t></is></c>');
    }
