...
class GoogleDriveBackupService {
  ...
  static Future<GoogleDriveBackupSettings> loadSettings() async {
    ...
    final savedClientSecret = await _readSecretWithLegacyMigration(
      _clientSecretKey,
    );
    final refreshToken = await _readSecretWithLegacyMigration(_refreshTokenKey);
    final accessToken = await _readSecretWithLegacyMigration(_accessTokenKey);
    return GoogleDriveBackupSettings(
  ...
  static Future<String> _readSecretWithLegacyMigration(String key) async {
    ...
    await LocalDatabaseService.setSecureString(key, legacy);
  ...
  static Future<void> saveSettings(GoogleDriveBackupSettings settings) async {
    ...
    await LocalDatabaseService.setSecureString(
    ...
    await Future.wait(<Future<void>>[
      LocalDatabaseService.deleteString(_clientSecretKey),
      LocalDatabaseService.deleteString(_refreshTokenKey),
      LocalDatabaseService.deleteString(_accessTokenKey),
    ]);
  ...
  static Future<String> createBackupNow(
    ...
    try {
      ...
      final recoverySecret = store.appIdentity.recoveryKey.trim();
      if (recoverySecret.length < 8) {
        throw StateError(
          'A valid Store Recovery Key is required before encrypted automatic backups can run.',
        );
      }
  ...
  static Future<bool> _folderIdExists(String token, String folderId) async {
    final response = await http.get(
      Uri.https('www.googleapis.com',
          '/drive/v3/files/$folderId', const <String, String>{
