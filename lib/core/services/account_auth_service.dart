...
class AccountAuthCache {
  ...
  static const _adminTokenSecureKey = 'account_auth_admin_token_v1';
  static const _accountTokenSecureKey = 'account_auth_account_token_v1';
  static const _refreshTokenSecureKey = 'account_auth_refresh_token_v1';
  ...
  static AccountAuthCache? load() {
    ...
    try {
      ...
      return AccountAuthCache(
        ...
        adminToken: LocalDatabaseService.getSecureString(_adminTokenSecureKey) ??
            (json['adminToken'] ?? '').toString(),
        accountToken:
            LocalDatabaseService.getSecureString(_accountTokenSecureKey) ??
                (json['accountToken'] ?? '').toString(),
        refreshToken:
            LocalDatabaseService.getSecureString(_refreshTokenSecureKey) ??
  ...
  static Future<void> save(AccountAuthCache cache) async {
    await Future.wait(<Future<void>>[
      LocalDatabaseService.setSecureString(
        _adminTokenSecureKey,
        cache.adminToken,
      ),
      LocalDatabaseService.setSecureString(
        _accountTokenSecureKey,
        cache.accountToken,
      ),
      LocalDatabaseService.setSecureString(
        _refreshTokenSecureKey,
        cache.refreshToken,
      ),
    ]);
  ...
  /// Moves tokens written by older builds out of the local database and into
  /// platform secure storage. This routine is idempotent.
  static Future<void> migrateLegacySecrets() async {
    ...
    try {
      ...
      final admin =
          LocalDatabaseService.getSecureString(_adminTokenSecureKey) ??
              (json['adminToken'] ?? '').toString();
      final account =
          LocalDatabaseService.getSecureString(_accountTokenSecureKey) ??
              (json['accountToken'] ?? '').toString();
      ...
      json
        ...
        ..remove('refreshToken');
      await Future.wait(<Future<void>>[
        LocalDatabaseService.setSecureString(_adminTokenSecureKey, admin),
        LocalDatabaseService.setSecureString(_accountTokenSecureKey, account),
        LocalDatabaseService.setSecureString(_refreshTokenSecureKey, refresh),
      ]);
      await LocalDatabaseService.setString(key, jsonEncode(json));
    } catch (_) {
      // Keep authentication unavailable rather than exposing or guessing tokens.
    }
  }

  static Future<void> clear() async {
    await Future.wait(<Future<void>>[
      LocalDatabaseService.deleteSecureString(_adminTokenSecureKey),
      LocalDatabaseService.deleteSecureString(_accountTokenSecureKey),
      LocalDatabaseService.deleteSecureString(_refreshTokenSecureKey),
    ]);
