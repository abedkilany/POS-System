...
class CashLedgerService {
  ...
  Future<CashLedgerTransaction> reverseTransactionInExistingTransaction(
    ...
    if (existing != null) {
      final saved = _fromRow(existing.data);
      await _refreshOpenSessionExpectedCash(
        saved.cashDrawerSessionId,
    ...
    final reversal = CashLedgerTransaction(
      ...
      createdBy:
          createdBy.trim().isEmpty ? original.createdBy : createdBy.trim(),
      createdByUserId: createdByUserId.trim(),
      deviceId: deviceId.trim().isEmpty ? original.deviceId : deviceId.trim(),
      branchId: original.branchId,
      storeId: original.storeId,
      notes:
          reason.trim().isEmpty ? 'Reversal of ${original.id}' : reason.trim(),
      idempotencyKey: 'reversal:${original.id}',
      occurredAt: when,
      createdAt: when,
      updatedAt: when,
      reversalOfId: original.id,
      lastModifiedByDeviceId: deviceId.trim().isEmpty
  ...
  /// Rebuilds the persisted expected-cash cache for an open shift from the
  /// authoritative opening balance plus Cash Ledger movements. Keeping this
  /// inside the same database transaction as a reversal prevents a reversed
  /// expense/refund/transfer from leaving the shift cache stale.
  Future<void> _refreshOpenSessionExpectedCash(
    String sessionId,
    DateTime when,
  ) async {
    ...
    final row = await _db.customSelect(
      "SELECT opening_balance FROM cash_drawer_sessions WHERE id = ? AND status = 'open' LIMIT 1",
      variables: <Variable<Object>>[Variable<String>(cleanSessionId)],
    ).getSingleOrNull();
    if (row == null) return;
    final opening = (row.data['opening_balance'] as num?)?.toDouble() ?? 0.0;
    final movement = await movementTotalForSession(cleanSessionId);
    final expected = ((opening + movement) * 100).roundToDouble() / 100;
    await _db.customUpdate(
      ...
      variables: <Variable<Object>>[
        ...
        Variable<String>(when.toUtc().toIso8601String()),
        Variable<String>(cleanSessionId),
      ],
    );
  }
  ...
  Future<CashLedgerDetails> detailsFor(CashLedgerTransaction item) async {
    ...
    return CashLedgerDetails(
      ...
      sessionOpenedAt:
          DateTime.tryParse(session?.data['opened_at']?.toString() ?? ''),
      sessionClosedAt:
          DateTime.tryParse(session?.data['closed_at']?.toString() ?? ''),
  ...
  Future<void> _validateLinks(CashLedgerTransaction item) async {
    ...
    if (location == null) {
      throw StateError(
          'Cash location ${item.cashLocationId} does not exist or is inactive.');
    }
...
class CashLedgerDetails {
  const CashLedgerDetails({
    ...
    required this.cashLocationCode,
    required this.sessionNumber,
    required this.sessionStatus,
    required this.sessionOpenedAt,
    required this.sessionClosedAt,
    required this.sessionOpenedBy,
    required this.sessionClosedBy,
    required this.journalEntryId,
    required this.journalEntryNo,
