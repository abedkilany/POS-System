...
class DatabaseSqlEditorService {
  ...
  static String? validateStatement(String statement,
    ...
    if (!readOnly && RegExp(r'\baudit_logs\b').hasMatch(normalized)) {
      return 'audit_logs is append-only and cannot be modified from the SQL editor.';
    }
  ...
  static Future<List<SqlEditorResult>> runScript(String script,
      {required bool allowWrites}) async {
    ...
    await db.transaction(() async {
      for (final statement in statements) {
        final validationError =
            validateStatement(statement, allowWrites: allowWrites);
        if (validationError != null) throw ArgumentError(validationError);
        final isQuery = isQueryStatement(statement);
        if (isQuery) {
          final rows =
              await db.customSelect(_withPreviewLimit(statement)).get();
  ...
  static bool _isReadOnlyFirstKeyword(String first) =>
      first == 'select' || first == 'with' || first == 'explain';

  static String _withPreviewLimit(String statement) {
    final normalized = _normalizedSql(statement);
    if (RegExp(r'\blimit\s+\d+', caseSensitive: false).hasMatch(normalized)) {
      return statement;
    }
  ...
  static bool _containsKeyword(String normalizedSql, String keyword) {
    ...
    return RegExp(r'(^|\s)' + RegExp.escape(keyword) + r'(\s|$|\()')
