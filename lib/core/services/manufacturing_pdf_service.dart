...
class ManufacturingPdfService {
      ...
      pw.MultiPage(
        ...
        header: (context) => context.pageNumber == 1
            ...
            : ProfessionalPdfTheme.compactHeader(
                profile: profile, title: labels.recipe),
        ...
        build: (_) => [
          ...
          if (bom.notes.trim().isNotEmpty) ...[
            pw.SizedBox(height: 14),
            ProfessionalPdfTheme.note(labels.notes, bom.notes.trim(),
                isArabic: isArabic),
          ],
        ],
      ),
  ...
  static Future<Uint8List> buildManufacturingOrderPdf({
    ...
    final isArabic = labels.isArabic;
    final pdf = pw.Document(theme: await ProfessionalPdfTheme.loadTheme());
    final ratio = bom == null || bom.outputQuantity <= 0
        ? 0.0
        : order.quantity / bom.outputQuantity;
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) => context.pageNumber == 1
            ...
            : ProfessionalPdfTheme.compactHeader(
                profile: profile, title: labels.order),
        footer: (context) => ProfessionalPdfTheme.footer(
            context: context,
            profile: profile,
            isArabic: isArabic,
        ...
        build: (_) => [
          ...
          if (bom != null && bom.components.isNotEmpty) ...[
            ...
            pw.Text(labels.consumedMaterials,
                textDirection:
            ...
            ProfessionalPdfTheme.table(
              ...
              data: bom.components
                  .map(
                      (item) => [item.productName, _qty(item.quantity * ratio)])
                  .toList(growable: false),
              columnWidths: const {
          ...
          if (order.notes.trim().isNotEmpty) ...[
            pw.SizedBox(height: 14),
            ProfessionalPdfTheme.note(labels.notes, order.notes.trim(),
                isArabic: isArabic),
          ],
        ],
      ),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) => context.pageNumber == 1
            ...
            : ProfessionalPdfTheme.compactHeader(
                profile: profile, title: labels.orders),
        footer: (context) => ProfessionalPdfTheme.footer(
            context: context,
            profile: profile,
            isArabic: isArabic,
          ...
          if (!includeDetails)
            ...
            for (final order in orders) ...[
              _manufacturingOrderBlock(
                  order: order, bom: bomsById[order.bomId], labels: labels),
              pw.SizedBox(height: 10),
            ],
        ],
      ),
  ...
  static pw.Widget _manufacturingOrderBlock({
    ...
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(
          border: pw.Border.all(color: ProfessionalPdfTheme.line, width: .8)),
      child: pw.Column(
          ...
          if (bom != null && bom.components.isNotEmpty) ...[
            ...
            pw.Text(labels.noRecipeDetails,
                ...
                style: const pw.TextStyle(
                    color: ProfessionalPdfTheme.muted, fontSize: 8)),
          ],
        ],
      ),
    );
...
class _Labels {
  ...
  String _v(String ar, String en, String fr) => isArabic
      ? ar
      : isFrench
          ? fr
          : en;
  String get recipe =>
      _v('وصفة تصنيع', 'Manufacturing Recipe', 'Recette de fabrication');
  String get order =>
      _v('أمر تصنيع', 'Manufacturing Order', 'Ordre de fabrication');
  ...
  String get outputProduct =>
      _v('المنتج النهائي', 'Output product', 'Produit fini');
  String get outputQuantity =>
      _v('كمية الناتج', 'Output quantity', 'Quantité produite');
  ...
  String get producedQuantity =>
      _v('الكمية المصنعة', 'Produced quantity', 'Quantité fabriquée');
  String get status => _v('الحالة', 'Status', 'Statut');
  String get rawWarehouse => _v('مستودع المواد الأولية',
      'Raw materials warehouse', 'Entrepôt matières premières');
  String get finishedWarehouse => _v('مستودع المنتج النهائي',
      'Finished goods warehouse', 'Entrepôt produits finis');
  String get consumedMaterials =>
      _v('المواد المستهلكة', 'Consumed materials', 'Matières consommées');
  String get orders =>
      _v('أوامر التصنيع', 'Manufacturing Orders', 'Ordres de fabrication');
  String get orderCount => _v('عدد الأوامر', 'Order count', 'Nombre d’ordres');
  String get recipeDetails =>
      _v('تفاصيل الوصفة', 'Recipe details', 'Détails de la recette');
  String get noRecipeDetails => _v('لا توجد تفاصيل وصفة متاحة',
      'No recipe details available', 'Aucun détail de recette disponible');
}
