...
class ProfessionalPdfTheme {
  ...
  static pw.TextAlign textAlignFor(String value,
      {pw.TextAlign fallback = pw.TextAlign.left}) {
  ...
  static pw.Alignment alignmentFor(String value,
      {pw.Alignment fallback = pw.Alignment.centerLeft}) {
    final trimmed = value.trim();
    if (trimmed.isEmpty) return fallback;
  ...
  static pw.Widget brandRule() => pw.Directionality(
        ...
        child: pw.Row(
          children: [
            ...
            pw.SizedBox(width: 5),
            pw.Expanded(
                flex: 42, child: pw.Container(height: 2.2, color: navy)),
            pw.Expanded(
                flex: 18, child: pw.Container(height: 2.2, color: gold)),
            pw.Expanded(
                flex: 42, child: pw.Container(height: 2.2, color: navy)),
            pw.SizedBox(width: 5),
            pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    if (englishTitle.trim().isNotEmpty) ...[
                      ...
                      pw.Text(englishTitle.toUpperCase(),
                          textDirection: pw.TextDirection.ltr,
                          style: const pw.TextStyle(
                              color: gold, fontSize: 7, letterSpacing: 1.5)),
                    ],
                    if (meta.isNotEmpty) pw.SizedBox(height: 11),
                    ...meta.map((entry) => pw.Padding(
                          padding: const pw.EdgeInsets.only(bottom: 5),
                          child: metaItem(entry.key, entry.value,
                              isArabic: isArabic),
                        )),
                  ],
                ),
                    ...
                    if (logo != null)
                      ...
                      pw.Container(
                        ...
                        child: pw.Text(profile.name,
                            ...
                            textDirection: directionFor(profile.name),
                            style: pw.TextStyle(
                                color: navy,
                                fontSize: 18,
                                fontWeight: pw.FontWeight.bold)),
                ...
                child: pw.Column(
                  ...
                  children: [
                    pw.Text(profile.name,
                        textAlign: textAlignFor(profile.name,
                    ...
                    if (profile.phone.trim().isNotEmpty) ...[
                      ...
                      pw.Text(profile.phone.trim(),
                          ...
                          textDirection: pw.TextDirection.ltr,
                          style: const pw.TextStyle(color: ink, fontSize: 8.5)),
                    ],
                    if (profile.address.trim().isNotEmpty) ...[
                      pw.SizedBox(height: 5),
                      pw.Text(profile.address.trim(),
                          textAlign: textAlignFor(profile.address.trim(),
                              fallback: pw.TextAlign.right),
                          textDirection: directionFor(profile.address.trim()),
                          style: const pw.TextStyle(
                              color: muted, fontSize: 7.8, lineSpacing: 2)),
                    ],
                  ],
                ),
              ),
  ...
  static pw.Widget compactHeader(
          {required StoreProfile profile, required String title}) =>
      pw.Column(
        children: [
          pw.Directionality(
            textDirection: pw.TextDirection.ltr,
            child: pw.Row(
              ...
              children: [
                pw.Text(title,
                    textDirection: directionFor(title),
                    style: pw.TextStyle(
                        color: navy,
                        fontSize: 10,
                        fontWeight: pw.FontWeight.bold)),
  ...
  static String _localizedFooterNote(
      StoreProfile profile, String? languageCode) {
          ...
          child: pw.Row(
            ...
            children: [
              pw.Text(
                ...
                textDirection:
                    directionFor(_localizedFooterNote(profile, languageCode)),
                textAlign:
                    textAlignFor(_localizedFooterNote(profile, languageCode)),
  ...
  static pw.Widget metaItem(String label, String value,
          {required bool isArabic}) =>
      pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          ...
          pw.Text(value,
              ...
              textAlign: textAlignFor(value),
              style: pw.TextStyle(
                  color: ink, fontSize: 8.3, fontWeight: pw.FontWeight.bold)),
        ],
      );
  ...
  static pw.Widget infoStrip({
    ...
    return pw.Container(
      ...
      padding:
          pw.EdgeInsets.symmetric(horizontal: 13, vertical: verticalPadding),
      decoration: pw.BoxDecoration(
          color: soft, border: pw.Border.all(color: line, width: .7)),
  ...
  static pw.Table table({
    ...
    final resolvedCellStyle =
        cellStyle ?? const pw.TextStyle(color: ink, fontSize: 8);
  ...
  static pw.Widget summaryBox(
    ...
    return pw.Align(
      ...
      child: pw.Container(
        width: 260,
        decoration:
            pw.BoxDecoration(border: pw.Border.all(color: line, width: .8)),
        child: pw.Column(
          children: [
                  ...
                  child: pw.Row(
                    ...
                    children: [
                      pw.Text(rows[i].key,
                          style: pw.TextStyle(
                              color:
                                  i == highlightIndex ? PdfColors.white : muted,
                              fontSize: i == highlightIndex ? 9 : 8,
                              fontWeight: i == highlightIndex
                                  ? pw.FontWeight.bold
                                  : pw.FontWeight.normal)),
                      pw.Text(rows[i].value,
                          textDirection: directionFor(rows[i].value),
                          textAlign: textAlignFor(rows[i].value),
                          style: pw.TextStyle(
                              color:
                                  i == highlightIndex ? PdfColors.white : ink,
                              fontSize: i == highlightIndex ? 10.5 : 8.5,
                              fontWeight: pw.FontWeight.bold)),
                    ],
                  ),
                ),
  ...
  static pw.Widget note(String title, String value, {required bool isArabic}) =>
      pw.Container(
        width: double.infinity,
        padding: const pw.EdgeInsets.all(11),
        decoration:
            pw.BoxDecoration(border: pw.Border.all(color: line, width: .7)),
        child: pw.Column(
          ...
          children: [
            pw.Text(title,
                textDirection:
                    isArabic ? pw.TextDirection.rtl : pw.TextDirection.ltr,
                style: pw.TextStyle(
                    color: navy, fontSize: 8, fontWeight: pw.FontWeight.bold)),
