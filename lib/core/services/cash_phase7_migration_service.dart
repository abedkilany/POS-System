...
class CashPhase7MigrationService {
  ...
  Future<CashPhase7Report> run() async {
    ...
    try {
      ...
      final purchasesBeforeMigration =
          await BusinessSqliteStore.readPurchases(_db);
      final saleTotals = <String, double>{
        for (final sale in salesBeforeMigration) sale.id: sale.total
      };
      final purchaseTotals = <String, double>{
        for (final purchase in purchasesBeforeMigration)
          purchase.id: purchase.subtotal
      };

      final legacyRows = await _legacyPaymentRows();
      legacyReceiptsFound = legacyRows
          .where((row) => row['transaction_type'] == 'paymentReceived')
          .length;
      legacyPaymentsFound = legacyRows
          .where((row) => row['transaction_type'] == 'paymentPaid')
          .length;
      ...
      final beforeInvoiceEntries =
          await _countJournalEntries(<String>['sale', 'purchase']);
      ...
      final afterInvoiceEntries =
          await _countJournalEntries(<String>['sale', 'purchase']);
      invoiceEntriesCreated =
          max(0, afterInvoiceEntries - beforeInvoiceEntries);

      final beforeVoucherEntries = await _countJournalEntries(
          <String>['receipt_voucher', 'payment_voucher']);
      await _repairVoucherAccounting(issues);
      final afterVoucherEntries = await _countJournalEntries(
          <String>['receipt_voucher', 'payment_voucher']);
      voucherEntriesCreated =
          max(0, afterVoucherEntries - beforeVoucherEntries);
      ...
      await _db.customUpdate(
        ...
        ''',
        variables: <Variable<Object>>[
          Variable<String>(issues.any((i) => i.severity == 'error')
              ? 'blocked'
              : 'completed'),
          ...
          Variable<String>(issues.any((i) => i.severity == 'error')
              ? 'Phase 7 blocked by reconciliation errors.'
              : (issues.isEmpty
                  ? 'Phase 7 completed cleanly.'
                  : 'Phase 7 completed with ${issues.length} non-blocking reconciliation issue(s).')),
          Variable<String>(runId),
        ],
      );
    } catch (error) {
  ...
  Future<(int, int)> _migrateLegacyPayment(
    ...
    final amount = _number(row['debit']) > 0
        ? _number(row['debit'])
        : _number(row['credit']);
    ...
    final paymentMethod =
        (row['payment_method']?.toString().trim().isEmpty ?? true)
            ? 'Cash'
    ...
    if (isCash && cashLocationId.isEmpty) {
      issues.add(CashPhase7Issue(
        ...
        referenceNo: referenceNo,
        details:
            'Historical cash payment cannot be assigned to a valid cash drawer without guessing.',
      ));
      return (0, 0);
    }

    final voucherId =
        '${isReceipt ? 'legacy_receipt' : 'legacy_payment'}_$legacyId';
    final voucherNo = 'LEG-${isReceipt ? 'RC' : 'PV'}-$legacyId';
    final when = _date(row['transaction_date']?.toString(),
        fallback: _date(row['created_at']?.toString()));
    final now = DateTime.now().toUtc().toIso8601String();
    final currency = (row['currency']?.toString().trim().isEmpty ?? true)
        ? 'USD'
    ...
    if (referenceId.isNotEmpty) {
      ...
      if (target != null && target.data['party_id']?.toString() == accountId) {
        targetExists = true;
        final double targetTotal = (isReceipt
                ? saleTotals[referenceId]
                : purchaseTotals[referenceId]) ??
            0.0;
        ...
        if (amount > allocationAmount + _epsilon) {
          issues.add(CashPhase7Issue(
            ...
            referenceNo: referenceNo,
            details:
                'Only $allocationAmount of $amount could be allocated to the referenced invoice; the remainder stays as party credit/advance.',
          ));
        }
      } else if (target != null) {
        issues.add(CashPhase7Issue(
          ...
          referenceNo: referenceNo,
          details:
              'Payment party does not match the referenced invoice; voucher will remain unallocated.',
        ));
      }
    }
    ...
    await _db.transaction(() async {
      if (isReceipt) {
        await _db.customInsert(
          ...
          variables: <Variable<Object>>[
            ...
            Variable<double>(max(0.0, amount - allocationAmount)),
            Variable<String>(currency),
            Variable<String>(paymentMethod),
            Variable<String>(cashLocationId),
            Variable<String>(row['note']?.toString() ?? ''),
            Variable<String>(row['device_id']?.toString() ?? ''),
            Variable<String>(row['branch_id']?.toString() ?? ''),
            Variable<String>(row['store_id']?.toString() ?? ''),
            Variable<String>('phase7:legacy:$legacyId'),
            Variable<String>(
                row['created_at']?.toString().trim().isNotEmpty == true
      ...
      if (targetExists && allocationAmount > _epsilon) {
        await _db.customInsert(
          ...
          ''',
          variables: <Variable<Object>>[
            Variable<String>('phase7_alloc_$legacyId'),
            Variable<String>(isReceipt ? 'receipt' : 'payment'),
            Variable<String>(voucherId),
            Variable<String>(isReceipt ? 'sale' : 'purchase'),
            Variable<String>(referenceId),
            Variable<String>(referenceNo),
            Variable<double>(allocationAmount),
            Variable<double>(allocationAmount),
            Variable<String>(currency),
            Variable<String>(currency),
            Variable<String>(when.toIso8601String()),
  ...
  Future<void> _normalizeLegacyInvoiceAccounting(
    ...
    for (final sale in sales) {
      ...
      try {
        await AccountingService.reverseEntryForReference(
          ...
          referenceId: sale.id,
          reason:
              'Phase 7: separate historical invoice accounting from payment accounting',
        ...
        issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'legacy_sale_accounting_normalization_failed',
          entityType: 'sale',
          entityId: sale.id,
    ...
    for (final purchase in purchases) {
      ...
      try {
        ...
        issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'legacy_purchase_accounting_normalization_failed',
          entityType: 'purchase',
  ...
  Future<bool> _invoiceNeedsPaymentSeparation(
  ...
  Future<void> _repairVoucherAccounting(List<CashPhase7Issue> issues) async {
    for (final spec in <(String, String, String, String)>[
      ...
      final rows = await _db
          .customSelect(
              'SELECT * FROM ${spec.$1} WHERE deleted_at = \'\' AND status = \'posted\'')
          .get();
  ...
  Future<void> _rebuildOnePaymentCache({
    ...
    if (rawPaid > total + _epsilon) {
      issues.add(CashPhase7Issue(
        severity: 'error',
        type: 'overallocated_invoice',
        entityType: referenceType,
        entityId: id,
        details:
            'Active allocations total $rawPaid while invoice total is $total.',
      ));
    }
  ...
  Future<void> _runIntegrityChecks(List<CashPhase7Issue> issues) async {
    final checks = <(String, String, String)>[
      ...
      (
        'missing_purchase_journal',
        "SELECT id, purchase_no AS ref FROM purchases p WHERE p.deleted_at = '' AND lower(trim(p.status)) = 'received' AND NOT EXISTS (SELECT 1 FROM journal_entries je WHERE je.deleted_at = '' AND je.status = 'posted' AND je.reference_type = 'purchase' AND (je.reference_id = p.id OR je.reference_id LIKE p.id || ':purchase_edit:%') AND NOT EXISTS (SELECT 1 FROM journal_entries rev WHERE rev.reversed_entry_id = je.id AND rev.deleted_at = '' AND rev.status = 'posted'))",
        'Authoritative purchase journal is still missing after Phase 7 reconciliation.'
      ),
      (
    ...
    for (final check in checks) {
      ...
      for (final row in rows) {
        issues.add(CashPhase7Issue(
            ...
            referenceNo: row.data['ref']?.toString() ?? '',
            details: check.$3));
      }
    }
    ...
    for (final spec in <(String, String)>[
      ...
      for (final row in rows) {
        issues.add(CashPhase7Issue(
            severity: 'error',
            type: 'voucher_allocation_mismatch',
            entityType: spec.$1,
            entityId: row.data['id']?.toString() ?? '',
            referenceNo: row.data['ref']?.toString() ?? '',
            details:
                'Voucher amount=${row.data['amount']}, refunded=${row.data['refunded']}, allocated=${row.data['allocated']}, unallocated=${row.data['unallocated_amount']}.'));
      }
    }
    ...
    for (final row in invalidAllocations) {
      issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'invalid_payment_allocation',
          entityType: 'payment_allocation',
          entityId: row.data['id']?.toString() ?? '',
          referenceNo: row.data['ref']?.toString() ?? '',
          details: 'Allocation integrity failure: ${row.data['problem']}.'));
    }
    ...
    for (final row in duplicateJournals) {
      issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'duplicate_posted_journal',
          entityType: row.data['reference_type']?.toString() ?? '',
          entityId: row.data['reference_id']?.toString() ?? '',
          referenceNo: row.data['ref']?.toString() ?? '',
          details:
              'Financial reference has ${row.data['c']} posted journal entries.'));
    }
    ...
    for (final row in duplicateLedger) {
      issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'duplicate_cash_ledger_reference',
          entityType: row.data['reference_type']?.toString() ?? '',
          entityId: row.data['reference_id']?.toString() ?? '',
          referenceNo: row.data['ref']?.toString() ?? '',
          details:
              'Voucher reference has ${row.data['c']} active Cash Ledger rows.'));
    }
    ...
    for (final row in await _db.customSelect(r'''
      WITH expected AS (
        ...
        WHERE je.deleted_at = '' AND je.status IN ('posted', 'reversed') AND jl.party_type = 'customer' AND jl.party_id <> ''
    ...
    ''').get()) {
      issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'customer_control_balance_mismatch',
          entityType: 'customer',
          entityId: row.data['party_id']?.toString() ?? '',
          details:
              'Legacy/customer balance=${row.data['expected_balance']}, accounting receivable=${row.data['ledger_balance']}.'));
    }
    ...
    for (final row in await _db.customSelect(r'''
      WITH expected AS (
        ...
        WHERE je.deleted_at = '' AND je.status IN ('posted', 'reversed') AND jl.party_type = 'supplier' AND jl.party_id <> ''
        GROUP BY jl.party_id
      )
      SELECT e.party_id, e.balance AS expected_balance, COALESCE(l.balance, 0) AS ledger_balance
      FROM expected e LEFT JOIN ledger l ON l.party_id = e.party_id
    ...
    ''').get()) {
      issues.add(CashPhase7Issue(
          severity: 'error',
          type: 'supplier_control_balance_mismatch',
          entityType: 'supplier',
          entityId: row.data['party_id']?.toString() ?? '',
          details:
              'Legacy/supplier balance=${row.data['expected_balance']}, accounting payable=${row.data['ledger_balance']}.'));
    }
    ...
    for (final row in unbalanced) {
      issues.add(CashPhase7Issue(
          ...
          referenceNo: row.data['ref']?.toString() ?? '',
          details:
              'Posted journal is not balanced; difference=${row.data['difference']}.'));
    }
  }
  ...
  Future<int> _countLedger() async {
    final row = await _db
        .customSelect(
            "SELECT COUNT(*) AS c FROM cash_ledger_transactions WHERE deleted_at = ''")
        .getSingle();
    return (row.data['c'] as num?)?.toInt() ?? 0;
  }

  Future<void> _persistIssues(
      String runId, List<CashPhase7Issue> issues) async {
    final now = DateTime.now().toUtc().toIso8601String();
    for (final issue in issues) {
      await _db.customInsert(
        ...
        ''',
        variables: <Variable<Object>>[
          Variable<String>(_id('cash_p7_issue')),
          Variable<String>(runId),
          Variable<String>(issue.severity),
          Variable<String>(issue.type),
          Variable<String>(issue.entityType),
          Variable<String>(issue.entityId),
          Variable<String>(issue.referenceNo),
          Variable<String>(issue.details),
          Variable<String>(now),
        ],
      );
    }
