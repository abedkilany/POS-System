...
class BarcodeFeedbackService {
  ...
  static const String _vibrationEnabledKey =
      'barcode_feedback_vibration_enabled_v1';
  ...
  static final AudioPlayer _player =
      AudioPlayer(playerId: 'ventio_barcode_feedback');
  ...
  static BarcodeFeedbackSettings loadSettings() {
    ...
    return BarcodeFeedbackSettings(
      soundEnabled: soundRaw == null
          ? BarcodeFeedbackSettings.defaults.soundEnabled
          : soundRaw == 'true',
      vibrationEnabled: vibrationRaw == null
          ? BarcodeFeedbackSettings.defaults.vibrationEnabled
          : vibrationRaw == 'true',
      volume: double.tryParse(volumeRaw ?? '')?.clamp(0.0, 1.0).toDouble() ??
          BarcodeFeedbackSettings.defaults.volume,
    );
  }

  static Future<void> saveSettings(BarcodeFeedbackSettings settings) async {
    await LocalDatabaseService.setString(
        _soundEnabledKey, settings.soundEnabled.toString());
    await LocalDatabaseService.setString(
        _vibrationEnabledKey, settings.vibrationEnabled.toString());
    await LocalDatabaseService.setString(
        _volumeKey, settings.volume.toStringAsFixed(2));
  }
  ...
  static Future<bool> play({bool force = false}) =>
      _playAsset(_successSoundAsset, force: force);

  static Future<bool> playError({bool force = false}) =>
      _playAsset(_errorSoundAsset, force: force, errorFeedback: true);

  static Future<bool> _playAsset(String asset,
      {bool force = false, bool errorFeedback = false}) async {
