...
class BusinessSqliteStore {
  ...
  static final Map<String, Future<Set<String>>> _tableColumnFutures =
  ...
  static const String warehouseTransferOrdersKey =
      'warehouse_transfer_orders_v1';
  ...
  static Future<BusinessQueryPage<Expense>> queryExpenses(
    ...
    ''', variables: <Variable<Object>>[
      ...variables,
      Variable<int>(queryLimit),
      Variable<int>(safeOffset),
    ]).get();
    final hasMoreSearchRows = usesTextSearch && rows.length > safeLimit;
    final pageRows =
        hasMoreSearchRows ? rows.take(safeLimit).toList(growable: false) : rows;
  ...
  static Future<BusinessQueryPage<Product>> queryProducts(
    ...
    if (normalized.isNotEmpty) {
      conditions.add(
        "id IN (SELECT entity_id FROM search_index WHERE entity_type = 'product' AND search_text LIKE ?)",
      );
      variables.add(Variable<String>(_likePattern(normalized)));
    }
    final whereSql = conditions.join(' AND ');
    final safeLimit = _safeLimit(limit);
    final safeOffset = _safeOffset(offset);
    ...
    final rows = await db.customSelect('''
      SELECT id, name, code, name_en AS nameEn, name_ar AS nameAr,
             ...
             exchange_rate_at_entry AS exchangeRateAtEntry,
             category, barcode, brand, supplier, description, unit,
             ...
             image_path AS imagePath, tax_profile_id AS taxProfileId, created_at AS createdAt,
             updated_at AS updatedAt, deleted_at AS deletedAt,
             device_id AS deviceId, sync_status AS syncStatus,
             store_id AS storeId, branch_id AS branchId, version,
             last_modified_by_device_id AS lastModifiedByDeviceId
    ...
    final pageRows =
        hasMoreSearchRows ? rows.take(safeLimit).toList(growable: false) : rows;
    final productIds =
        pageRows.map((row) => row.read<String>('id')).toList(growable: false);
    final stockByIdentity = await _warehouseInventoryStockByIdentity(
      db,
  ...
  static Future<Map<String, Object?>> buildDashboardSummary(
    ...
    ''');
    const inventoryQuantityExpr = 'COALESCE(wt.quantity, 0)';
    ...
    final inventoryRow = await db.customSelect('''
      WITH warehouse_totals AS (
        ...
        SELECT bb.store_id, bb.product_id,
      ...
      LEFT JOIN batch_values bv
        ON bv.product_id = p.id AND bv.store_id = p.store_id
  ...
  static Future<Map<String, Object?>> buildReportsSummary(
    ...
    final metricsRows = await _intScalar(
      db,
    ...
    final accountingNetProfitFuture = db.customSelect(
      '''
      SELECT COALESCE(SUM(
        CASE
          WHEN a.type = 'revenue' THEN jl.credit - jl.debit
          WHEN a.type IN ('cost_of_sales', 'expense') THEN -(jl.debit - jl.credit)
          ELSE 0
        END
      ), 0) AS net_profit
    ...
    final cashTotalsFuture = db.customSelect(
      '''
      SELECT
    ...
    final cashByMethodFuture = db.customSelect(
      '''
      SELECT CASE WHEN trim(payment_method) = '' THEN 'not_specified'
      ...
      WHERE deleted_at = ''
        AND occurred_at >= ?
        AND occurred_at < ?
      GROUP BY CASE WHEN trim(payment_method) = '' THEN 'not_specified'
    ...
    final metricsFuture = db.customSelect('''
      SELECT
        SUM(CASE WHEN metric_date >= ? AND metric_date < ?
          THEN sales_total ELSE 0 END) AS todaySales,
        SUM(CASE WHEN metric_date >= ? AND metric_date < ?
          THEN sales_total ELSE 0 END) AS monthSales,
        SUM(CASE WHEN metric_date >= ? AND metric_date < ?
          THEN purchases_total ELSE 0 END) AS monthPurchases,
        SUM(expenses_total) AS totalExpenses,
        SUM(stock_in) AS movementIn,
        SUM(stock_out) AS movementOut
      FROM daily_metrics
    ''', variables: <Variable<Object>>[
      Variable<String>(_dateKey(today)),
      Variable<String>(_dateKey(tomorrow)),
      Variable<String>(_dateKey(monthStart)),
      Variable<String>(_dateKey(nextMonth)),
      Variable<String>(_dateKey(monthStart)),
      Variable<String>(_dateKey(nextMonth)),
    ]).getSingle();
    final balancesFuture = _accountBalanceSummary(db, today, tomorrow);
    final autoCorrectionsFuture = _recentStockMovements(
      db,
      limit: 100,
      movementType: 'auto_correction',
    );
    final lowStockFuture = _lowStockRows(db);
    final stockMovementsFuture = _recentStockMovements(db, limit: 100);
    final topProductLinesFuture = _reportsTopProductLines(db);
    const inventoryQuantityExpr = 'COALESCE(wt.quantity, 0)';
    final inventoryFuture = db.customSelect('''
      WITH warehouse_totals AS (
        SELECT store_id, product_id, COALESCE(SUM(quantity), 0) AS quantity
        FROM warehouse_inventory
        GROUP BY store_id, product_id
    ...
    final accountingNetProfit = await accountingNetProfitFuture;
    final cashTotals = await cashTotalsFuture;
    final cashByMethodRows = await cashByMethodFuture;
    final todayCashInByMethod = <String, double>{};
    final todayCashOutByMethod = <String, double>{};
    for (final row in cashByMethodRows) {
      final method = _rowText(row, 'payment_method', fallback: 'not_specified');
      final amount = _rowDouble(row, 'amount');
      if (_rowText(row, 'direction').toLowerCase() == 'in') {
        todayCashInByMethod[method] = amount;
      } else {
        todayCashOutByMethod[method] = amount;
      }
    }
    final inventory = await inventoryFuture;
    final balances = await balancesFuture;
    final autoCorrections = await autoCorrectionsFuture;
    final lowStock = await lowStockFuture;
    final stockMovements = await stockMovementsFuture;
    final topProductLines = await topProductLinesFuture;

    return <String, Object?>{
      'reference': reference.toIso8601String(),
      'totalExpenses': _rowDouble(metrics, 'totalExpenses'),
      'netProfit': _rowDouble(accountingNetProfit, 'net_profit'),
      'todaySales': _rowDouble(metrics, 'todaySales'),
      'monthSales': _rowDouble(metrics, 'monthSales'),
      'monthPurchases': _rowDouble(metrics, 'monthPurchases'),
      'movementIn': _rowDouble(metrics, 'movementIn'),
      'movementOut': _rowDouble(metrics, 'movementOut'),
      ...
      'todayCashIn': _rowDouble(cashTotals, 'cash_in'),
      'todayCashOut': _rowDouble(cashTotals, 'cash_out'),
  ...
  static Future<Map<String, Object?>> buildAccountingMetrics(
    ...
    final cashRow = await db.customSelect(
      ...
      ''',
      variables: <Variable<Object>>[
        Variable<String>(today.toUtc().toIso8601String()),
        Variable<String>(tomorrow.toUtc().toIso8601String()),
      ],
    ).getSingle();
    return <String, Object?>{
      'reference': reference.toIso8601String(),
      ...
      'todayCashIn': _rowDouble(cashRow, 'cash_in'),
      'todayCashOut': _rowDouble(cashRow, 'cash_out'),
    };
  }
  ...
  static Future<List<Map<String, Object?>>> queryAccountingAccountRows(
    ...
    final isSupplier = normalizedType == 'supplier';
    final table = isSupplier ? 'suppliers' : 'customers';
    final nameSearch = isSupplier
        ? '''
          lower(a.name) LIKE ? OR lower(a.name_en) LIKE ? OR
          lower(a.name_ar) LIKE ? OR lower(a.phone) LIKE ? OR
          lower(a.address) LIKE ? OR lower(a.id) LIKE ?
        '''
        : '''
          lower(a.name) LIKE ? OR lower(a.phone) LIKE ? OR
          lower(a.address) LIKE ? OR lower(a.id) LIKE ?
        ''';
    final variables = <Variable<Object>>[
    ...
    final normalized = query.trim().toLowerCase();
    final whereParts = <String>["a.deleted_at = ''"];
    if (normalized.isNotEmpty) {
      whereParts.add('($nameSearch)');
      final pattern = _likePattern(normalized);
      final searchCount = isSupplier ? 6 : 4;
      for (var i = 0; i < searchCount; i += 1) {
        variables.add(Variable<String>(pattern));
      }
    }
    variables.add(Variable<int>(_safeLimit(limit)));
    final rows = await db.customSelect('''
      WITH balances AS (
        SELECT account_id AS accountId, SUM(debit - credit) AS balance
        FROM account_transactions
        WHERE deleted_at = ''
          AND lower(account_type) = ?
          AND trim(account_id) <> ''
        GROUP BY account_id
      )
      SELECT a.id, a.name, a.phone, a.address,
             COALESCE(b.balance, 0) AS balance
      FROM $table a
      LEFT JOIN balances b ON b.accountId = a.id
      WHERE ${whereParts.join(' AND ')}
      ORDER BY ABS(COALESCE(b.balance, 0)) DESC, lower(a.name) ASC, a.id ASC
      LIMIT ?
    ''', variables: variables).get();
    return rows.map((row) {
      final phone = _rowText(row, 'phone');
      final address = _rowText(row, 'address');
      return <String, Object?>{
        'id': _rowText(row, 'id'),
        'name': _rowText(row, 'name'),
        'subtitle': <String>[phone, address]
            .where((part) => part.trim().isNotEmpty)
            .join(' - '),
        'balance': _rowDouble(row, 'balance'),
      };
    }).toList(growable: false);
  }
  ...
  static Future<void> _rebuildDailyMetrics(VentioDriftDatabase db) async {
    final now = DateTime.now().toUtc().toIso8601String();
    await db.customStatement('DELETE FROM daily_metrics');
    await db.customInsert('''
      INSERT OR REPLACE INTO daily_metrics (
        metric_date, sales_total, sales_profit, sales_count, purchases_total,
        expenses_total, stock_in, stock_out, cash_in, cash_out, updated_at
      )
      SELECT metric_date,
             ...
             ? AS updated_at
      FROM (
        SELECT substr(s.document_date, 1, 10) AS metric_date,
               SUM(max(
                 COALESCE(si.unit_price * si.quantity, 0) - s.discount,
                 0
               )) AS sales_total,
               SUM(COALESCE((si.unit_price - si.unit_cost) * si.quantity, 0))
                 - SUM(s.discount) AS sales_profit,
        ...
        WHERE s.deleted_at = ''
          ...
          AND trim(s.document_date) <> ''
        GROUP BY substr(s.document_date, 1, 10)
        UNION ALL
        SELECT substr(p.document_date, 1, 10) AS metric_date,
               0, 0, 0,
               SUM(COALESCE(pi.quantity * pi.unit_cost, 0)),
               0, 0, 0, 0, 0
        FROM purchases p
        LEFT JOIN purchase_items pi ON pi.purchase_id = p.id
        WHERE p.deleted_at = ''
          AND lower(p.status) NOT IN ('cancelled', 'returned')
          AND trim(p.document_date) <> ''
        GROUP BY substr(p.document_date, 1, 10)
        UNION ALL
        SELECT substr(expense_date, 1, 10) AS metric_date,
               0, 0, 0, 0, SUM(amount), 0, 0, 0, 0
        FROM expenses
        WHERE deleted_at = ''
          AND lower(expense_status) = 'posted'
          AND trim(expense_date) <> ''
        GROUP BY substr(expense_date, 1, 10)
        UNION ALL
        SELECT substr(movement_date, 1, 10) AS metric_date,
               0, 0, 0, 0, 0,
               SUM(CASE WHEN quantity > 0 THEN quantity ELSE 0 END),
               SUM(CASE WHEN quantity < 0 THEN abs(quantity) ELSE 0 END),
               0, 0
        FROM stock_movements
        WHERE deleted_at = '' AND trim(movement_date) <> ''
        GROUP BY substr(movement_date, 1, 10)
        UNION ALL
        SELECT substr(occurred_at, 1, 10) AS metric_date,
               0, 0, 0, 0, 0, 0, 0,
               SUM(CASE WHEN direction = 'in' THEN amount ELSE 0 END),
               SUM(CASE WHEN direction = 'out' THEN amount ELSE 0 END)
        FROM cash_ledger_transactions
        WHERE deleted_at = '' AND trim(occurred_at) <> ''
        GROUP BY substr(occurred_at, 1, 10)
      )
      WHERE trim(metric_date) <> ''
      GROUP BY metric_date
    ''', variables: <Variable<Object>>[Variable<String>(now)]);
  }

  static Future<void> _rebuildSearchIndex(VentioDriftDatabase db) async {
    final now = DateTime.now().toUtc().toIso8601String();
    ...
    await db.customInsert('''
      ...
      SELECT 'product', id,
             lower(trim(name || ' ' || name_en || ' ' || name_ar || ' ' ||
    ...
    await db.customInsert('''
      INSERT OR REPLACE INTO search_index
        (entity_type, entity_id, search_text, updated_at)
      SELECT 'sale', s.id,
             lower(trim(s.invoice_no || ' ' || s.customer_name || ' ' ||
                        s.payment_method || ' ' || s.payment_status || ' ' ||
                        s.status || ' ' || s.note || ' ' ||
                        COALESCE(group_concat(si.product_name || ' ' ||
                          si.unit_name, ' '), ''))),
             ?
      FROM sales s
      LEFT JOIN sale_items si ON si.sale_id = s.id
    ...
    await db.customInsert('''
      ...
      SELECT 'stock_movement', id,
             lower(trim(product_name || ' ' || reference_no || ' ' ||
                        movement_type || ' ' || warehouse_name || ' ' ||
                        reason || ' ' || notes)),
             ?
      FROM stock_movements
      WHERE deleted_at = ''
    ...
    try {
      await db.customStatement('DELETE FROM search_index_fts');
      await db.customInsert('''
        INSERT INTO search_index_fts(entity_type, entity_id, search_text)
        SELECT entity_type, entity_id, search_text FROM search_index
      ''');
    } catch (_) {
      // Fallback search_index table is authoritative when FTS5 is unavailable.
    }
  }

  static Future<void> _upsertSearchIndex(
    VentioDriftDatabase db, {
    ...
    if (deletedAt.trim().isNotEmpty || normalized.isEmpty) {
  ...
  static void _upsertSearchIndexIntoBatch(
    Batch batch, {
    required String entityType,
    required String entityId,
    required String searchText,
    ...
    batch.customStatement(
      ...
      VALUES (?, ?, ?, ?)
      ''',
      <Object?>[type, id, normalized, updatedAt],
    );
  }
  ...
  static Future<List<Map<String, Object?>>> _lowStockRows(
    ...
    final rows = await db.customSelect('''
      ...
      SELECT p.name, p.code, COALESCE(wt.quantity, 0) AS stock
      FROM products p
      LEFT JOIN warehouse_totals wt
        ON wt.product_id = p.id AND wt.store_id = p.store_id
      WHERE p.deleted_at = ''
        AND p.track_stock = 1
        AND COALESCE(wt.quantity, 0) <= p.low_stock_threshold
      ORDER BY COALESCE(wt.quantity, 0) ASC, lower(p.name) ASC
      LIMIT 100
    ''').get();
    return rows
        .map((row) => <String, Object?>{
  ...
  static Future<List<Map<String, Object?>>> _namedBalanceRows(
    ...
    final topEntries = entries.take(20).toList(growable: false);
    final ids = topEntries.map((entry) => entry.key).toList(growable: false);
  ...
  static Future<BusinessQueryPage<StockMovement>> queryStockMovements(
    ...
    if (normalizedWarehouseId.isNotEmpty) {
      conditions.add('warehouse_id = ?');
      variables.add(Variable<String>(normalizedWarehouseId));
    }
    if (from != null) {
      conditions.add('movement_date >= ?');
    ...
    if (reviewed != null) {
      conditions.add(reviewed ? "reviewed_at <> ''" : "reviewed_at = ''");
    }
    final normalized = query.trim().toLowerCase();
    if (normalized.isNotEmpty) {
      conditions.add(
        "id IN (SELECT entity_id FROM search_index WHERE entity_type = 'stock_movement' AND search_text LIKE ?)",
      );
      variables.add(Variable<String>(_likePattern(normalized)));
    }
    final whereSql = conditions.join(' AND ');
    ...
    final totalFuture = usesTextSearch
        ? null
        : _countWhere(db, 'stock_movements', whereSql, variables);
    final orderBy = switch (sortMode.trim().toLowerCase()) {
      'oldest' => 'movement_date ASC, updated_at ASC, id ASC',
      'product' => 'lower(product_name) ASC, movement_date DESC, id DESC',
      'quantity_high' => 'ABS(quantity) DESC, movement_date DESC, id DESC',
      'quantity_low' => 'ABS(quantity) ASC, movement_date DESC, id DESC',
      _ => 'movement_date DESC, updated_at DESC, id DESC',
    };
    final rows = await db.customSelect('''
      SELECT id, product_id, product_name, movement_type, quantity,
             movement_date, reference_id, reference_no, reason,
    ...
    final pageRows =
        hasMoreSearchRows ? rows.take(safeLimit).toList(growable: false) : rows;
    final total = usesTextSearch
        ? safeOffset + pageRows.length + (hasMoreSearchRows ? 1 : 0)
        : await totalFuture!;
    return BusinessQueryPage<StockMovement>(
      items: pageRows.map(_stockMovementFromRow).toList(growable: false),
      totalCount: total,
      limit: safeLimit,
      offset: safeOffset,
    );
  ...
  static Future<BusinessQueryPage<StockMovement>> queryWasteLossMovements(
    VentioDriftDatabase db, {
    int limit = 50,
    int offset = 0,
  }) async {
    const whereSql =
        "(movement_type = 'inventory_loss' OR (movement_type = 'inventory_adjustment' AND quantity < 0))";
    final total = await _countWhere(db, 'stock_movements', whereSql, const []);
    final safeLimit = _safeLimit(limit);
    final safeOffset = _safeOffset(offset);
    final rows = await db.customSelect('''
      SELECT id, product_id, product_name, movement_type, quantity,
  ...
  static Future<List<StockMovement>> readUnreversedInventoryCountMovements(
    VentioDriftDatabase db,
    String inventoryCountId,
  ) async {
    final rows = await db.customSelect('''
      SELECT sm.id, sm.product_id, sm.product_name, sm.movement_type,
             sm.quantity, sm.movement_date, sm.reference_id, sm.reference_no,
             sm.reason, sm.adjustment_category, sm.notes, sm.evidence_ref,
             sm.warehouse_id, sm.warehouse_name, sm.batch_id,
             sm.movement_group_id, sm.document_line_id, sm.source_movement_id,
             sm.reversal_of_movement_id, sm.idempotency_key, sm.unit_cost,
             sm.created_at, sm.updated_at, sm.device_id, sm.sync_status,
             sm.store_id, sm.branch_id, sm.version,
             sm.last_modified_by_device_id, sm.reviewed_at, sm.reviewed_by,
             sm.review_note
      FROM stock_movements sm
      WHERE sm.reference_id = ?
        AND sm.movement_type = 'count_adjustment'
        AND sm.deleted_at = ''
        AND NOT EXISTS (
  ...
  static Future<Map<String, int>> countStockMovementsAfterByProduct(
    ...
    if (countedAtByProduct.isEmpty) return const <String, int>{};
    final result = <String, int>{};
    for (final entry in countedAtByProduct.entries) {
      ...
      ''', variables: <Variable<Object>>[
        ...
        Variable<String>(_iso(entry.value.toLocal())),
      ]).get();
      result[productId] = rows.isEmpty ? 0 : rows.first.read<int>('c');
    }
    return result;
  }
  ...
  static Future<List<ProductCost>> readProductCosts(
      VentioDriftDatabase db) async {
    // ProductCost is a current snapshot, not an append-only ledger. Older
    // builds could create duplicate rows because the model has no explicit id.
    final rows = await db.customSelect('''
      WITH ranked AS (
      ...
      SELECT productId, averageCost, lastCost, currencyCode,
             createdAt, updatedAt
      FROM ranked
      WHERE rn = 1
  ...
  static Future<List<InventoryCostLayer>> readInventoryCostLayers(
    ...
    return rows.map((row) {
      final data = Map<String, dynamic>.from(row.data);
      // quantity_remaining is authoritative. is_closed is a denormalized cache
      // and must not hide a layer that still has quantity available.
      final remaining = (data['quantityRemaining'] as num? ?? 0).toDouble();
      data['isClosed'] = remaining <= 0.000001;
  ...
  static Future<List<Product>> readProducts(VentioDriftDatabase db) async {
    ...
    return productRows.map((row) {
      ...
      data['inventoryStock'] = derivedStock ?? 0;
  ...
  static Future<Map<String, Object?>?> findSaleBarcodeProduct(
    ...
    final productRows = await db.customSelect('''
      ...
      WHERE deleted_at = ''
        AND is_active = 1
        AND (lower(code) = ? OR lower(barcode) = ?)
      ORDER BY updated_at DESC, id DESC
      LIMIT 1
    ''', variables: <Variable<Object>>[
      Variable<String>(normalized),
    ...
    if (productRows.isNotEmpty) {
      final productId = productRows.first.read<String>('id');
      final product = await readProductById(db, productId);
  ...
  static Future<List<Sale>> readSales(VentioDriftDatabase db) async {
    ...
    final saleRows = await db.customSelect('''
      SELECT id, invoice_no AS invoiceNo, customer_name AS customerName,
             ...
             posted_snapshot_json AS postedSnapshotJson,
             $warehouseSelect,
             created_at AS createdAt, updated_at AS updatedAt,
             deleted_at AS deletedAt, device_id AS deviceId,
             sync_status AS syncStatus, store_id AS storeId,
    ...
    final batchAllocationRows = await db.customSelect('''
      SELECT sale_item_id AS saleItemId, line_no AS lineNo,
             batch_id AS batchId, quantity, unit_cost AS unitCost,
    ...
    return saleRows.map((row) {
      ...
      if (postedSnapshotRaw.trim().isNotEmpty) {
        try {
          ...
          if (decodedSnapshot is Map) {
            data['postedSnapshot'] = Map<String, dynamic>.from(decodedSnapshot);
          }
        } catch (_) {}
      }
  ...
  static Future<List<Sale>> queryEligibleDeliveryNoteSales(
    ...
    final rows = await db.customSelect('''
      ...
      FROM sales s
      WHERE s.deleted_at = ''
        AND lower(s.status) NOT IN ('cancelled', 'canceled')
        AND NOT EXISTS (
          SELECT 1
          FROM delivery_notes dn
          WHERE dn.sale_id = s.id AND dn.deleted_at = ''
        )
    ...
    final ids = rows
        ...
        .toList(growable: false);
    if (ids.isEmpty) return const <Sale>[];
    final sales = await _readSalesByIds(db, ids);
    final byId = <String, Sale>{for (final sale in sales) sale.id: sale};
    return ids.map((id) => byId[id]).whereType<Sale>().toList(growable: false);
  }
  ...
  static _SqlFilter _saleFilter({
    ...
    if (normalized.isNotEmpty) {
      conditions.add(
        "id IN (SELECT entity_id FROM search_index WHERE entity_type = 'sale' AND search_text LIKE ?)",
      );
      variables.add(Variable<String>(_likePattern(normalized)));
    }
    return _SqlFilter(
  ...
  static Future<BusinessQueryPage<Sale>> querySales(
    ...
    final totalFuture = usesTextSearch
        ? null
        : _countWhere(db, 'sales', filter.whereSql, filter.variables);
  ...
  static Future<List<Purchase>> readPurchases(VentioDriftDatabase db) async {
    ...
    final hasPayloadJson =
        await _tableHasColumn(db, 'purchases', 'payload_json');
    ...
    final itemRows = await db.customSelect('''
      SELECT id, purchase_id AS purchaseId, line_no AS lineNo,
             ...
             exchange_rate_at_entry AS exchangeRateAtEntry,
             requested_supplier_batch_number AS requestedSupplierBatchNumber,
             requested_manufacturing_date AS requestedManufacturingDate,
             requested_expiration_date AS requestedExpirationDate
      FROM purchase_items
        ...
        data['batchAllocations'] = <Map<String, dynamic>>[
          <String, dynamic>{
            ...
            'quantity': ((data['quantity'] as num? ?? 0).toDouble()) *
                ((data['conversionToBase'] as num? ?? 1).toDouble()),
            'supplierBatchNumber': data['requestedSupplierBatchNumber'] ?? '',
            'manufacturingDate': data['requestedManufacturingDate'] ?? '',
            'expirationDate': data['requestedExpirationDate'] ?? '',
          }
        ];
      } else {
        data['batchAllocations'] = const <Map<String, dynamic>>[];
      }
      itemsByPurchase
          .putIfAbsent(
            purchaseId,
      ...
      if (hasPayloadJson &&
        ...
        try {
          final payload = Map<String, dynamic>.from(
              jsonDecode(data['payloadJson'].toString()));
          data['warehouseId'] =
  ...
  static Future<List<InventoryCountSession>> readInventoryCounts(
    ...
    final sessionRows = await db.customSelect('''
      SELECT id, count_no AS countNo, created_at AS createdAt,
             ...
             reversal_journal_entry_id AS reversalJournalEntryId,
             reversed_at AS reversedAt, reversed_by AS reversedBy,
             reversal_reason AS reversalReason, updated_at AS updatedAt
    ...
    final lineRows = await db.customSelect('''
      SELECT id, inventory_count_id AS inventoryCountId, line_no AS lineNo,
             ...
             system_qty_at_approval AS systemQtyAtApproval,
             difference_qty AS differenceQty, unit_cost AS unitCost,
             difference_value AS differenceValue,
  ...
  static Future<List<ManufacturingOrder>> readManufacturingOrders(
    ...
    final hasWarehouseColumns = await _tableHasColumn(
            db, 'manufacturing_orders', 'raw_materials_warehouse_id') &&
        await _tableHasColumn(
            db, 'manufacturing_orders', 'finished_goods_warehouse_id');
    final payloadJsonColumn =
        hasPayloadJson ? 'payload_json AS payloadJson,' : '';
    final rows = await db.customSelect('''
      SELECT id, order_no AS orderNo, bom_id AS bomId, bom_name AS bomName,
             ...
             output_product_name AS outputProductName,
              ...
              actual_output_quantity AS actualOutputQuantity,
              total_material_cost AS totalMaterialCost,
              total_waste_cost AS totalWasteCost,
              total_eligible_cost AS totalEligibleCost,
              actual_unit_cost AS actualUnitCost,
              material_costs_json AS materialCostsJson,
              waste_lines_json AS wasteLinesJson,
              journal_entry_id AS journalEntryId,
              reversal_journal_entry_id AS reversalJournalEntryId,
    ...
    return rows.map((row) {
      final data = Map<String, dynamic>.from(row.data);
      for (final field in <String>['materialCosts', 'wasteLines']) {
        final jsonField = '${field}Json';
        final encoded = data[jsonField]?.toString() ?? '';
        if (encoded.isNotEmpty) {
          try {
            data[field] = jsonDecode(encoded);
          } catch (_) {
            data[field] = const <Map<String, dynamic>>[];
          }
        }
      }
      if (hasPayloadJson &&
        ...
        try {
          ...
          data['rawMaterialsWarehouseName'] =
              payload['rawMaterialsWarehouseName']?.toString().isNotEmpty ==
                      true
          ...
          data['finishedGoodsWarehouseName'] =
              payload['finishedGoodsWarehouseName']?.toString().isNotEmpty ==
                      true
  ...
  static Future<void> upsertEntityPayloads(
    ...
    if (key == productsKey && payloads.length > 1) {
      await _upsertProductPayloads(
        db,
        table,
        entityType,
        payloads,
    ...
    await db.transaction(() async {
      for (var index = 0; index < payloads.length; index += 1) {
        ...
        final sortIndex = sortIndices != null && index < sortIndices.length
            ...
            : null;
        final now = DateTime.now().toUtc().toIso8601String();
        final explicitId = payload['id']?.toString().trim() ?? '';
        final productCostProductId =
            key == productCostsKey ? _textValue(payload['productId']) : '';
        final id = explicitId.isNotEmpty
            ? explicitId
            : productCostProductId.isNotEmpty
                ? 'product_cost_$productCostProductId'
  ...
  static Future<void> _upsertProductPayload(
    ...
    await _upsertTypedEntityRow(
      ...
      typedColumns: <String, Object?>{
        ...
        'tax_profile_id': _textValue(payload['taxProfileId']),
      },
    );
    ...
    await _upsertSearchIndex(
      ...
      searchText: <Object?>[
        ...
        payload['unit'],
        payload['description'],
        ..._payloadMapList(payload['saleUnits']).expand(
          (unit) => <Object?>[unit['name'], unit['barcode']],
        ),
        ..._payloadMapList(payload['purchaseUnits']).expand(
          (unit) => <Object?>[unit['name'], unit['barcode']],
        ),
      ].join(' '),
      updatedAt: updatedAt,
  ...
  static Future<void> _upsertProductPayloads(
    VentioDriftDatabase db,
    String table,
    String entityType,
    List<Map<String, dynamic>> payloads, {
    ...
    await db.batch((batch) {
      for (var index = 0; index < payloads.length; index += 1) {
        ...
        batch.customStatement(
          ...
          'sort_index, name, code, name_en, name_ar, price, cost, original_cost, '
          'cost_currency, usd_cost, cost_exchange_rate_at_entry, original_price, '
          'original_currency, usd_price, exchange_rate_at_entry, category, '
          ...
          'minimum_receipt_shelf_life_days, is_active, image_path, tax_profile_id) '
          'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          <Object?>[
            id,
            entityType,
            ...
            updatedAt,
            deletedAt,
            _textValue(payload['deviceId']),
            _textValue(payload['syncStatus']),
            ...
            _doubleValue(payload['costExchangeRateAtEntry']),
            _doubleValue(payload['originalPrice'] ?? payload['price']),
            _textValue(payload['originalCurrency'], fallback: 'USD'),
            _doubleValue(payload['usdPrice'] ?? payload['price']),
            _doubleValue(payload['exchangeRateAtEntry']),
            _textValue(payload['category'], fallback: 'General'),
            _textValue(payload['barcode']),
            _textValue(payload['brand']),
            _textValue(payload['supplier']),
            _textValue(payload['description']),
            _textValue(payload['unit'], fallback: 'pcs'),
            _textValue(
              payload['quantityType'],
              fallback: ProductQuantityType.countable.code,
            ),
            ...
            _textValue(payload['taxProfileId']),
          ],
        );
  ...
  static void _insertProductUnitsIntoBatch(
    Batch batch,
    String table,
    String productId,
    Object? unitsValue,
    ...
    for (var index = 0; index < units.length; index += 1) {
      ...
      batch.customStatement(
        ...
        ''',
        <Object?>[
          '${productId}_${table}_$index',
          productId,
          index,
          unitId,
          _textValue(unit['name']),
          _doubleValue(unit['conversionToBase'], fallback: 1),
          _doubleValue(unit['price']),
          _doubleValue(
            unit['originalPrice'],
            fallback: _doubleValue(unit['price']),
          ),
          _textValue(unit['originalCurrency'], fallback: 'USD'),
          _textValue(unit['barcode']),
          _boolValue(unit['isDefault']) ? 1 : 0,
        ],
      );
    }
  }
  ...
  static Future<void> _insertProductUnits(
    ...
    await db.batch((batch) {
      _insertProductUnitsIntoBatch(batch, table, productId, unitsValue);
    });
  }
  ...
  static Future<void> _upsertSalePayload(
    ...
    var postedSnapshotJson = payload['postedSnapshot'] is Map
        ? jsonEncode(payload['postedSnapshot'])
        : '';
    final existingSnapshotRow = await db.customSelect(
      'SELECT posted_snapshot_json FROM $table WHERE id = ? LIMIT 1',
      variables: <Variable<Object>>[Variable<String>(id)],
    ).getSingleOrNull();
    final existingSnapshot =
        existingSnapshotRow?.data['posted_snapshot_json']?.toString() ?? '';
    if (existingSnapshot.trim().isNotEmpty) {
      // Posted document snapshots are append-once. Older sync clients may send
      // no snapshot at all; cancellations/payments may update the live row;
      // neither case is allowed to rewrite the historical document image.
      postedSnapshotJson = existingSnapshot;
    }
    final typedColumns = <String, Object?>{
      ...
      'payment_method': _textValue(payload['paymentMethod'], fallback: 'Cash'),
      'payment_status': _textValue(payload['paymentStatus'], fallback: 'paid'),
      'invoice_currency':
      ...
      'posted_snapshot_json': postedSnapshotJson,
    };
    ...
    await _upsertSearchIndex(
      ...
      searchText: <Object?>[
        ...
        payload['status'],
        payload['note'],
        ..._payloadMapList(payload['items']).expand(
          (item) => <Object?>[item['productName'], item['unitName']],
        ),
      ].join(' '),
      updatedAt: updatedAt,
      deletedAt: deletedAt,
  ...
  /// Persists one inventory-count session inside the caller-owned SQLite
  /// transaction. Phase 3 uses this so stock movements, accounting and the
  /// count status commit or roll back as one unit.
  static Future<void> upsertInventoryCountInTransaction(
    VentioDriftDatabase db,
    InventoryCountSession session,
  ) async {
    final payload = session.toJson();
    final existing = await db.customSelect(
      'SELECT sort_index FROM inventory_counts WHERE id = ? LIMIT 1',
      variables: <Variable<Object>>[Variable<String>(session.id)],
    ).getSingleOrNull();
    late final int sortIndex;
    if (existing != null) {
      sortIndex = (existing.data['sort_index'] as num?)?.toInt() ?? 0;
    } else {
      final next = await db
          .customSelect(
            'SELECT COALESCE(MAX(sort_index), 0) + 1 AS next_sort FROM inventory_counts',
          )
          .getSingle();
      sortIndex = (next.data['next_sort'] as num?)?.toInt() ?? 1;
    }
    await _upsertInventoryCountPayload(
      db,
      _tableByKey[inventoryCountsKey]!,
  ...
  static Future<void> _upsertInventoryCountPayload(
    ...
    await _upsertTypedEntityRow(
      ...
      typedColumns: <String, Object?>{
        ...
        'journal_entry_id': _textValue(payload['journalEntryId']),
        'reversal_journal_entry_id':
            _textValue(payload['reversalJournalEntryId']),
        'reversed_at': _dateString(payload['reversedAt']) ?? '',
        'reversed_by': _textValue(payload['reversedBy']),
        'reversal_reason': _textValue(payload['reversalReason']),
      },
    );
  ...
  static Future<void> _upsertManufacturingOrderPayload(
    ...
    await _upsertTypedEntityRow(
      ...
      typedColumns: <String, Object?>{
        ...
        'notes': _textValue(payload['notes']),
        'document_date': _dateString(payload['date']) ?? createdAt,
        'actual_output_quantity': _doubleValue(payload['actualOutputQuantity']),
        'total_material_cost': _doubleValue(payload['totalMaterialCost']),
        'total_waste_cost': _doubleValue(payload['totalWasteCost']),
        'total_eligible_cost': _doubleValue(payload['totalEligibleCost']),
        'actual_unit_cost': _doubleValue(payload['actualUnitCost']),
        'material_costs_json': jsonEncode(payload['materialCosts'] ?? const []),
        'waste_lines_json': jsonEncode(payload['wasteLines'] ?? const []),
        'journal_entry_id': _textValue(payload['journalEntryId']),
        'reversal_journal_entry_id':
            _textValue(payload['reversalJournalEntryId']),
        'completed_at': _dateString(payload['completedAt']) ?? '',
        'completed_by': _textValue(payload['completedBy']),
        'reversed_at': _dateString(payload['reversedAt']) ?? '',
        'reversed_by': _textValue(payload['reversedBy']),
        'reversal_reason': _textValue(payload['reversalReason']),
      },
  ...
  static Future<void> upsertBillOfMaterialsPayloadInTransaction(
    VentioDriftDatabase db,
    Map<String, dynamic> payload, {
    required String id,
    required String payloadJson,
        ...
        for (var batchIndex = 0;
          ...
          await db.customInsert(
            ...
            INSERT OR REPLACE INTO sale_item_batch_allocations
              (id, sale_item_id, line_no, batch_id, quantity, unit_cost, expiration_date)
            SELECT ?, ?, ?, ?, ?, ?, ?
            ...
            variables: <Variable<Object>>[
              ...
              Variable<double>(_doubleValue(allocation['unitCost'])),
  ...
  static Future<void> _insertPurchaseItems(
    ...
    for (var index = 0; index < items.length; index += 1) {
      final item = items[index];
      final stableLineId = _textValue(item['lineId']).trim().isEmpty
          ? '$purchaseId:$index'
          : _textValue(item['lineId']).trim();
      final batchAllocations = _payloadMapList(item['batchAllocations']);
      final requestedAllocation =
          batchAllocations.isEmpty ? null : batchAllocations.first;
      await db.customInsert(
        '''
      ...
      for (var batchIndex = 0;
        ...
        await db.customInsert(
          ...
          ''',
          variables: <Variable<Object>>[
            Variable<String>('$stableLineId:batch:$batchIndex'),
            Variable<String>(stableLineId),
            Variable<int>(batchIndex),
      ...
      await db.customInsert(
        ...
        INSERT OR REPLACE INTO inventory_count_lines
          (id, inventory_count_id, line_no, product_id, product_name,
           ...
           note, system_qty_at_approval, difference_qty, unit_cost,
           difference_value, stock_movement_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''',
        variables: <Variable<Object>>[
          ...
          Variable<double>(_doubleValue(line['systemQtyAtApproval'])),
          Variable<double>(_doubleValue(line['differenceQty'])),
          Variable<double>(_doubleValue(line['unitCost'])),
          Variable<double>(_doubleValue(line['differenceValue'])),
          Variable<String>(_textValue(line['stockMovementId'])),
        ],
      );
    }
  }
    ...
    for (final table in _tableByKey.values) {
      if (await _tableHasColumn(db, table, 'deleted_at')) {
        await db.customUpdate(
          "UPDATE $table SET deleted_at = ? WHERE deleted_at = ''",
          variables: <Variable<Object>>[Variable<String>(now)],
        );
      } else {
        await db.customStatement('DELETE FROM $table;');
  ...
  static Future<Map<String, double>> _warehouseInventoryStockByIdentity(
    ...
    if (normalizedProductIds.isNotEmpty) {
      conditions.add(
        'product_id IN (${List.filled(normalizedProductIds.length, '?').join(', ')})',
      );
      variables.addAll(normalizedProductIds.map(Variable<String>.new));
    }
  ...
  static Future<void> _mergeEntityList(
    ...
    final supportsSoftDelete = await _tableHasColumn(db, table, 'deleted_at');
    final deletedAt = DateTime.now().toUtc().toIso8601String();
    for (final row in existingRows) {
      final id = row.read<String>('id');
      if (!seenIds.contains(id)) {
        // Business snapshots must never physically remove rows referenced by
        // stock, costing, accounting, or audit history. An empty in-memory
        // list can occur briefly during recovery before the backup payload is
        // imported; hard deletion here both loses history and violates FKs.
        if (supportsSoftDelete) {
          await db.customUpdate(
            "UPDATE $table SET deleted_at = ? WHERE id = ? AND deleted_at = ''",
            variables: <Variable<Object>>[
              Variable<String>(deletedAt),
              Variable<String>(id),
            ],
          ...
          await db.customStatement(
              'DELETE FROM $table WHERE id = ?;', <Object?>[id]);
        }
      }
    }
  }
  ...
  static Future<Set<String>> _tableColumns(
    ...
    return _tableColumnFutures.putIfAbsent(table, () async {
