...
class SqliteMigrationManager {
  ...
  static Future<SqliteMigrationStatus>
      initializeFromExistingSqliteIfValidated() async {
    if (_initialized && _database != null) {
      return const SqliteMigrationStatus(
        ...
        message:
            'SQLite/Drift phase 3B already initialized from validated typed tables.',
      );
    }
    ...
    try {
      ...
      return const SqliteMigrationStatus(
        ...
        lastStatus: 'completed',
        message:
            'SQLite/Drift phase 3B restored from validated typed tables; legacy storage is not opened.',
      );
        ...
        try {
          ...
          final busy = (data['busy'] as num?)?.toInt() ??
              (data.values.isNotEmpty
                  ? (data.values.first as num?)?.toInt()
                  : 0) ??
              0;
          if (busy == 0) {
            checkpointError = null;
