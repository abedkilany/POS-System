...
class InventoryReconciliationRepository {
  ...
  static Future<List<Product>> listProductsWithoutInventoryRow(
    ...
    final rows = await db.customSelect('''
      SELECT p.id, p.name, p.code, p.name_en AS nameEn, p.name_ar AS nameAr,
             ...
             p.category, p.barcode, p.brand, p.supplier,
  ...
  static Future<void> backfillFromLegacyData(
    ...
    final productColumns = await db.customSelect('PRAGMA table_info(products);').get();
    final hasLegacyStockColumn = productColumns.any(
      (row) => (row.data['name']?.toString() ?? '') == 'stock',
    );
    if (!hasLegacyStockColumn) {
    ...
    await db.transaction(() async {
      ...
      for (final productRow in products) {
        ...
        // warehouse_inventory is authoritative. If this product already has
        // any authoritative row, never let the retired products.stock value
        // overwrite or "reconcile" it during schema retirement. The legacy
        // value is consulted only for genuinely old products that have no
        // warehouse inventory at all.
        final existingInventory = await db.customSelect(
          '''
          SELECT 1
          FROM warehouse_inventory
        ...
        final rowsForProduct =
            groupedMovements[productKey] ?? const <QueryRow>[];
      ...
      final inventoryProducts = <String>{
        for (final row in products) row.read<String>('id')
      };
