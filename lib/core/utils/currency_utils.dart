...
double normalizeAccountingAmount(
  ...
  return _roundToDecimals(
      value, accountingDecimalsForCurrency(currency, profile));
}
...
double exchangeRate(
  ...
  if (from == to) return 1;
  final configured =
      profile.exchangeRateForDate(from, to, effectiveAt: effectiveAt);
...
double convertCurrency(
  ...
  final to = toCurrency.toUpperCase();
  final converted =
      amount * exchangeRate(from, to, profile, effectiveAt: effectiveAt);
  return normalizeResult
      ? normalizeAccountingAmount(converted, to, profile)
      : converted;
}
...
String formatCurrency(
  ...
  final separator =
      symbol.endsWith(' ') || symbol.length <= 2 && symbol != 'LBP' ? '' : ' ';
...
double toUsdReferencePrice(
    double amount, String currency, StoreProfile profile) {
  return convertCurrency(amount, currency, 'USD', profile,
      normalizeResult: false);
}

double fromUsdReferencePrice(
    double usdAmount, String currency, StoreProfile profile) {
