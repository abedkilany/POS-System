...
check('manufacturing keeps strict physical guard when negative stock is disabled', "if (!_storeProfile.allowNegativeStock)" in MFG and "operation: 'manufacturing'" in MFG and 'requirePhysicalUnifiedStockInTransaction' in MFG)
check('manufacturing honors store negative-stock policy', 'allowNegativeStock: _storeProfile.allowNegativeStock' in MFG and 'includeProvisionalShortage: true' in MFG)
...
check('sqlite sync rejects deficit transfer/output but allows manufacturing input deficit lineage', 'BatchInventoryService.isDeficitBatchId(movement.batchId)' in SQLITE_SYNC and "normalizedType.startsWith('transfer_')" in SQLITE_SYNC and "normalizedType.startsWith('manufacturing_produce')" in SQLITE_SYNC)
